...
runs once, the conversation is created, and the entry is written. Absence is read
per endpoint and nothing else is consulted, so an endpoint whose
[principal](./principal) already holds conversations still gets a new one.
