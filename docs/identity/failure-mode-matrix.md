...
| 3 | WASM policy bundle panic | Tier-3 component trap/panic during request evaluation (wasmtime host abort). Phase 3; not wired in Phase 1. | **CLOSED** | `-32101` `policy_fault` **(proposed;** defined in [MCP_GATEWAY_PLAN.md](../../MCP_GATEWAY_PLAN.md) §6, not yet in `rpc_codes.rs`) | `{prefix}.audit.error.request.{method_root}` | Bundle rollback: repoint active version in `mcp-gateway-config` KV; signal `mcp.control.bundle.reload`. Disable Tier-3 in bundle manifest **(proposed)**. | Tenant WASM is untrusted code; panic is indistinguishable from deny for security purposes (same as agentgateway `failClosed`). |
...
| 5 | WASM bundle missing for tenant | No active bundle pointer for tenant in `mcp-gateway-config` KV (day-zero or mis-provisioned). | **CLOSED** | `-32108` `no_policy` **(proposed;** plan §6) | `{prefix}.audit.deny.request.{method_root}` | Bootstrap bundle in KV; org-wide base layer + tenant override per [MCP_GATEWAY_PLAN.md](../../MCP_GATEWAY_PLAN.md) Block C "Bootstrap / day-zero". `MCP_GATEWAY_SPICEDB_ENDPOINT` unset still allow-all **(today)** without bundle. | Default deny for policy-gated traffic prevents ungoverned MCP egress; empty cluster must not imply allow. |
