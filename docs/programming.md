...
## Gap summary by runner (current state — May 2026)

| Feature | acp-runner | xai-runner | codex-runner | openrouter-runner |
|---------|-----------|-----------|-------------|------------------|
| TROGON.md injection | ✅ | ✅ | ❌ (Stage 5) | ✅ |
| Full trogon-tools (13 tools) | ✅ | ✅ | via Codex CLI | ✅ |
| Bash (wasm-runtime) | ✅ stateful | ✅ stateless | Codex-native | ✅ stateless |
| export/import (cross-runner) | ✅ lossy v1 | ✅ | ✅ | ✅ |
| Permission modes enforced | partial (Stage 1) | ❌ (Stage 1) | ❌ (Stage 1) | ❌ (Stage 1) |
| Compactor at 85% | ✅ | trim only (Stage 5) | Codex-managed | trim only (Stage 5) |

See [docs/terminal-agent-definitive-plan.md](terminal-agent-definitive-plan.md) for the full PR sequence and bug list.
...
- **Codex pending_history requires empirical validation**: the mechanism of prepending
  imported history as text in the first `userInput` bypasses the Codex CLI's internal
  thread history management and has not been tested against the real Codex CLI. PR 7
  must not be merged until this is validated. If the mechanism fails, the fallback for
  v1 is to not support history import in codex-runner — when the user switches to
  codex, a fresh session starts without prior context. Less convenient but correct.
