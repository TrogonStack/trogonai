...
**1. The agent is a resource, but a thin and unversioned one.** Making the agent server-side removes config drift between callers and keeps the definition out of every request body. But the design stops short: mutation is in-place with no revision history, and the session copies only the `name`. The gap is precise and worth stating as a field-level absence rather than a complaint: `AgentSession.agent` returns `instructions`, `model`, `tools`, and the rest of the settings, but carries no `version`, `revision`, `etag`, or `digest`, and `POST /agents/{agent_id}` updates the saved agent in place. A past session therefore exposes configuration *values* with no immutable identity for the configuration those values came from. A team that edits a production agent has no documented way to tell which configuration a past session actually ran with, and no way to pin one. The docs give no reasoning for this; the closest they come is the framing "The session holds the conversation and work, while the saved agent holds reusable settings", which treats the agent as configuration rather than as an audited artifact.
...
**5. Vault credentials bind by URL, not by name.** "The Agents API selects a credential that matches the server URL." This makes attachment declarative (add a vault ID, keep the URL consistent, credentials appear) but it is implicit: nothing in the session request states which credential will be used, and ambiguity is resolved only by optionally setting `credential_id`. What happens when no credential matches is **not documented**: the docs establish the match rule and the `credential_id` override but never state the no-match behavior, so whether a URL typo produces an unauthenticated request, a tool error, or a session failure is an evidence gap rather than a finding. An earlier draft of this dossier asserted the unauthenticated-request reading; it is not supported by any cited page and has been withdrawn.
