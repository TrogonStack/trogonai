...
9. **Secrets are write-only by construction, and the `environment_variable` type puts the substitution boundary at network egress, not in the sandbox.** The agent holds a placeholder; the platform swaps it at the proxy, scoped by request part (`injection_location.header` / `.body`) and, **only under `networking: {"type":"limited"}`**, by host (`allowed_hosts`, max 16). Under `{"type":"unrestricted"}` there is no per-credential host scope at all and, verbatim, "The Environment's network policy is the only boundary on where the secret can reach." This is the single most interesting decision in the whole surface, stated precisely: a compromised agent process cannot *read* the secret value. It can still use that value against every host the applicable boundary permits, and read the response, so the mechanism bounds where the credential reaches rather than what the agent learns by using it. `injection_location` is independent of `allowed_hosts` precisely so "where it can go" and "where in the request it lands" are separate axes.

10. **Vaults are workspace-scoped, and the docs flag that as a hazard rather than a feature.** "any API key with workspace access can reference them when creating a session. To revoke access, delete the vault or credential." There is no per-agent or per-session ACL on a vault. The revocation primitives are the archive/delete pair in finding 11, not a single destroy: archiving purges the payload while retaining the record for audit, and deleting hard-deletes the record. The docs give the instruction as "To revoke access, delete the vault or credential" without stating whether either operation affects a session already running on that credential, which is recorded in the gaps below.
...
- **Compliance**: "Managed Agents is not currently eligible for Zero Data Retention (ZDR) or HIPAA Business Associate Agreement (BAA) coverage." (source: https://platform.claude.com/docs/en/managed-agents/overview)
- **Whether revoking a credential affects a running session is not documented.** "To revoke access, delete the vault or credential" is stated without a stated effect on sessions already holding a substitution placeholder for it, and neither the archive nor the delete description mentions in-flight sessions.
- **Redirect handling for `environment_variable` substitution is not documented.** The docs fix the host scope and the request part but say nothing about whether a redirect target is revalidated against `allowed_hosts`, or whether the substituted header is stripped when it is not. This matters because an allowlisted host can redirect to one that is not.
