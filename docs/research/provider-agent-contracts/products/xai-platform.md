...
- Because `store` defaults to `true` and responses are retained 30 days, the input messages and model responses of a request carrying an MCP `authorization` token are stored server-side for 30 days unless the caller sets `store: false`. Whether the `tools[].authorization` value itself is part of that stored record, and whether it is redacted if so, is **not documented** and I could not verify it: the `store` contract is described in terms of messages and responses, and says nothing about the tool configuration block. `store: false` also says nothing about request logging, which is a separate retention path the docs do not describe at all. Treat the retention of the third-party token as an open evidence gap, not as an established finding.
