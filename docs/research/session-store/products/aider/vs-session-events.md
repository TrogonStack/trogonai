...
(ADR#0035 decisions 1-6), and the workspace is a referenced, versioned fact
...
| Aider | Ours | Verdict |
| --- | --- | --- |
| `.aider.chat.history.md`, appended per UI event, write-mostly (`aider/io.py:1117-1136`, `:789`, `:795`, `:905`, `:923`, `:960`, `:970`, `:973`, `:999`) | `UserMessageRecorded`, `AssistantMessageStarted`/`Completed`, typed and always folded into the model-visible context (ADR#0035 facet 8) | Ours, decisively: theirs is not even read back on a normal run |
...
| No session id; identity is the working directory / git root expressed as fixed file names (`aider/args.py:271-276`) | Opaque `SessionId`, one logical stream per session on `session.sessions.events.<session_id>` (ADR#0035 decision 1) | Gap, deliberate on Aider's part: there is nothing to list, resume by id, or reference from outside the one shared file |
...
| Mode switching (`/code`, `/ask`, `/architect`) via `Coder.create(from_coder=...)`, sharing one `io` instance and one file (`aider/coders/base_coder.py:125-181`, `:146`, `:171-179`; `aider/coders/architect_coder.py:9-46`) | `DelegationDispatched`, `ParentLinked`, `CascadePolicy` (ADR#0035 decision 6) | Gap, deliberate: Aider has no subagent concept at all, only an in-process object handoff with no persisted parent-child link and no independent child lifecycle |
| No retention policy, no TTL, no delete verb; `check_gitignore` only excludes the files from the user's own git history (`aider/main.py:155-171`, `:163-164`) | `SessionHidden`, `RedactionApplied`, `ArtifactErased` (ADR#0035 decision 7) | Ours, decisively: no analog of any kind exists in Aider |
| No format-version field; heuristic Markdown line-classification parser, silently accepts foreign input (`aider/utils.py:148-188`) | Typed protobuf events, schema-validated at the append and replay boundary (`validate_session_event`, ADR#0035 decision 3) | Ours, decisively |
| `.aider.tags.cache.v{3,4}/`, a rebuildable code-search cache keyed by file identity (`aider/repomap.py:43`, `:186-224`) | Not a session concept on either side; closest conceptual analog is a rebuildable projection (ADR#0035 decision 8), but keyed by conversation, not source file | Not comparable: different problem (code search vs. conversation state), noted for completeness only |
...
| No lock, no fsync, no torn-write handling; concurrent writers interleave undetected (`aider/io.py:1131-1136`) | Every write path carries an explicit `WRITE_PRECONDITION` (ADR#0035 decision 2), enforced by JetStream at the broker | Ours, decisively |
...
- **Typed, schema-validated events versus a heuristic parser with no version
  ...
  malformed event before it is ever persisted (ADR#0035 decision 3).
- **A guarded write path versus an unlocked, unmanaged append.** Aider opens
  ...
  explicit `WRITE_PRECONDITION` enforced by the broker (ADR#0035 decision 2).
- **A real retention and redaction contract versus none.** `SessionHidden`,
  `RedactionApplied`, and `ArtifactErased` (ADR#0035 decision 7) have no
...
- **Restore-as-reseed versus resume-as-replay.** Aider's own documentation
  ...
  (`SessionForked`, ADR#0035 decision 5, inheriting a context prefix by
  reference into a genuinely new session) than to our resume semantics
  (ADR#0035 decision 8, replaying the tail of the *same* aggregate). Aider
...
- **Plain append with no lock and no fsync.** `open(path, "a")` /
  ...
  per-command `WRITE_PRECONDITION` (ADR#0035 decision 2) exists to prevent.
...
- **Naming that does not match the mechanism.** `/save` and `/load`
  ...
  ADR#0035's own vocabulary (harness recovery checkpoint, aggregate
...
- Should ADR#0035's Context or Consequences state explicitly that durable,
...
- Is the boundary between "resume this exact session" (ADR#0035 decision 8,
