...
| `a2a.agents.{agent_id}.{method}`               | Gateway → agent backend                                       | Gateway                    |
| `a2a.tasks.{task_id}.events.{req_id}`          | Per-task event stream (JetStream)                             | Agent (via gateway)        |
...
- **Liveness from PING is theatre on the discovery path.** "Agent responded to PING" ≠ "agent will accept the next message". The request/reply on `a2a.agents.{id}.message.send` is the real liveness signal — and it's the call the client was about to make anyway.
...
client ──PUB──▶ a2a.gateway.{agent_id}.message.send  (JSON-RPC envelope, reply_to = _INBOX.x)
        ...
        6. publish a2a.agents.{agent_id}.message.send
...
- On `message/stream`, the gateway:
  ...
  2. ensures the Account's `A2A_EVENTS` JetStream stream with subject filter `a2a.tasks.>` exists,
  3. forwards the request to the agent on `a2a.agents.{agent_id}.message.stream` with `events_subject = a2a.tasks.{task_id}.events`,
...
The agent publishes each `Task` / `TaskStatusUpdateEvent` / `TaskArtifactUpdateEvent` to `a2a.tasks.{task_id}.events`. The gateway consumer rewrites/redacts in flight and forwards to the caller inbox.
...
4. **Task stream manager** — ensures the Account's `A2A_EVENTS` JetStream stream exists with subject filter `a2a.tasks.>`, configures retention (default `interest` with `max_age = 24h`, per-Account override), provisions consumers.
...
Agents subscribe to `a2a.agents.{agent_id}.>` as a queue group. An A2A SDK adapter handles:
...
caller ──PUB──▶ a2a.gateway.{agent}.message.stream
              ...
              └────────────────────┘
                        │
        forward ──PUB──▶ a2a.agents.{agent}.message.stream
                                        │
                                        ▼
                              ┌─────────────────┐
                              │  Agent backend  │
                              └─────────────────┘
                                        │
                  publish events ──PUB──▶ a2a.tasks.{task_id}.events
                                        │
                ┌───────────────────────┘
                ▼
...
- **Stream topology = shared `A2A_EVENTS` per Account, subject-filtered `a2a.tasks.>`**. Per-task streams rejected — JS metadata cost dominates at fleet scale.
...
- **`a2a-bridge` = HTTPS sidecar; per-request re-mint into caller's Account; SSE↔JetStream consumer mapping** on `a2a.tasks.{task_id}.events.>`.
