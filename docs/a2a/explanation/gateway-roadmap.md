...
| Document | Purpose |
|----------|---------|
| [A2A architecture](architecture.md) | Architecture, subject shapes, policy tiers, SpiceDB tuples, audit schema |
| [A2A architecture](architecture.md) | Open engineering items and suggested ordering (auth callout, Tier 1, gateway audit) |
| [Streaming back-pressure](../how-to/operators/streaming-backpressure.md) | Task event egress — pull consumer flow control, `A2A_EVENTS` policy, agent `Bridge` limits |
| [NSC account bootstrap](../how-to/operators/nsc-account-bootstrap.md) | Per-Account NSC provisioning, caller/gateway/registrar ACL templates |
...
Env knobs (see [Runtime env](../reference/runtime-env.md)):
...
Do not implement auth-callout or SpiceDB in this crate until NSC + ACL templates from the bootstrap runbook are in place. Track ordering in [A2A architecture](architecture.md) §Suggested ordering.

### NATS `$SYS` / auth-callout integration

- [ ] Wire gateway to expect **Account-bound User JWTs** minted by an auth-callout subscriber on **`$SYS.REQ.USER.AUTH`** (OIDC primary, mTLS service-to-service, API keys transitional). See [A2A architecture](architecture.md) Phase 0 and [NSC account bootstrap](../how-to/operators/nsc-account-bootstrap.md) §Auth callout service — **reference only; do not implement callout in `a2a-gateway`.**
...
- [ ] Enforce **30s gateway deadline** on unary `message/send` request/reply; longer work must use `message/stream` (see [A2A architecture](architecture.md) §6).
...
| **AgentCard validation at gateway** | `a2a-nats-discovery` / KV catalog | Schema validation on registrar write and KV read defense-in-depth today. Gateway re-validation only if an **edge path materializes AgentCards outside NATS KV** ([A2A architecture](architecture.md) Phase 0). |
| **JetStream provisioning** | `a2a-nats` provision / discovery processes | Gateway does not provision streams or manage streaming back-pressure yet (Phase 2). See [Streaming back-pressure](../how-to/operators/streaming-backpressure.md). |
