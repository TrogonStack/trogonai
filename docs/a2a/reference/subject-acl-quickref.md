...
One-page summary of Phase 0 per-User NATS subject ACL inside a tenant Account. Full provisioning steps: [A2A NSC account bootstrap](../how-to/operators/nsc-account-bootstrap.md).

**Related:** [A2A plan](../explanation/architecture.md) · [A2A pending decisions](../explanation/architecture.md) · [A2A JetStream account streams](jetstream-account-streams.md)
...
| **Gateway User** | `a2a.agents.>`, `a2a.tasks.>`, `a2a.push.>` | `a2a.gateway.>` | Long-lived `a2a-gateway` service; queue-group consumer on ingress; forwards to agents, task events, push envelopes. Read-only catalog (KV get/watch or `{prefix}.discover.*`); no KV put. |
...
- **Optional ops read:** Registrar (or a dedicated ops User) may be granted JetStream **read** on stream **`A2A_EVENTS`** (`{prefix}.task.*.events.*`) and **`A2A_PUSH_DLQ`** (`{prefix}.push.dlq.*.*`) for troubleshooting. See [JetStream account streams](jetstream-account-streams.md) and [bootstrap § JetStream assets](../how-to/operators/nsc-account-bootstrap.md).
...
Details: [A2A auth callout design](../explanation/auth-callout-design.md).
