...
Vault key; on AWS KMS, key material generated by KMS itself (`AWS_KMS`
origin), which is created and held in KMS HSMs, never imported material, a
CloudHSM-backed custom key store, or an external key store, which carry a
different chain of trust. The key
lives in a
