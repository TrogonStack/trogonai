...
satisfy, not current behavior. ADR#0023 records that the write model below
...
bot tokens, and API credentials (ADR#0023 Decision 2). Cryptographic
...
path, and a caller cannot parse or construct one (ADR#0023 Decision 2).
...
OpenBao security boundaries (ADR#0023 Decision 5). Whether a vault is a
...
fingerprints, and reason enums, never values (ADR#0023 Decision 4). A
...
revocation) without carrying the credential value. ADR#0023 calls these
...
The write model has three parts, which ADR#0023 Decision 4 records as
...
failure (ADR#0023 Decision 4).

ADR#0023 attributes this shape to a prototype that already built it end to
...
by ADR#0023; the complete credential state machine is an open decision
...
instead (ADR#0023 Decision 4): the key-level `current_version` pointer
...
ADR#0023 does not enumerate a complete credential lifecycle state machine
...
already mints (ADR#0023 Decision 3). Tenant identity is always derived
...
never the value itself (ADR#0023 Decision 3).
...
in-flight operation, never as ambient state (ADR#0023 Decision 6).
...
that wrapper requires, is not fixed by ADR#0023 and is an open decision.
...
(ADR#0023 Decision 6). This extends to the service's own bootstrap: an
...
retries with backoff rather than serving in a degraded mode (ADR#0023
Decision 3).
...
categories is not fixed by ADR#0023, so this taxonomy is an open decision
rather than a restatement of an existing model.

ADR#0023 commits to dual audit: a business-context record (caller,
...
asked and why (ADR#0023 Decision 6). What the service does when an audit
sink itself fails to accept a record is not fixed; ADR#0023 lists correct
...
- Hold a standing, broad OpenBao token; it mints short-lived, narrowly
  scoped credentials per request instead (ADR#0023 Decision 3).
...
- **`SecretRef` identity scheme.** How the opaque handle is constructed
  and encoded is not fixed by ADR#0023.
- **Reason value typing.** Whether the reason field recorded on a state
  change becomes a closed set of typed values, as ADR#0023 Decision 4's
  wording suggests, or stays a bounded non-secret string is not settled.
- **Credential lifecycle state machine.** ADR#0023 fixes only the pending
...
- **Caller-visible resolve failure taxonomy.** Whether resolve failures
  ...
  eight, is not fixed.
- **Audit-sink failure behavior.** ADR#0023 names correct behavior when
  audit sinks fail as a required adoption proof, without answering what
  that behavior is (ADR#0023 Consequences).
- **Anomaly detection scope.** ADR#0023 states that anomaly detection on
