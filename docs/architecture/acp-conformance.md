...
| `agent-client-protocol-http` | 2.0.0 | staged (`server` feature) | Official axum-based Streamable HTTP + WebSocket transport, overlapping `acp-nats-server/src/transport.rs`. The dependency is in the graph and `acp-nats-server/src/component.rs` implements the `ConnectTo<Client>` factory `AcpHttpServer::new` requires, proven end-to-end by `acp-nats-server` test `upstream_http_transport_round_trips_initialize_through_the_bridge`: a real HTTP `initialize` crosses the SDK transport, the NATS bridge, and returns. Not yet serving traffic; `main` still wires the hand-rolled transport. The cutover is atomic under `-D warnings` (retiring `transport.rs` and `connection.rs` also retires the ~2,400 test lines bound to their internals) and owes four behaviors upstream does not provide: `Acp-Protocol-Version` validation against the negotiated version, a shutdown/drain hook (`ConnectionRegistry` is `pub(crate)`), `Origin` enforcement on POST/GET/DELETE rather than only the WebSocket upgrade, and `X-Accel-Buffering: no` on SSE. Two further deltas are behavior changes, not gaps: connection ids become UUIDv4 (`next_connection_id` is private and hardcoded), and `connectionId` disappears from the `initialize` result body, which is arguably more correct since that field is not in the v1 schema. |
