...
> give you that control ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Consequences).
...
1. In your Google Cloud project, create a CryptoKey with purpose
   **Symmetric encrypt/decrypt**. This is the only key shape the platform's
   ...
   are not supported by this capability ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Decision 2 and
   Non-Goals).
2. The platform binds the fully qualified CryptoKey resource name you
   ...
   CryptoKeyVersion ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Decision 4).
3. The adapter requires that the bound CryptoKey support authenticated data
   ...
   plaintext CRC32C ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Decisions 2, 5, and 7). You do not configure
...
2. Configure your workload identity pool and provider to bind the exact
   ...
   specifically ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Decision 5).
3. By default, grant that federated principal direct access to the exact
   ...
   either arrangement ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Decision 5).
...
1. Complete the registration started in Step 2. The platform records your
   ...
   ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Decision 3).
...
3. The platform validates access through the federated principal, or the
   ...
   opaque `KeyRef` ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Decision 4). A denied access attempt, an
...
4. The adapter retains the CryptoKeyVersion name that encryption returns as
   ...
   the adapter does not claim to verify a decrypt-time version ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md)
   Decision 4).
...
cannot be recalled, by Google Cloud or by the platform ([ADR#0030](../adr/0030-customer-controlled-key-backend-routing.md) Decision 6).
