...
Trogonai debe medir si cambiar de modelo realmente preserva continuidad.
...
Estos valores pueden ajustarse con datos reales, pero el producto debe tener targets explicitos.
...
Los formatos durables deben definirse como structs/enums Rust versionados con `serde`.
...
pub struct SessionEventV1 {
    ...
    pub actor: Actor,
    pub payload: SessionEventPayloadV1,
}
```

Reglas:

- no usar strings sueltos para estados criticos;
- usar enums Rust para switch/cancel/recovery/failure states;
- usar newtypes para IDs (`SessionId`, `EventId`, `OperationId`, etc.);
- generar JSON schema desde Rust si se necesita interoperabilidad externa;
- mantener golden fixtures por version.
...
```text
trogon-session-core        -> eventos, IDs, schemas, validation
trogon-session-kernel      -> session lease policy, append, materialization, recovery
trogon-session-projection  -> Context Twin, Prompt Compiler, token budgeting
trogon-artifacts           -> artifact metadata, refs, NatsObjectStore/claim-check integration
trogon-capabilities        -> model capabilities, probes, certification sobre trogon-registry
trogon-switching           -> switch state machine, safety gate, checkpoint
```

La regla importante no es el nombre de la crate, sino que los contratos durables no queden dispersos en runners. `trogon-transcript` puede inspirar patrones append-only, pero no debe absorber el Session Kernel tal cual: su modelo actual es audit trail de actores y no contiene `seq`, `idempotency_key`, snapshots, runner bindings, leases ni prompt projection.
...
NATS KV           -> ACP_SESSION_LEASES, backed by trogon-nats::lease
...
Los defaults deben vivir en config Rust y ser sobreescribibles por entorno.
...
Todo estado durable y todo contrato entre componentes debe ser:

```text
tipado en Rust
versionado
validado antes de persistir
serializable con serde
reconstruible desde NATS/JetStream/KV
testeado con fixtures y NATS integration tests
```
...
| --- | --- | --- | --- | --- | --- |
| 1 | Contratos Rust basicos y schemas | Crear tipos Rust versionados, newtypes de IDs y fixtures serde | `session_kernel_enabled=false` | unit/golden fixtures | structs versionados compilan y serializan |
...
1. **PR 1: Session schema crate/module**
   - Agregar IDs, `SessionEventV1`, `SessionSnapshotV1`, `ArtifactMetadataV1`, `ContextTwinV1`.
   - Agregar serde + fixtures.
...
| --- | --- | --- |
| Ubicacion de schemas Rust | Define ownership, imports y migraciones | Crear `trogon-session-core`/`trogon-session-kernel` o modulo equivalente; no poner el kernel dentro de `trogon-transcript` tal cual |
...
| --- | --- | --- | --- | --- |
| Definir crate/module canonico de schemas | Session Kernel/Core | Revision de crates actuales | Ruta Rust decidida, preferentemente `trogon-session-core`/`trogon-session-kernel`, ownership claro y fixtures iniciales | PR 1, migraciones, runners adapters |
