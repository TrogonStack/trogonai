...
- [~] Auth callout service — NATS subscriber on `$SYS.REQ.USER.AUTH`; OIDC primary, mTLS for service-to-service, API keys transitional. Mints Account-bound User JWT (`sub` external, `aud` = Account, SpiceDB principal in `data`). Landed in `rsworkspace/crates/a2a-auth-callout/`: OIDC JWKS verify, mTLS x509 chain verify, JWT mint. API-key path remains transitional stub.
