...
- **Phase 2** *(partial)* — `message/stream` and `tasks/resubscribe` over JetStream consumers. Task-lifecycle audit. Tier 2 CEL.
  ...
  - Pending: equivalent lifecycle coverage for unary `message/send` task evolution — product-scope decision (callers observe post-reply transitions today only by attaching via `message/stream` / `tasks/resubscribe` or to the task event subject).
