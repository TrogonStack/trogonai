...
- **No gateway auth callout / SpiceDB authz yet.** The auth-callout crate ships verifiers (OIDC, mTLS, HMAC API-key) and a JWT minter, but a deployed subscriber on `$SYS.REQ.USER.AUTH` is operator work. The gateway calls its Wasmtime substrate as a **Tier-2 predicate seam** (`NoopTier2Evaluator` until CEL→WASM lands) and emits decision-site `AuditEnvelope` JSON with `trace_id` + `rules_fired` populated when `A2A_GATEWAY_AUDIT_PUBLISH` is on; `rewrites` + `stream_consumer` audit fields and an authoritative JWT-derived `caller_id` are still future. SpiceDB Tier 1 + JWT verification at ingress, authoritative Tier-3 redaction at the request path, and bundle signing remain to be built.
...
- **Phase 4** *(partial)* — `a2a-bridge` HTTPS↔NATS runtime landed: axum HTTPS, auth-callout client, NATS publish/consume, SSE↔JetStream framing, outbound HTTPS path, trait-mocked tests (see **`docs/A2A_BRIDGE_SKETCH.md`**). Federated discovery import-gate trait + `AllowAllImportGate` default landed in `a2a-nats::catalog::import_gate` (SpiceDB impl stubbed pending Phase 1 SpiceDB integration; **[`docs/A2A_FEDERATED_DISCOVERY_SKETCH.md`](./docs/A2A_FEDERATED_DISCOVERY_SKETCH.md)**). Pending: production wiring against a deployed auth-callout, cross-binding collaboration tests against a live NATS+gateway, operator-signed Account export contract for `a2a.discover.>`.
...
- `a2a-gateway` *(partial)* — optional queue-group subscriber on **`{prefix}.gateway.>`**; forwards JSON-RPC payloads to **`{prefix}.agent.{agent_id}.{method}`** with the caller reply inbox, deadline-bounded for `message.send` (`A2A_GATEWAY_UNARY_DEADLINE_SECS`). Wasmtime policy substrate (`src/policy/`) invoked as a Tier-2 predicate seam when `A2A_GATEWAY_POLICY_BUNDLE_DIR` is set (Tier 2 `Tier2CelEvaluator` trait + `NoopTier2Evaluator`; Tier 3 host via `a2a_redaction::wasm::WasmRedactorHost` preloaded by `A2A_GATEWAY_POLICY_SKILLS`); decision-site `AuditEnvelope` JSON published on `{prefix}.audit.{ok,err}.{method}` when `A2A_GATEWAY_AUDIT_PUBLISH` is on (`trace_id` + `rules_fired` populated). Caller attribution rides NATS header `X-A2a-Caller-Id` (HTTPS `x-a2a-caller-id` mapped by `a2a-bridge`). Auth-callout client, SpiceDB Tier 1, authoritative Tier-2 CEL evaluator, Tier-3 redaction call site, and `rewrites` + `stream_consumer` audit fields remain to be built.
