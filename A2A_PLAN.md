...
- **Phase 3** *(partial)* — `tasks/pushNotificationConfig/*` with NATS-subject and JetStream-stream targets; HTTPS webhook target as a special case. Tier 3 WASM redaction over `parts[*]`.
  ...
  - Pending: optional gateway-side DLQ mirroring; richer Tier-3 skill catalog beyond manifest stubs (authoritative ingress call site **landed** — [`docs/A2A_TIER3_REDACTION.md`](./docs/A2A_TIER3_REDACTION.md)). Exactly-once opt-in **landed** (`DeliverySemantics`, `PushIdempotencyKey`, `IdempotencyKeyHeader`; sketch **[`docs/A2A_PUSH_EXACTLY_ONCE_SKETCH.md`](./docs/A2A_PUSH_EXACTLY_ONCE_SKETCH.md)**). Digest webhook auth deferred per §Decisions.
...
- `a2a-redaction` *(partial)* — Wasmtime engine + module loader + skill-id dispatch + refusal sentinel for Tier 3 part-level redaction; gateway authoritative call site wired via `RealTier3RedactionGate`.
