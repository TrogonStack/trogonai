...
- Audit: `Bridge::dispatch` emits one `AuditEnvelope` per handled RPC (`AuditEmitter`; default noop). **`message/stream` pump emits `TaskLifecycleEnvelope` payloads to `{prefix}.audit.lifecycle` when streamed task numeric state transitions** (`NatsAuditEmitter`). Gateway decision-site envelopes carry `trace_id`, `rules_fired`, `rewrites`, `refusal_skill`, `stream_consumer`, `zed_token_snapshot`, `tier1_decision`, and `tier3_decision` (see `a2a-gateway`).
...
- **HTTPS↔NATS interop bridge listeners ship in `a2a-bridge`** (axum HTTPS surface, auth-callout client, SSE↔JetStream framing, outbound HTTPS). Production wiring against a deployed auth-callout remains operator work. `a2a-nats-server` is a narrower HTTP/SSE surface over the embedded Rust `a2a_nats::Client`, not a foreign-client interop bridge.
...
All phases are proven end-to-end by `make smoke` / `make smoke-full`. Production fleet rollout is out of scope for this branch.

- **Phase 0** *(done)* — Account provisioning + auth callout + subject ACL + AgentCard KV bucket + unary surface. Unary `message/send`, `tasks/get`, and the rest of the unary methods ship; in-crate `catalog/` (`CatalogStore`, `KvCatalogStore`, `DiscoverService`, `CatalogRegistrarService`) backs `{prefix}.discover.*` reads and `{prefix}.catalog.register.*` writes; standalone `a2a-nats-discovery` binary provisions `A2A_AGENT_CARDS`. NSC bootstrap (`scripts/a2a-nsc-bootstrap.sh` + `scripts/acl-templates/{caller,gateway,registrar}.acl`) and the auth-callout subscriber (OIDC / mTLS / HMAC API-key + Account-bound JWT mint) run inside `compose.a2a.smoke.yml`; KV watch ergonomics ship via `A2aClient::watch_agent_card()`.
- **Phase 1** *(done)* — Tier 1 policies, SpiceDB authz, audit to JetStream, and discovery shaping. SpiceDB Tier-1 gate (`A2A_GATEWAY_TIER1_SPICEDB_ENABLED`), declarative Tier-1 (`A2A_GATEWAY_TIER1_DECLARATIVE_ENABLED`) with clock-aware time-of-day predicates, in-crate `audit/` emitter + envelope with `tier1_decision` / `tier3_decision` / `refusal_skill` / `stream_consumer` / `zed_token_snapshot`, and `BulkCheckPermission`-driven discovery shaping (`LiveAgentViewGate`) all exercised by `make smoke-full`.
- **Phase 2** *(done)* — `message/stream` + `tasks/resubscribe` over ephemeral pull consumers with `inactive_threshold` and replay at `last_seq + 1`; per-state-transition `TaskLifecycleEnvelope` on `{prefix}.audit.lifecycle`; env-gated Tier-2 CEL (`A2A_GATEWAY_TIER2_CEL_ENABLED`); env-gated gateway streaming ingress pipe (`A2A_GATEWAY_STREAMING_INGRESS=on`). Unary `message/send` task-lifecycle envelope coverage is out of scope per §Decisions — callers observe post-reply transitions via `message/stream` / `tasks/resubscribe`.
- **Phase 3** *(done)* — `tasks/pushNotificationConfig/*` CRUD, `CompositePushDispatcher` covering HTTP(S), core NATS (`subject:{…}`), and JetStream ack-publish (`jetstream:{…}`) with bounded retries / backoff (`constants::HTTP_PUSH_WEBHOOK_MAX_ATTEMPTS`); `pushNotificationAuthenticationInfo` mapping (Bearer / Basic / jwt → Bearer); `A2A_PUSH_DLQ` provisioning + `push::dlq` JSON envelopes; principal-derived DLQ `caller_id` via `PrincipalCarrier` + `CallerId::from_principal`; Tier-3 WASM redaction (`A2A_GATEWAY_TIER3_REDACTION_ENABLED`) with signed-bundle preload (`A2A_GATEWAY_TIER3_SIGNING_PUBKEY`) + `a2a-sign-bundle` operator CLI. Digest webhook auth is out of scope per §Decisions.
- **Phase 4** *(done)* — `a2a-bridge` HTTPS↔NATS runtime (axum + auth-callout client + SSE↔JetStream framing) with `A2A_BRIDGE_TRANSPORT=nats` wired in smoke; federated discovery via `SpiceDbImportGate` (`CheckBulkPermissions` + ZedToken cache) and operator-signed `SignedExportEnvelope` (`RealOperatorSignatureGate` keyed by `A2A_DISCOVERY_OPERATOR_KEYS`); cross-binding collaboration validated by the `a2a-smoke-test` `full` profile against live NATS + gateway + bridge + auth-callout + spicedb in `compose.a2a.full.yml`.
