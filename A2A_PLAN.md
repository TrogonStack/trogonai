...
- `rsworkspace/crates/a2a-gateway` — NATS-connected process with optional queue group on **`{prefix}.gateway.>`**; **opaque request/reply forward** from gateway-shaped subjects to **`{prefix}.agent.{agent_id}.{method}`** (`a2a_nats::gateway_ingress`). Wasmtime policy substrate scaffolded under `src/policy/` (Tier 2 `Tier2CelEvaluator` trait + `NoopTier2Evaluator`; Tier 3 re-uses `a2a_redaction::wasm::WasmRedactorHost`); not yet called from the request path. Auth callout / SpiceDB / decision-site audit hooks remain future (see README). The **`planned::`** submodule collects **compile-only** roadmap seams (back-pressure egress planner traits, Unary deadline metadata hooks, NSC ACL snippets, Wasmtime pool traits, SpiceDB seam traits, auth-callout consts, HTTPS bridge structs).
...
- **No gateway auth callout / policy engine / SpiceDB authz yet.** The auth-callout crate ships verifiers (OIDC, mTLS, HMAC API-key) and a JWT minter, but a deployed subscriber on `$SYS.REQ.USER.AUTH` is operator work. The gateway has a Wasmtime policy substrate scaffold (`a2a-gateway/src/policy/`) and a Tier 2 `Tier2CelEvaluator` trait, but the request path forwards opaquely and does **not** call the substrate. `a2a-gateway` only routes **`{prefix}.gateway.…`** → **`{prefix}.agent.…`** and does **not** validate JWTs or emit gateway decision audits. Gateway decision sites (`rules_fired`, rewrites), full §Audit attribution (`trace_id`, …), and bundle signing remain to be built.
...
- `a2a-gateway` *(partial ingress relay)* — optional queue-group subscriber on **`{prefix}.gateway.>`**; forwards JSON-RPC payloads to **`{prefix}.agent.{agent_id}.{method}`** with the caller reply inbox. Wasmtime policy substrate scaffold (`src/policy/`) landed (Tier 2 `Tier2CelEvaluator` trait + `NoopTier2Evaluator`; Tier 3 host via `a2a_redaction::wasm::WasmRedactorHost`); not yet wired into the request path. Auth callout client, SpiceDB Tier 1, ingress audit hooks remain to be built.
