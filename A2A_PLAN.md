...
- `rsworkspace/crates/a2a-gateway` — NATS-connected process with optional queue group on **`{prefix}.gateway.>`**; request/reply forward from gateway-shaped subjects to **`{prefix}.agent.{agent_id}.{method}`** (`a2a_nats::gateway_ingress`). Wasmtime policy substrate lives under `src/policy/` (Tier 2 CEL evaluator env-gated via `A2A_GATEWAY_TIER2_CEL_ENABLED`; Tier 3 re-uses `a2a_redaction::wasm::WasmRedactorHost`) and is invoked from the request path when `A2A_GATEWAY_POLICY_BUNDLE_DIR` is set (`A2A_GATEWAY_POLICY_SKILLS` preloads named WASM bundles). Unary `message.send` enforces a deadline (`A2A_GATEWAY_UNARY_DEADLINE_SECS`, default `DEFAULT_OPERATION_TIMEOUT`) returning JSON-RPC `-32800` on overrun; policy denial returns `-32801`. Decision-site `AuditEnvelope` JSON is published when `A2A_GATEWAY_AUDIT_PUBLISH` is truthy (`trace_id`, `rules_fired`, `rewrites`, and `stream_consumer` populated). Caller attribution rides `X-A2a-Caller-Id` on ingress. Env-gated durable JetStream pull egress on **`A2A_EVENTS`** when **`A2A_GATEWAY_EVENTS_PULL=on`** (`gw_pull_backpressure`). Env-gated push DLQ mirror on **`{prefix}.push.dlq.>`** when **`A2A_GATEWAY_PUSH_DLQ_MIRROR=on`**. Auth-callout / SpiceDB remain future. The **`planned::`** submodule collects **compile-only** roadmap seams for remaining Phase 2+ items.
...
- **No gateway auth callout / SpiceDB authz yet.** The auth-callout crate ships verifiers (OIDC, mTLS, HMAC API-key) and a JWT minter, but a deployed subscriber on `$SYS.REQ.USER.AUTH` is operator work. The gateway calls its Wasmtime substrate as a **Tier-2 CEL evaluator** (env-gated via `A2A_GATEWAY_TIER2_CEL_ENABLED`; noop when off) and emits decision-site `AuditEnvelope` JSON with `trace_id`, `rules_fired`, `rewrites`, and `stream_consumer` populated when `A2A_GATEWAY_AUDIT_PUBLISH` is on; an authoritative JWT-derived `caller_id` is still future. SpiceDB Tier 1 + JWT verification at ingress, authoritative Tier-3 redaction at the request path, and bundle signing remain to be built.
...
- `a2a-gateway` *(partial)* — optional queue-group subscriber on **`{prefix}.gateway.>`**; forwards JSON-RPC payloads to **`{prefix}.agent.{agent_id}.{method}`** with the caller reply inbox, deadline-bounded for `message.send` (`A2A_GATEWAY_UNARY_DEADLINE_SECS`). Wasmtime policy substrate (`src/policy/`) invoked when `A2A_GATEWAY_POLICY_BUNDLE_DIR` is set (Tier 2 CEL evaluator env-gated via `A2A_GATEWAY_TIER2_CEL_ENABLED`; Tier 3 host via `a2a_redaction::wasm::WasmRedactorHost` preloaded by `A2A_GATEWAY_POLICY_SKILLS`); decision-site `AuditEnvelope` JSON published on `{prefix}.audit.{ok,err}.{method}` when `A2A_GATEWAY_AUDIT_PUBLISH` is on (`trace_id`, `rules_fired`, `rewrites`, and `stream_consumer` populated). Caller attribution rides NATS header `X-A2a-Caller-Id` (HTTPS `x-a2a-caller-id` mapped by `a2a-bridge`). Auth-callout client, SpiceDB Tier 1, and Tier-3 redaction call site remain to be built.
