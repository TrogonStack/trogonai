...
//! schema-driven redaction (ADR 0027); scaffold types in
...
mod hash_redaction {
    ...
    #[tokio::test]
    #[ignore = "scaffold; implement when schema-driven redaction lands"]
...
mod schema_cache_gate {
    ...
    async fn schema_cache_miss_returns_schema_unknown_error() {
        ...
        // Assert: JSON-RPC error code `-32104`, message `schema_unknown` (per reference-error-codes.md).
...
mod audit_rewrites_envelope {
    ...
    async fn audit_rewrites_use_jsonpath_and_op_wire_format() {
        ...
        unimplemented!("rewrites wire format matches reference-audit-envelope.md");
    }
}
