...
use trogon_mcp_gateway::spicedb::{CacheKeyParams, ResourceKey, ZedTokenCache, ZedTokenCacheConfig};
...
mod cold_path {
    ...
    async fn first_tools_list_stores_zedtoken_for_session_principal_permission() {
        ...
        cache.insert_zed_token(&params, "zed-from-bulk".into(), None).await;

        assert_eq!(cache.get_zed_token(&params).await.as_deref(), Some("zed-from-bulk"));
        assert_eq!(
...
mod cache_hit_within_ttl {
    ...
    async fn second_tools_list_within_ttl_uses_cached_zedtoken_for_consistency() {
        ...
        cache.insert_zed_token(&params, "tok-warm".into(), None).await;
        cache
            .insert_results(&params, "tok-warm", &[(resource.clone(), true)], None)
            .await;

        assert_eq!(cache.get_zed_token(&params).await.as_deref(), Some("tok-warm"));
        assert_eq!(cache.get_result(&params, &resource, Some("tok-warm")).await, Some(true));
    }
    ...
    async fn repeat_tools_list_returns_same_filtered_catalog_when_graph_unchanged() {
        ...
        assert_eq!(cache.get_result(&params, &allowed, Some("tok-warm")).await, Some(true));
        assert_eq!(cache.get_result(&params, &denied, Some("tok-warm")).await, Some(false));
    }
...
mod list_changed_invalidation {
    ...
    async fn list_changed_notification_drops_session_zedtoken_entries() {
        ...
        cache.insert_zed_token(&params, "tok-inv".into(), None).await;
        cache
        ...
        assert!(cache.get_result(&params, &resource, Some("tok-inv")).await.is_none());
    }
...
mod principal_isolation {
    ...
    async fn different_jwt_sub_does_not_hit_other_principals_permission_results() {
        ...
        assert_eq!(cache.get_result(&params_a, &resource, Some("tok-a")).await, Some(true));
        assert!(cache.get_result(&params_b, &resource, Some("tok-a")).await.is_none());
    }
    ...
    async fn same_session_id_different_principal_gets_independent_cache_namespace() {
        ...
        assert_ne!(ZedTokenCache::zed_key(&params_a), ZedTokenCache::zed_key(&params_b));
    }
}
