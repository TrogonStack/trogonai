...
use config::LlmOpenAiConfig;
use futures::StreamExt;
use llm_types::{llm_subject_for_account, LlmPromptRequest, LLM_REQUEST_OPENAI};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let config = LlmOpenAiConfig::from_env(&SystemEnv);
    let account_id = config.account_id.as_deref();

    tracing::info!(
        model = %config.default_model,
        max_tokens = config.default_max_tokens,
        "Starting llm-openai worker"
    );
    ...
    let subject = llm_subject_for_account(LLM_REQUEST_OPENAI, account_id);

    let mut sub = nats_client
        .queue_subscribe(subject.clone(), "llm-openai-workers".to_string())
        .await?;
