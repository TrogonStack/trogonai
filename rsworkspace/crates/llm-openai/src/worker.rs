...
/// Handle one `LlmPromptRequest`: call the OpenAI API, stream chunks to
            ...
            Ok(resp) => {
                ...
                if retryable && attempt + 1 < retry_attempts.max(1) {
                    tracing::warn!(
                        ...
                        %status,
                        retry_in = ?delay,
                        "OpenAI API retryable error, retrying"
                    );
                    tokio::time::sleep(delay).await;
                    delay *= 2;
                } else {
                    publish_error(nats, &inbox, &format!("OpenAI API {status}: {body}")).await;
                    return;
                }
            }
            Err(e) => {
...
/// Convert an `LlmMessage` content value (Anthropic format) to OpenAI content format.
///
/// - String content passes through unchanged.
/// - Array content blocks are converted: image blocks become `image_url` blocks.
