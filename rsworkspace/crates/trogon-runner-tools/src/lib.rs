...
pub const COMPLETION_GUIDANCE: &str = "When a turn involved running tools or changing files, end it with a brief, plain-language summary of what you did and the result — what changed, whether it worked, and any next step the user should know. Never end a turn silently right after a tool call: the user sees only what you tell them, not the raw tool output. But when you did NOT run tools or make changes — a greeting, a question you answered directly, ordinary conversation — just respond naturally; do not tack on a 'what I did' recap or a bullet summary. Keep any summary concise: a sentence or a few short bullets, never a long report.";
