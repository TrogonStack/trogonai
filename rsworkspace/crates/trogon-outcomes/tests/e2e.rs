...
use trogon_outcomes::{
    AnthropicEvaluationProvider, Criterion, EvalAuthStyle, EvalLlmConfig, EvaluationService, Evaluator, RalphLoop,
    ResultClient, Rubric, RubricClient, SequencedTaskExecutor, SubAgentEvaluationProvider, provision_results_kv,
    provision_rubrics_kv, provision_stream, trigger_evaluation,
};
...
async fn evaluation_service_stores_result_for_matching_rubric() {
    ...
    let rubric = Rubric::new(
        ...
        vec![Criterion {
            name: "clarity".into(),
            description: "Is feedback clear?".into(),
            weight: 1.0,
        }],
    );
    ...
    let provider = AnthropicEvaluationProvider::with_client(llm_config(llm_url), reqwest::Client::new());
    ...
    // Trigger evaluation
    trigger_evaluation(&js, "pr", "owner/repo/7", &session_id, &[])
        .await
        .unwrap();
    ...
    let results = ResultClient::new(result_kv)
...
async fn evaluation_skips_rubric_with_non_matching_actor_type() {
    ...
    let llm_url =
        spawn_mock_llm(r#"{"scores":[{"criterion":"q","score":0.8,"reasoning":"ok"}],"overall_reasoning":"fine"}"#)
            .await;
    ...
    trigger_evaluation(&js, "pr", "owner/repo/8", &session_id, &[])
        .await
        .unwrap();
...
async fn malformed_trigger_is_skipped_and_service_continues() {
    ...
    let rubric = Rubric::new(
        ...
        vec![Criterion {
            name: "alive".into(),
            description: "Is service alive?".into(),
            weight: 1.0,
        }],
    );
    RubricClient::new(rubric_kv.clone()).put(&rubric).await.unwrap();
    ...
    js.publish(
        "sessions.evaluate.pr.owner_repo_9.bad-sess",
    ...
    let session_id = session.id().to_string();
    trigger_evaluation(&js, "pr", "owner/repo/9", &session_id, &[])
        .await
        .unwrap();
    ...
    assert_eq!(
        results.len(),
        1,
        "service should have processed the valid trigger after the bad one"
    );
...
async fn ralph_loop_retries_until_rubric_passes() {
    ...
    let result = ralph
        .run("write a summary", "pr", "owner/repo/7", Some("ralph-rubric"))
        .await
        .unwrap();
    ...
    let results_0 = ResultClient::new(result_kv.clone())
        .list_for_session(session_0)
        .await
        .unwrap();
