...
async fn oneshot_call_with_client<C: SpawnHttpClient>(
    ...
    client.complete(api_key, model, prompt, url, site_url, site_name).await
}
...
mod tests {
    ...
    struct MockSpawnClient {
        response: String,
        captured: Arc<Mutex<Option<(String, String, String, String, String, String)>>>,
    }
    ...
    async fn returns_assistant_content_on_success() {
        let (mock, _) = MockSpawnClient::responding("hello from openrouter");
        assert_eq!(call(&mock).await, "hello from openrouter");
    }
    ...
    fn extract_prompt_accepts_non_empty_prompt() {
        let req = serde_json::json!({"prompt": "summarise this file"});
        assert_eq!(extract_prompt(&req).unwrap(), "summarise this file");
    }
}
