...
//! mock HTTP client so no real OpenRouter API key is needed.
//!
//! Run with:
//!   cargo test -p trogon-openrouter-runner --test spawn_integration
...
use trogon_openrouter_runner::spawn_handler::{SpawnHttpClient, run_spawn_subscriber};
...
fn subscriber(nats: async_nats::Client, client: Arc<MockSpawnClient>) -> tokio::task::JoinHandle<()> {
