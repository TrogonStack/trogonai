...
//!
//! Run with:
//!   cargo test -p trogon-openrouter-runner --test kv_integration
...
async fn agent_loader_parses_full_config() {
    ...
    kv_put(
        ...
        "CONSOLE_AGENTS",
        "agent-1",
        r#"{"skill_ids":["sk1","sk2"],"system_prompt":"Be concise.","model":{"id":"anthropic/claude-sonnet-4-6"}}"#,
    )
    .await;
    ...
    assert_eq!(cfg.model_id.as_deref(), Some("anthropic/claude-sonnet-4-6"));
}
...
use trogon_openrouter_runner::session_store::{
