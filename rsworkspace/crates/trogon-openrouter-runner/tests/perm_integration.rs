...
async fn bypass_permissions_meta_skips_deny_rule_from_trogon_md() {
    ...
    local().run_until(async move {
        ...
        assert_eq!(audit[0].outcome, AuditOutcome::Allowed, "bypass mode must record Allowed in audit");
    }).await;
}
...
async fn set_session_mode_bypass_overrides_deny_rule() {
    ...
    local().run_until(async move {
        ...
        let audit = agent.test_session_audit_log(&sid).await;
        assert_eq!(audit.len(), 1);
        assert_eq!(audit[0].outcome, AuditOutcome::Allowed, "audit must record Allowed after set_session_mode bypass");
    }).await;
}
...
async fn mixed_allowed_and_denied_tools_in_same_prompt() {
    ...
    local().run_until(async move {
        ...
        let denied_msg = second.messages.iter().find(|m| {
            m.tool_call_id.as_deref() == Some("cid-bad")
        });
