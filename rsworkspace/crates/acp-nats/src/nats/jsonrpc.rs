...
/// Like [`jsonrpc_request_with_timeout`] but never times out — for requests that
/// block on human input (e.g. permission prompts), where any fixed timeout would
/// wrongly deny the user while they are still deciding.
pub async fn jsonrpc_request_no_timeout<N, Req, Res>(
    client: &N,
    subject: &impl Requestable,
    method: &str,
    request: &Req,
...
where
    ...
    N::RequestError: 'static,
{
    jsonrpc_request_with_timeout(client, subject, method, request, Duration::MAX).await
}
