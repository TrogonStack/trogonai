...
#[tokio::test]
async fn elicitation_create_returns_session_mismatch_when_session_id_differs() {
    let client = MockClient::new(ElicitationAcceptAction::new());
    ...
    match result.unwrap_err() {
        ElicitationCreateError::SessionMismatch { params, subject } => {
            assert_eq!(params, "session-other");
            assert_eq!(subject, "session-001");
        }
        other => panic!("expected SessionMismatch, got {other:?}"),
    }
}

fn sample_wire_error() -> crate::wire::WireError {
    crate::wire::decode_request_params::<CreateElicitationRequest>("elicitation/create", &empty_headers(), b"not json")
        .unwrap_err()
}

#[test]
fn error_code_and_message_invalid_request_returns_invalid_params() {
    let ec_err = ElicitationCreateError::InvalidRequest(sample_wire_error());
    let (code, _) = error_code_and_message(&ec_err);
    assert_eq!(code, ErrorCode::InvalidParams);
}

#[test]
fn error_code_and_message_session_mismatch_returns_invalid_params() {
    let ec_err = ElicitationCreateError::SessionMismatch {
        params: "session-other".to_string(),
        subject: "session-001".to_string(),
    };
    let (code, message) = error_code_and_message(&ec_err);
    assert_eq!(code, ErrorCode::InvalidParams);
    assert_eq!(
        message,
        "scope.sessionId (session-other) does not match subject session id (session-001)"
    );
}
