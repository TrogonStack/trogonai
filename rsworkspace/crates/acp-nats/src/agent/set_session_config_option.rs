...
fn map_set_session_config_option_error(e: NatsError) -> Error {
    match &e {
        NatsError::Timeout { subject } => {
            warn!(subject = %subject, "set_session_config_option request timed out");
            Error::new(
                ErrorCode::Other(AGENT_UNAVAILABLE).into(),
                "Set session config option request timed out; agent may be overloaded or unavailable",
            )
        }
        NatsError::Request { subject, error } => {
            warn!(subject = %subject, error = %error, "set_session_config_option NATS request failed");
            Error::new(
                ErrorCode::Other(AGENT_UNAVAILABLE).into(),
        ...
        NatsError::Serialize(inner) => {
            warn!(error = %inner, "failed to serialize set_session_config_option request");
            Error::new(
                ErrorCode::InternalError.into(),
                format!(
                    "Failed to serialize set_session_config_option request: {}",
                    inner
                ),
            )
        }
        NatsError::Deserialize(inner) => {
            warn!(error = %inner, "failed to deserialize set_session_config_option response");
            Error::new(
                ErrorCode::InternalError.into(),
        ...
        _ => {
            warn!(error = %e, "set_session_config_option NATS request failed");
            Error::new(
                ErrorCode::InternalError.into(),
                "Set session config option request failed",
            )
        }
    }
}
...
#[instrument(
    ...
    skip(bridge, args),
    fields(session_id = %args.session_id, config_id = %args.config_id, value = %args.value)
)]
...
mod tests {
    ...
    async fn set_session_config_option_forwards_request_and_returns_response() {
        ...
        set_json_response(
            &mock,
            "acp.session-cfg-001.agent.session.set_config_option",
            &expected,
        );
    ...
    async fn set_session_config_option_validates_session_id() {
        ...
        let request = SetSessionConfigOptionRequest::new("invalid.session.id", "cfg", "val");
        let err = bridge.set_session_config_option(request).await.unwrap_err();
    ...
    fn map_error_timeout() {
        let err = map_set_session_config_option_error(NatsError::Timeout {
            subject: "acp.s1.agent.session.set_config_option".into(),
        });
    ...
    fn map_error_request() {
        let err = map_set_session_config_option_error(NatsError::Request {
            subject: "acp.s1.agent.session.set_config_option".into(),
    ...
    fn map_error_serialize() {
        ...
        let err = map_set_session_config_option_error(NatsError::Serialize(serde_err));
