use crate::AgentHandler;
use crate::agent::test_support::{has_request_metric, mock_bridge, mock_bridge_with_metrics, set_json_response};
use crate::error::AGENT_UNAVAILABLE;
use agent_client_protocol::ErrorCode;
use agent_client_protocol::schema::v1::{DisableProviderRequest, DisableProviderResponse};

fn request() -> DisableProviderRequest {
    DisableProviderRequest::new("anthropic")
}

#[tokio::test]
async fn disable_provider_forwards_request_and_returns_response() {
    let (mock, _js, bridge) = mock_bridge();
    let expected = DisableProviderResponse::new();
    set_json_response(&mock, "acp.agent.providers.disable", &expected);

    let result = bridge.disable_provider(request()).await;

    assert!(result.is_ok());
}

#[tokio::test]
async fn disable_provider_returns_error_when_nats_fails() {
    let (mock, _js, bridge) = mock_bridge();
    mock.fail_next_request();

    let err = bridge.disable_provider(request()).await.unwrap_err();

    assert_eq!(err.code, ErrorCode::Other(AGENT_UNAVAILABLE));
}

#[tokio::test]
async fn disable_provider_returns_error_when_response_is_invalid_json() {
    let (mock, _js, bridge) = mock_bridge();
    mock.set_response("acp.agent.providers.disable", "not json".into());

    let err = bridge.disable_provider(request()).await.unwrap_err();

    assert_eq!(err.code, ErrorCode::InternalError);
}

#[tokio::test]
async fn disable_provider_records_metrics_on_success() {
    let (mock, _js, bridge, exporter, provider) = mock_bridge_with_metrics();
    set_json_response(&mock, "acp.agent.providers.disable", &DisableProviderResponse::new());

    let _ = bridge.disable_provider(request()).await;

    provider.force_flush().unwrap();
    let finished_metrics = exporter.get_finished_metrics().unwrap();
    assert!(
        has_request_metric(&finished_metrics, "disable_provider", true),
        "expected acp.requests with method=disable_provider, success=true"
    );
    provider.shutdown().unwrap();
}

#[tokio::test]
async fn disable_provider_records_metrics_on_failure() {
    let (mock, _js, bridge, exporter, provider) = mock_bridge_with_metrics();
    mock.fail_next_request();

    let _ = bridge.disable_provider(request()).await;

    provider.force_flush().unwrap();
    let finished_metrics = exporter.get_finished_metrics().unwrap();
    assert!(
        has_request_metric(&finished_metrics, "disable_provider", false),
        "expected acp.requests with method=disable_provider, success=false"
    );
    provider.shutdown().unwrap();
}
