...
impl<N: RequestClient + PublishClient + FlushClient> NatsClientProxy<N> {
    ...
    async fn request_no_timeout<Req: serde::Serialize, Resp: serde::de::DeserializeOwned>(
        ...
        crate::nats::jsonrpc::jsonrpc_request_no_timeout(&self.nats, subject, method, args)
            .await
            .map_err(|e| match e {
                crate::nats::jsonrpc::JsonRpcRequestError::Agent(err) => err,
                other => to_acp_error(other),
...
impl<N: RequestClient + PublishClient + FlushClient> NatsClientProxy<N> {
    ...
    pub async fn request_elicitation(&self, args: ElicitationRequest) -> Result<ElicitationResponse> {
        let s = client_ops::SessionElicitationSubject::new(self.prefix(), self.session_id());
        self.request(&s, "session/elicitation", &args).await
    }
}
...
impl<N: RequestClient + PublishClient + FlushClient> Client for NatsClientProxy<N> {
    async fn request_permission(&self, args: RequestPermissionRequest) -> Result<RequestPermissionResponse> {
        ...
        self.request_no_timeout(&s, "session/request_permission", &args).await
    }
