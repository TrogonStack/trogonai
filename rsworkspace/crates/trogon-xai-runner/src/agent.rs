...
use trogon_runner_tools::{ElicitationTx, StoredMcpServer, build_session_mcp, convert_mcp_servers, elicit_via_channel};
...
impl<H: XaiHttpClient, N: SessionNotifier, M: TrogonMdLoading> XaiAgent<H, N, M> {
    ...
    pub fn with_default_session_store(mut self, selection: crate::session_store::DefaultSessionStore) -> Self {
        match selection {
    ...
    async fn prompt(&self, req: PromptRequest) -> agent_client_protocol::Result<PromptResponse> {
        ...
        let stop_reason = 'outer: loop {
                    ...
                    let result = if !allowed {
                        ...
                        match name.as_str() {
                            ...
                            "spawn_agent" => {
                                ...
                                let subagent = input["agent"].as_str().filter(|s| !s.is_empty()).and_then(|n| {
                            ...
                            _ => {
                                    ...
                                    mcp_dispatch.iter().find(|(prefixed, _, _)| prefixed == &name)
                                {
                                    ...
                                    if !trogon_runner_tools::egress::EgressPolicy::default_safe().is_allowed(url) {
                                        ...
                                        trogon_tools::dispatch_tool(&ctx, &name, &tool_input)
                                            .await
                                            .display_text()
                                    }
                                } else {
                ...
                && let Some(resp_id) = current_response_id.clone()
            {
                ...
                current_prev_response_id = Some(resp_id);
                current_input = vec![InputItem::user(trogon_runner_tools::AUTO_SUMMARY_NUDGE.to_string())];
                current_response_id = None;
                continuation_in_progress = true;
...
mod tests {
    ...
    async fn set_session_config_option_unknown_option_returns_full_config_options() {
        ...
        assert!(
            resp.config_options
                .iter()
                .any(|o| o.id.to_string() == "compactor_model"),
    ...
    async fn set_session_config_option_compactor_model_is_stored_and_cleared() {
        ...
        agent
            .set_session_config_option(SetSessionConfigOptionRequest::new("cfg-cm", "compactor_model", ""))
            .await
            .unwrap();
        assert_eq!(
            agent.test_session_compactor_model("cfg-cm").await,
    ...
    fn trim_history_orphaned_user_removed_individually() {
        let mut h = vec![msg("user", "orphan"), msg("user", "new"), msg("assistant", "reply")];
        trim_history(&mut h, 2);
        assert_eq!(h.len(), 2);
