...
use config::LlmAnthropicConfig;
use futures::StreamExt;
use llm_types::{LlmPromptRequest, llm_subject_for_account, LLM_REQUEST_ANTHROPIC};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let config = LlmAnthropicConfig::from_env(&SystemEnv);
    let account_id = config.account_id.as_deref();

    tracing::info!(
        model = %config.default_model,
        max_tokens = config.default_max_tokens,
        "Starting llm-anthropic worker"
    );
    ...
    let subject = llm_subject_for_account(LLM_REQUEST_ANTHROPIC, account_id);

    // Queue-group subscription: multiple workers share the load without
    // duplicating responses.
    let mut sub = nats_client
        .queue_subscribe(subject.clone(), "llm-anthropic-workers".to_string())
        .await?;
