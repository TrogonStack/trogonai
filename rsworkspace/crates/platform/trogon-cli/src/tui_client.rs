...
use agent_client_protocol::schema::v1::{
    ...
    CreateTerminalResponse, ElicitationAcceptAction, ElicitationAction, ElicitationContentValue, ElicitationMode,
    ElicitationPropertySchema, ElicitationSchema, KillTerminalRequest, KillTerminalResponse, PermissionOption,
    ...
    SessionNotification, StringPropertySchema, TerminalOutputRequest, TerminalOutputResponse,
    ...
    async fn elicitation_create(
        ...
        match &req.mode {
            ElicitationMode::Form(form) => {
                handle_form_elicitation(&req.message, &form.requested_schema, self.coordinator.clone()).await
            }
            ElicitationMode::Url(url_mode) => {
                handle_url_elicitation(&req.message, &url_mode.url, self.coordinator.clone()).await
            }
            // `ElicitationMode::Other` and any future non-exhaustive variant: decline.
            _ => Ok(CreateElicitationResponse::new(ElicitationAction::Decline)),
        }
    }

    async fn elicitation_complete(
        &self,
        _args: CompleteElicitationNotification,
    ) -> agent_client_protocol::Result<()> {
        Ok(())
    }
}

/// Read one line for an elicitation field, off the async runtime thread.
///
/// Returns `None` on a tty I/O error or a `spawn_blocking` panic — the caller
/// treats either as a hard failure and cancels the whole elicitation.
async fn read_field_line(prompt: String) -> Option<String> {
    match tokio::task::spawn_blocking(move || read_line_from_dev_tty(&prompt)).await {
        Ok(Ok(line)) => Some(line),
        Ok(Err(e)) => {
            tracing::warn!(error = %e, "elicitation tty read failed");
            None
        }
        Err(e) => {
            tracing::warn!(error = %e, "elicitation spawn_blocking panicked");
            None
        }
    }
}

/// Returns `Some(title)` if the string property is a single-select enum,
/// pairing each option's wire value with its display title.
fn string_options(s: &StringPropertySchema) -> Option<Vec<(String, String)>> {
    if let Some(one_of) = &s.one_of {
        return Some(one_of.iter().map(|o| (o.value.clone(), o.title.clone())).collect());
    }
    if let Some(values) = &s.enum_values {
        return Some(values.iter().map(|v| (v.clone(), v.clone())).collect());
    }
    None
}

fn property_title(prop: &ElicitationPropertySchema) -> Option<&str> {
    match prop {
        ElicitationPropertySchema::String(s) => s.title.as_deref(),
        ElicitationPropertySchema::Number(s) => s.title.as_deref(),
        ElicitationPropertySchema::Integer(s) => s.title.as_deref(),
        ElicitationPropertySchema::Boolean(s) => s.title.as_deref(),
        ElicitationPropertySchema::Array(s) => s.title.as_deref(),
        _ => None,
    }
}

fn property_description(prop: &ElicitationPropertySchema) -> Option<&str> {
    match prop {
        ElicitationPropertySchema::String(s) => s.description.as_deref(),
        ElicitationPropertySchema::Number(s) => s.description.as_deref(),
        ElicitationPropertySchema::Integer(s) => s.description.as_deref(),
        ElicitationPropertySchema::Boolean(s) => s.description.as_deref(),
        ElicitationPropertySchema::Array(s) => s.description.as_deref(),
        _ => None,
    }
}

/// Build a display label for a form field: title (fallback to the field name),
/// its description in parens, and a `(required)` suffix.
fn property_label(name: &str, prop: &ElicitationPropertySchema, required: bool) -> String {
    let mut label = property_title(prop).unwrap_or(name).to_string();
    if let Some(description) = property_description(prop) {
        label.push_str(" (");
...
/// Prompt for one elicitation form field.
///
/// Returns `Ok(None)` when the field is declined/omitted (blank input with no
/// default, or an unsupported `Other` schema) and `Err(())` on a hard tty
/// failure — the caller cancels the whole elicitation in that case.
async fn prompt_for_property(
    name: &str,
    prop: &ElicitationPropertySchema,
    required: bool,
) -> Result<Option<ElicitationContentValue>, ()> {
    let label = property_label(name, prop, required);
    match prop {
        ElicitationPropertySchema::String(s) => {
            if let Some(options) = string_options(s) {
                eprintln!("{label}:");
                for (i, (_, title)) in options.iter().enumerate() {
                    eprintln!("  {}. {title}", i + 1);
                }
                flush_stderr();
                let Some(line) = read_field_line("Choice: ".to_string()).await else {
                    return Err(());
                };
                ...
                {
                    return Ok(Some(ElicitationContentValue::String(value)));
                }
                return Ok(s.default.clone().map(ElicitationContentValue::String));
            }
            let Some(line) = read_field_line(format!("{label}: ")).await else {
                return Err(());
            };
            let trimmed = line.trim();
            if trimmed.is_empty() {
                Ok(s.default.clone().map(ElicitationContentValue::String))
            } else {
                Ok(Some(ElicitationContentValue::String(trimmed.to_string())))
            }
        }
        ElicitationPropertySchema::Number(s) => {
            let Some(line) = read_field_line(format!("{label}: ")).await else {
                return Err(());
            };
            let trimmed = line.trim();
            if trimmed.is_empty() {
                return Ok(s.default.map(ElicitationContentValue::Number));
            }
            Ok(trimmed
                .parse::<f64>()
                .ok()
                .map(ElicitationContentValue::Number)
                .or_else(|| s.default.map(ElicitationContentValue::Number)))
        }
        ElicitationPropertySchema::Integer(s) => {
            let Some(line) = read_field_line(format!("{label}: ")).await else {
                return Err(());
            };
            let trimmed = line.trim();
            if trimmed.is_empty() {
                return Ok(s.default.map(ElicitationContentValue::Integer));
            }
            Ok(trimmed
                .parse::<i64>()
                .ok()
                .map(ElicitationContentValue::Integer)
                .or_else(|| s.default.map(ElicitationContentValue::Integer)))
        }
        ElicitationPropertySchema::Boolean(s) => {
            let Some(line) = read_field_line(format!("{label} (y/n): ")).await else {
                return Err(());
            };
            match line.trim().to_lowercase().as_str() {
                "y" | "yes" | "true" => Ok(Some(ElicitationContentValue::Boolean(true))),
                "n" | "no" | "false" => Ok(Some(ElicitationContentValue::Boolean(false))),
                _ => Ok(s.default.map(ElicitationContentValue::Boolean)),
            }
        }
        ElicitationPropertySchema::Array(s) => {
            let Some(line) = read_field_line(format!("{label} (comma-separated): ")).await else {
                return Err(());
            };
            let trimmed = line.trim();
            if trimmed.is_empty() {
                return Ok(s.default.clone().map(ElicitationContentValue::StringArray));
            }
        ...
        // Unknown/future property schema type: nothing sensible to prompt for, so omit it.
        _ => Ok(None),
    }
}
...
async fn handle_form_elicitation(
    ...
    for (name, prop) in &schema.properties {
        let required = schema.required.as_ref().is_some_and(|r| r.iter().any(|f| f == name));
        match prompt_for_property(name, prop, required).await {
            Ok(Some(value)) => {
                content.insert(name.clone(), value);
            }
            Ok(None) => {}
            Err(()) => return Ok(CreateElicitationResponse::new(ElicitationAction::Cancel)),
        }
    }
...
async fn handle_url_elicitation(
    ...
    let Some(line) = read_field_line("Press Enter after opening the URL, or type 'n' to decline: ".to_string()).await
    else {
