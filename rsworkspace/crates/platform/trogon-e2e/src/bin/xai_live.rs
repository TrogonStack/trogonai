...
use trogon_xai_runner::{FinishReason, InputItem, MockXaiHttpClient, NatsSessionNotifier, XaiAgent, XaiEvent};
...
async fn prompt(nats: &async_nats::Client, prefix: &str, sid: &str, text: &str) -> serde_json::Value {
    ...
    let raw =
        Bytes::from(serde_json::to_vec(&PromptRequest::new(sid.to_string(), vec![ContentBlock::from(text)])).unwrap());
...
async fn test_trogon_tools_present_in_request(h: &Harness) {
    ...
    for expected in &["read_file", "write_file", "str_replace", "search_files", "git_status"] {
        assert!(
            ...
            "{expected} missing from tools: {tools:?}"
        );
    }
    println!("ok");
}
...
async fn test_read_file_relative_path(h: &Harness) {
    ...
    assert!(
        out.unwrap().contains("live-content-xyz"),
        "output must contain file content, got: {:?}",
        out
    );
    println!("ok");
}
...
async fn test_search_files_finds_content(h: &Harness) {
    ...
    assert!(
        ...
        "output must contain the match, got: {:?}",
        out
    );
    println!("ok");
}
...
async fn test_fetch_url_egress_blocked(h: &Harness) {
    ...
    assert!(
        out.unwrap().contains("blocked by egress policy"),
        "output must contain egress error, got: {:?}",
        out
    );
    println!("ok");
}
...
async fn test_git_status_in_real_repo(h: &Harness) {
    ...
    h.http
        .push_response(tool_call_response("r-gs", "call-gs", "git_status", "{}"));
    ...
    assert!(
        ...
        "git_status output must include the untracked file, got: {:?}",
        out
    );
    println!("ok");
}
...
async fn test_fetch_url_allowed_local_server(h: &Harness) {
    ...
    server.mock(|when, then| {
        ...
        then.status(200)
            ...
            .body("live-fetch-response");
    });
    let url = server.url("/hello");

    let args = serde_json::json!({"url": url}).to_string();
    h.http
        .push_response(tool_call_response("r-fu", "call-fu", "fetch_url", &args));
    ...
    assert!(
        ...
        "output must contain the response body, got: {:?}",
        out
    );
    println!("ok");
}
...
async fn test_list_dir_returns_entries(h: &Harness) {
    ...
    h.http
        .push_response(tool_call_response("r-ld", "call-ld", "list_dir", "{}"));
    ...
    assert!(
        ...
        "list_dir must include alpha.rs, got: {text:?}"
    );
...
async fn test_glob_finds_files_by_pattern(h: &Harness) {
    ...
    h.http
        .push_response(tool_call_response("r-gl", "call-gl", "glob", r#"{"pattern":"*.rs"}"#));
    ...
    assert!(
        ...
        "glob must not match .toml, got: {text:?}"
    );
    println!("ok");
}
...
async fn test_git_diff_shows_unstaged_changes(h: &Harness) {
    ...
    h.http
        .push_response(tool_call_response("r-gd", "call-gd", "git_diff", "{}"));
    ...
    assert!(
        ...
        "git_diff must show changed lines, got: {text:?}"
    );
    println!("ok");
}
...
async fn test_git_log_shows_commits(h: &Harness) {
    ...
    std::process::Command::new("git")
        ...
        .unwrap();
    std::process::Command::new("git")
        .args(["commit", "-m", "live-log-commit-marker"])
        .current_dir(dir.path())
        .output()
        .unwrap();
    let cwd = dir.path().to_str().unwrap().to_string();
    ...
    h.http
        .push_response(tool_call_response("r-glog", "call-glog", "git_log", "{}"));
    ...
    assert!(
        ...
        "git_log must include the commit message, got: {:?}",
        out
    );
    println!("ok");
}
...
async fn test_multi_turn_uses_previous_response_id(h: &Harness) {
    ...
    assert!(
        calls.len() >= 3,
        "expected at least 3 HTTP calls (turn1 tool, turn1 reply, turn2), got {}",
        calls.len()
    );
    assert_eq!(
    ...
    let has_only_new_msg =
        calls[2].input.len() == 1 && matches!(&calls[2].input[0], InputItem::Message { role, .. } if role == "user");
    assert!(
        has_only_new_msg,
        "turn 2 input must be just the new user message, got: {:?}",
        calls[2].input
    );
    println!("ok");
}
...
async fn main() {
    ...
    local
        .run_until(async {
            {
                let h = start_harness("xai.live.t1").await;
                test_trogon_tools_present_in_request(&h).await;
            }
            {
                let h = start_harness("xai.live.t2").await;
                test_read_file_relative_path(&h).await;
            }
            {
                let h = start_harness("xai.live.t3").await;
                test_search_files_finds_content(&h).await;
            }
            {
                let h = start_harness("xai.live.t4").await;
                test_fetch_url_egress_blocked(&h).await;
            }
            {
                let h = start_harness("xai.live.t5").await;
                test_write_file_creates_file(&h).await;
            }
            {
                let h = start_harness("xai.live.t6").await;
                test_str_replace_edits_file(&h).await;
            }
            {
                let h = start_harness("xai.live.t7").await;
                test_git_status_in_real_repo(&h).await;
            }
            {
                let h = start_harness("xai.live.t8").await;
                test_fetch_url_allowed_local_server(&h).await;
            }
            {
                let h = start_harness("xai.live.t9").await;
                test_list_dir_returns_entries(&h).await;
            }
            {
                let h = start_harness("xai.live.t10").await;
                test_glob_finds_files_by_pattern(&h).await;
            }
            {
                let h = start_harness("xai.live.t11").await;
                test_git_diff_shows_unstaged_changes(&h).await;
            }
            {
                let h = start_harness("xai.live.t12").await;
                test_git_log_shows_commits(&h).await;
            }
            {
                let h = start_harness("xai.live.t13").await;
                test_multi_turn_uses_previous_response_id(&h).await;
            }
