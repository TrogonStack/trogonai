...
// ── Any non-zero exit code triggers retry ─────────────────────────────────────

/// A spawn job that exits with code 42 (not 1) must still be retried.
/// After exhausting retries the dead-letter must report the correct attempt count.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_non_one_exit_code_triggers_retry() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-exit42");
    ...
    assert_eq!(dl.attempts, 3, "exit 42 must trigger retry just like exit 1");

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── Multiple failing jobs each receive their own dead-letter ──────────────────

/// Two independent spawn jobs both always fail.  Each must produce its own
/// dead-letter on `cron.errors` with the correct job_id.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_multiple_failing_jobs_each_dead_lettered() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id_a = unique_id("test-multi-dl-a");
    let id_b = unique_id("test-multi-dl-b");
    ...
    let retry_cfg = || trogon_cron::RetryConfig {
        max_retries: 1,
        retry_backoff_sec: 1,
        max_backoff_sec: None,
        max_retry_duration_sec: None,
    ...
    while !got_a || !got_b {
        ...
        assert!(remaining > Duration::ZERO,
            "Timed out waiting for both dead-letters (got_a={got_a}, got_b={got_b})");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        let dl: DeadLetter = serde_json::from_slice(&msg.payload).unwrap();
        if dl.job_id == id_a { assert_eq!(dl.attempts, 2); got_a = true; }
        if dl.job_id == id_b { assert_eq!(dl.attempts, 2); got_b = true; }
    }
...
// ── max_retries at the cap (10) runs all 11 attempts ─────────────────────────

/// A spawn job with `max_retries: 10` (the maximum allowed) always fails.
/// The dead-letter must report exactly 11 attempts (1 initial + 10 retries).
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_max_retries_at_cap_runs_all_attempts() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-cap10");
    ...
    client.register_job(&JobConfig {
        ...
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 10,
            retry_backoff_sec: 1,
            max_backoff_sec: Some(1), // cap at 1 s so test completes in ~10 s
            max_retry_duration_sec: None,
        }),
    }).await.unwrap();
    ...
    // 10 retries × 1 s delay ≈ 10 s; allow 25 s for process overhead.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(25);
    let dl = loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for dead-letter");
    ...
    assert_eq!(dl.attempts, 11, "max_retries=10 must produce exactly 11 total attempts");

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── max_retry_duration shorter than backoff delay stops after first retry ──────

/// With `retry_backoff_sec=10` but `max_retry_duration_sec=1`, the sleep
/// before the first retry is capped to 1 s.  After waking up, elapsed time
/// has reached the budget so the loop breaks before a second retry fires.
/// The dead-letter must report ≤ 2 attempts (not 3+).
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_duration_shorter_than_backoff() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-dur-short");
    ...
    client.register_job(&JobConfig {
        ...
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 5,
            retry_backoff_sec: 10,      // would wait 10 s before retry
            max_backoff_sec: None,
            max_retry_duration_sec: Some(1), // budget: 1 s — caps sleep to 1 s
        }),
    }).await.unwrap();
    ...
    let dl = loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out — duration cap did not stop retries early");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        let dl: DeadLetter = serde_json::from_slice(&msg.payload).unwrap();
        if dl.job_id == id { break dl; }
    ...
    // Duration cap (1 s) must stop well before all 5 retries.
    assert!(dl.attempts <= 2,
        "expected ≤ 2 attempts with 1 s budget and 10 s backoff; got {}", dl.attempts);
    // And it must complete quickly — not wait the full 10 s backoff.
    assert!(start.elapsed() < Duration::from_secs(8),
        "took too long — backoff was not capped by max_retry_duration_sec: {:?}", start.elapsed());

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── concurrent=true: new ticks fire during retry backoff ─────────────────────

/// With `concurrent: true`, the retry loop of a previous tick must not block
/// subsequent ticks from firing.  With `retry_backoff_sec=3` and `interval_sec=1`,
/// new ticks keep arriving every second even while earlier ticks are waiting to retry.
/// We verify this by counting dead-letters: at least 2 full retry cycles must
/// complete within 12 s, which is impossible with concurrent=false (one cycle
/// takes ~3 s backoff minimum).
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_concurrent_true_ticks_fire_during_retry() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-conc-retry");
    ...
    client.register_job(&JobConfig {
        ...
        action: Action::Spawn {
            bin: "/usr/bin/false".to_string(),
            args: vec![],
            concurrent: true,  // key: new ticks must not be blocked
            timeout_sec: None,
        },
        enabled: true,
        payload: None,
    ...
    // Collect dead-letters for 12 s and assert we received at least 2.
    // With concurrent=false only ~2 full cycles (~3 s backoff each) would fit.
    // With concurrent=true many more cycles overlap, so 2+ is easy.
    #[derive(serde::Deserialize)]
    struct DeadLetter { job_id: String }
    ...
    assert!(count >= 2,
        "expected >= 2 dead-letters with concurrent=true in 12 s; got {count}");

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── Each tick cycle has a distinct execution_id in its dead-letter ────────────

/// Retry is scoped to a single tick: all retry attempts within one tick share
/// the same execution_id.  When the job fires again on the next tick, a new
/// execution_id is generated.  Two consecutive dead-letters must therefore
/// have different execution_ids.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_each_tick_cycle_has_distinct_execution_id() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-exec-id");
    ...
    #[derive(serde::Deserialize)]
    struct DeadLetter { job_id: String, execution_id: String }

    // Collect two dead-letters from this job.
    let mut execution_ids = Vec::new();
    let deadline = tokio::time::Instant::now() + Duration::from_secs(20);
    while execution_ids.len() < 2 {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for two dead-letters");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        let dl: DeadLetter = serde_json::from_slice(&msg.payload).unwrap();
        if dl.job_id == id {
            execution_ids.push(dl.execution_id);
        }
    }

    assert_ne!(execution_ids[0], execution_ids[1],
        "consecutive tick cycles must produce different execution_ids");

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── All retry attempts within one tick share the same execution_id ────────────

/// Every retry attempt within a single tick cycle must see the same
/// CRON_EXECUTION_ID environment variable.  A fresh execution_id is only
/// generated at tick time, not per-retry.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_execution_id_same_within_cycle() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-execid-same");

    let log = std::env::temp_dir().join(format!("trogon-execids-{id}"));
    let _ = std::fs::remove_file(&log);

    // Append CRON_EXECUTION_ID to log file on every invocation, then exit 1.
    let script = format!(
        "echo \"$CRON_EXECUTION_ID\" >> '{p}'; exit 1",
        p = log.display(),
    );
    ...
    // Wait until 3 lines appear in the log (1 initial + 2 retries).
    let deadline = tokio::time::Instant::now() + Duration::from_secs(15);
    loop {
        ...
        if lines >= 3 { break; }
        assert!(tokio::time::Instant::now() < deadline,
            "Expected 3 log entries, got {lines}");
        tokio::time::sleep(Duration::from_millis(200)).await;
    }

    let content = std::fs::read_to_string(&log).unwrap();
    let ids: Vec<&str> = content.lines().filter(|l| !l.is_empty()).collect();
    assert_eq!(ids.len(), 3, "expected 3 invocations");
    assert!(!ids[0].is_empty(), "execution_id must not be empty");
    assert_eq!(ids[0], ids[1], "retry 1 must share execution_id with initial attempt");
    assert_eq!(ids[0], ids[2], "retry 2 must share execution_id with initial attempt");
...
// ── Publish action with retry config fires normally ───────────────────────────

/// An Action::Publish job with a retry policy must still publish ticks on the
/// NATS subject normally.  Adding a retry config must not break publish behavior
/// when the publish succeeds on the first attempt (no regression).
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_publish_job_with_retry_config_fires_normally() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-pub-retry");
    ...
    let tick: trogon_cron::TickPayload = serde_json::from_slice(&msg.payload).unwrap();
    assert_eq!(tick.job_id, id, "publish with retry config must still deliver correct job_id");

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── Hot-reload removing retry stops dead-letters ──────────────────────────────

/// A job starts with `max_retries: 2`.  After the first dead-letter arrives,
/// the job is hot-reloaded to `retry: None`.  Subsequent failing ticks must
/// no longer produce dead-letters.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_hot_reload_removes_retry_stops_dead_letter() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-hotrl-retry");
    ...
    // Register with retry so the first cycle produces a dead-letter.
    client.register_job(&JobConfig {
        id: id.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn {
    ...
    // Wait for the first dead-letter from this job.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(15);
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for first dead-letter");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        let dl: DeadLetter = serde_json::from_slice(&msg.payload).unwrap();
        if dl.job_id == id { break; }
    }

    // Hot-reload: remove the retry policy.
    client.register_job(&JobConfig {
        id: id.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn {
    ...
    // Wait 5 s and assert no further dead-letters with our job_id arrive.
    let mut got_second_dl = false;
    ...
    loop {
        ...
        match tokio::time::timeout(remaining, errors.next()).await {
            Ok(Some(Ok(msg))) => {
                if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
                    if dl.job_id == id { got_second_dl = true; break; }
                }
            }
            _ => break,
        }
    ...
    assert!(!got_second_dl,
        "After hot-reload to retry: None, no dead-letter must be published");

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── Hot-reload adds retry: dead-letters start appearing ──────────────────────

/// A job starts with `retry: None`.  After confirming no dead-letters, the job
/// is hot-reloaded to add `max_retries: 1`.  A dead-letter must now arrive.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_hot_reload_adds_retry_starts_dead_lettering() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-hotrl-add");
    ...
    // Confirm no dead-letter for our job in the first 4 seconds.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(4);
    let mut got_early_dl = false;
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        if remaining.is_zero() { break; }
        match tokio::time::timeout(remaining, errors.next()).await {
            Ok(Some(Ok(msg))) => {
                if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
                    if dl.job_id == id { got_early_dl = true; break; }
                }
            }
            _ => break,
        }
    }
    assert!(!got_early_dl, "retry: None must not produce dead-letters");

    // Hot-reload: add retry policy.
    client.register_job(&JobConfig {
        id: id.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn {
    ...
    // A dead-letter must now arrive.
    let deadline2 = tokio::time::Instant::now() + Duration::from_secs(10);
    loop {
        let remaining = deadline2.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for dead-letter after hot-reload");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id { break; }
...
// ── Cron schedule with retry ──────────────────────────────────────────────────

/// A job with a Cron schedule (every second) and a retry policy must dead-letter
/// when all attempts fail — confirming retry works with both schedule types.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_cron_schedule_with_retry() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-cron-retry");
    ...
    // Wait for a dead-letter with exactly 2 attempts (initial + 1 retry).
    let deadline = tokio::time::Instant::now() + Duration::from_secs(15);
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for dead-letter (cron schedule)");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                assert_eq!(dl.attempts, 2, "cron schedule retry must produce 2 attempts");
                break;
            }
        }
    }
...
// ── Dead-letter error field contains exit code ────────────────────────────────

/// When a spawned process exits with a specific non-zero code, the dead-letter
/// `error` field must mention that exit code so operators can diagnose failures.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_dead_letter_error_contains_exit_code() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-dl-exitcode");
    ...
    // Exit with a distinctive code (77) so we can confirm it appears in the error field.
    client.register_job(&JobConfig {
        id: id.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn {
            bin: "/bin/sh".to_string(),
            args: vec!["-c".to_string(), "exit 77".to_string()],
            concurrent: false,
            timeout_sec: None,
        },
        enabled: true,
    ...
    #[derive(serde::Deserialize)]
    struct DeadLetter { job_id: String, error: String }
    ...
    loop {
        ...
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                assert!(
                    dl.error.contains("77"),
                    "error field must contain exit code 77, got: {:?}", dl.error
                );
                break;
            }
        }
...
// ── Disabled job does not retry ───────────────────────────────────────────────

/// A job with `enabled: false` must never fire — not even trigger the retry
/// cycle — regardless of its retry configuration.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_disabled_job_does_not_retry() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-disabled-retry");
    ...
    loop {
        ...
        match tokio::time::timeout(remaining, errors.next()).await {
            Ok(Some(Ok(msg))) => {
                if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
                    if dl.job_id == id { got_dl = true; break; }
                }
            }
            _ => break,
        }
    ...
    assert!(!got_dl, "disabled job must not produce dead-letters even with retry config");

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── Backoff timing: dead-letter arrives after the expected minimum delay ──────

/// With `retry_backoff_sec = 2` and `max_retries = 1` the scheduler sleeps
/// ~2 s between the initial attempt and the retry.  The dead-letter must arrive
/// no sooner than 1.5 s after the scheduler starts (0.8× jitter floor of 2 s).
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_backoff_timing() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-backoff-timing");
    ...
    loop {
        ...
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                ...
                // 0.8× jitter floor on 2 s backoff = 1.6 s minimum sleep.
                assert!(
                    elapsed >= Duration::from_millis(1500),
                    "dead-letter arrived too fast ({elapsed:?}); backoff must sleep at least ~1.6 s"
                );
                break;
            }
        }
...
// ── Job enabled after being disabled: retry fires normally ───────────────────

/// A job registered with `enabled: false` produces no dead-letters.  After
/// hot-reloading to `enabled: true`, the retry cycle fires and a dead-letter
/// must arrive.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_job_enabled_after_disabled_retries_normally() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-enable-retry");
    ...
    // Confirm no dead-letter while disabled.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(3);
    let mut got_early_dl = false;
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
    ...
    assert!(!got_early_dl, "disabled job must not produce dead-letters");

    // Enable the job via hot-reload.
    client.register_job(&JobConfig { enabled: true, ..job }).await.unwrap();

    // Dead-letter must now arrive.
    let deadline2 = tokio::time::Instant::now() + Duration::from_secs(10);
    loop {
        let remaining = deadline2.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for dead-letter after enabling");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id { break; }
...
// ── is_running cleared after successful retry: subsequent ticks fire ──────────

/// When a retry cycle *succeeds* (process exits 0 on a retry attempt),
/// `is_running` must be cleared so that the next scheduled tick is not skipped.
/// This verifies recovery after mid-cycle success, not just after exhaustion.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_success_subsequent_ticks_fire() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-succ-cont");

    // First invocation: create sentinel file, exit 1.
    // Second+ invocations: sentinel exists → exit 0 (success).
    // Every invocation appends a line to a log file for counting.
    let sentinel = format!("/tmp/{id}.sentinel");
    let log      = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&sentinel);
    let _ = std::fs::remove_file(&log);

    let script = format!(
        "echo x >> {log}; if [ -f {sentinel} ]; then exit 0; fi; touch {sentinel}; exit 1"
    );

    // Subscribe to errors BEFORE registering to catch any unexpected dead-letter.
    let errors_inbox = nats.new_inbox();
    let mut errors = js.get_stream(trogon_cron::kv::TICKS_STREAM)
        .await.expect("CRON_TICKS stream not found")
        .create_consumer(consumer::push::Config {
    ...
    // Wait long enough for the retry cycle to succeed and at least 2 more ticks.
    tokio::time::sleep(Duration::from_secs(8)).await;
    handle.abort();

    // No dead-letter: the retry succeeded before exhaustion.
    #[derive(serde::Deserialize)]
    struct DeadLetter { job_id: String }
    let mut got_dl = false;
    loop {
        match tokio::time::timeout(Duration::from_millis(200), errors.next()).await {
            Ok(Some(Ok(msg))) => {
                if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
                    if dl.job_id == id { got_dl = true; break; }
                }
    ...
    assert!(!got_dl, "retry succeeded → must not produce a dead-letter");

    // At least 3 invocations: attempt 1 (fail) + attempt 2 (success) + ≥1 subsequent tick.
    ...
    assert!(
        invocations >= 3,
        "after successful retry, subsequent ticks must fire; got {invocations} invocations"
    );
...
// ── max_retry_duration exhausted by the first slow attempt ───────────────────

/// If the first attempt itself runs longer than `max_retry_duration_sec`, the
/// duration budget is already exceeded when the retry loop checks it.  Only
/// 1 attempt must be recorded in the dead-letter (no retries ran).
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_max_retry_duration_exhausted_by_first_attempt() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-dur-firstatt");
    ...
    // First attempt sleeps 3 s then exits 1.
    // max_retry_duration = 2 s → by the time attempt 0 finishes (3 s elapsed)
    // the duration is exceeded and no retry runs.
    client.register_job(&JobConfig {
        id: id.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn {
    ...
    loop {
        ...
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                assert_eq!(
                    dl.attempts, 1,
                    "duration exhausted by first attempt → only 1 attempt must be recorded, got {}",
                    dl.attempts
                );
                break;
            }
...
// ── concurrent=false skips ticks while retry cycle is running ────────────────

/// With `concurrent: false`, new ticks that arrive while a retry cycle is in
/// progress must be skipped.  Over a 14 s window with interval_sec=1 and a
/// retry cycle that holds the lock for ~6 s (backoff=5 s), invocations must be
/// far fewer than one-per-second.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_concurrent_false_skips_ticks_during_retry() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-conc-false-retry");

    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    // Script appends one line per invocation, then exits 1.
    let script = format!("echo x >> {log}; exit 1");
    ...
    // Wait long enough for at least 2 full retry cycles (each ~5 s).
    ...
    // Without the concurrent=false guard: ~14 ticks × 2 attempts = 28 invocations.
    // With the guard: ≤2 cycles × 2 attempts = ≤4, generously allow up to 8.
    assert!(
        invocations <= 8,
        "concurrent=false must skip ticks during retry; got {invocations} invocations in 14 s"
    );
    // Sanity-check: scheduler actually fired at least one cycle.
    assert!(invocations >= 2, "expected at least 2 invocations, got {invocations}");

    client.remove_job(&id).await.unwrap();
    let _ = std::fs::remove_file(&log);
}

// ── Env vars present on every retry attempt ──────────────────────────────────

/// CRON_JOB_ID and CRON_EXECUTION_ID must be injected into the process
/// environment on *every* attempt of the retry cycle, not just the first.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_env_vars_present_on_every_attempt() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-envvars");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    // Script: log "OK" if both vars are set, "MISSING" otherwise. Always exit 1.
    let script = format!(
        r#"if [ -n "$CRON_JOB_ID" ] && [ -n "$CRON_EXECUTION_ID" ] && [ -n "$CRON_FIRED_AT" ]; then echo OK >> {log}; else echo MISSING >> {log}; fi; exit 1"#
    );
    ...
    // Wait for the dead-letter (3 attempts: initial + 2 retries).
    let deadline = tokio::time::Instant::now() + Duration::from_secs(15);
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for dead-letter");
    ...
    assert_eq!(lines.len(), 3, "expected 3 log lines (one per attempt), got: {lines:?}");
    for line in &lines {
        assert_eq!(*line, "OK", "env vars must be present on every attempt; got: {line:?}");
    }
...
// ── Removing and re-adding a job resets retry state ──────────────────────────

/// After a job is removed from KV and then re-registered, the scheduler must
/// treat it as a fresh job.  The retry cycle must fire again and produce a
/// new dead-letter (proving that no stale `is_running` lock is leaked).
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_remove_readd_resets_state() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-readd");
    ...
    // Wait for the first dead-letter.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for first dead-letter");
    ...
    // Remove the job, wait briefly, then re-register it.
    client.remove_job(&id).await.unwrap();
    tokio::time::sleep(Duration::from_secs(2)).await;
    client.register_job(&job).await.unwrap();

    // A second dead-letter must arrive, proving fresh state after re-add.
    let deadline2 = tokio::time::Instant::now() + Duration::from_secs(10);
    loop {
        let remaining = deadline2.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO,
            "Timed out waiting for second dead-letter after re-add");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id { break; }
...
// ── Args forwarded correctly on every retry attempt ──────────────────────────

/// Command-line arguments must be forwarded to the process on *every* attempt
/// of the retry cycle.  A script that logs its arguments to a file verifies
/// they are identical across all attempts.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_args_forwarded_on_every_attempt() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-args");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);
    ...
    // Script receives the extra args and logs them; always fails so we get a dead-letter.
    let script = format!("echo \"$@\" >> {log}; exit 1");
    let sentinel_arg = format!("--run-id={id}");
    ...
    client.register_job(&JobConfig {
        ...
        action: Action::Spawn {
            bin: "/bin/sh".to_string(),
            args: vec!["-c".to_string(), script, "sh".to_string(), sentinel_arg.clone()],
            concurrent: false,
            timeout_sec: None,
        },
        enabled: true,
    ...
    let content = std::fs::read_to_string(&log).unwrap_or_default();
    let lines: Vec<&str> = content.lines().collect();
    assert_eq!(lines.len(), 2, "expected 2 log lines (one per attempt), got: {lines:?}");
    for line in &lines {
        assert!(
            line.contains(&sentinel_arg),
            "args must be forwarded on every attempt; expected {sentinel_arg:?} in {line:?}"
        );
    }
...
// ── CRON_PAYLOAD env var forwarded on retry attempts ─────────────────────────

/// When a job has a `payload` field, `CRON_PAYLOAD` must be injected into the
/// spawned process on *every* retry attempt, not just the first one.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_cron_payload_env_var_forwarded() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-payload");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    // Script: log $CRON_PAYLOAD (empty string if unset), always exit 1.
    let script = format!(r#"echo "${{CRON_PAYLOAD:-MISSING}}" >> {log}; exit 1"#);
    ...
    client.register_job(&JobConfig {
        ...
        enabled: true,
        payload: Some(serde_json::json!({ "db": "main", "mode": "full" })),
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 1,
            retry_backoff_sec: 1,
            max_backoff_sec: None,
    ...
    let content = std::fs::read_to_string(&log).unwrap_or_default();
    let lines: Vec<&str> = content.lines().collect();
    assert_eq!(lines.len(), 2, "expected 2 log lines (one per attempt)");
    for line in &lines {
        assert_ne!(*line, "MISSING",
            "CRON_PAYLOAD must be set on every retry attempt; got MISSING on attempt");
        assert!(line.contains("main"),
            "CRON_PAYLOAD must contain the job payload JSON; got: {line:?}");
    }
...
// ── In-flight retry cycle uses config snapshot, ignores hot-reload ────────────

/// Config values extracted at tick dispatch are fixed for the lifetime of the
/// retry cycle.  A hot-reload that raises `max_retries` while the cycle is
/// sleeping between attempts must NOT extend the current cycle.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_inflight_cycle_ignores_hot_reload() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-inflight");
    ...
    let base_job = JobConfig {
        ...
        enabled: true,
        payload: None,
        // Original: only 1 retry (2 total attempts).
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 1,
            retry_backoff_sec: 5,
            max_backoff_sec: None,
    ...
    // Wait ~2 s: first attempt has fired and the cycle is now sleeping its ~5 s backoff.
    tokio::time::sleep(Duration::from_secs(2)).await;

    // Hot-reload: bump max_retries to 5 while the cycle is mid-sleep.
    client.register_job(&JobConfig {
        retry: Some(trogon_cron::RetryConfig { max_retries: 5, ..trogon_cron::RetryConfig::default() }),
        ..base_job
    }).await.unwrap();
    ...
    // The first dead-letter must still show attempts=2 (the original max_retries=1).
    let deadline = tokio::time::Instant::now() + Duration::from_secs(15);
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for dead-letter");
        ...
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                assert_eq!(
                    dl.attempts, 2,
                    "in-flight cycle must use original max_retries=1 (2 attempts), got {}",
                    dl.attempts
                );
                break;
            }
...
// ── Backoff delay doubles between consecutive retry attempts ──────────────────

/// With `retry_backoff_sec = 2` and `max_retries = 2` the delays should be
/// approximately 2 s and 4 s.  By timestamping each invocation we verify that
/// the second inter-attempt gap is at least 1.2× the first.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_backoff_doubles_between_attempts() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-doubles");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    // Script: log current time in milliseconds, then exit 1.
    let script = format!("date +%s%3N >> {log}; exit 1");
    ...
    // Wait for the dead-letter (3 attempts, ~2 + 4 = 6 s of sleep).
    let deadline = tokio::time::Instant::now() + Duration::from_secs(20);
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for dead-letter");
        ...
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                assert_eq!(dl.attempts, 3, "expected 3 total attempts");
                break;
            }
        }
    }
    ...
    let ts: Vec<u64> = content
        ...
        .collect();
    assert_eq!(ts.len(), 3, "expected 3 timestamps in log, got: {ts:?}");

    let gap1 = ts[1] - ts[0]; // ms between attempt 1 and 2 (backoff ~2 s)
    let gap2 = ts[2] - ts[1]; // ms between attempt 2 and 3 (backoff ~4 s)

    assert!(
        gap2 > (gap1 as f64 * 1.2) as u64,
        "second backoff gap ({gap2} ms) must be > 1.2× first gap ({gap1} ms); \
         exponential doubling not detected"
    );
...
// ── fired_at in dead-letter reflects original tick time ───────────────────────

/// The `fired_at` timestamp in the dead-letter must reflect when the tick
/// originally fired — not when the dead-letter was published after all retries.
/// It must fall between job registration and receipt of the dead-letter.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_fired_at_preserved_in_dead_letter() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-firedat");
    ...
    loop {
        ...
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                ...
                // fired_at must be after we registered the job (minus small clock tolerance).
                let earliest = registered_at - chrono::Duration::seconds(5);
                assert!(
                    dl.fired_at >= earliest,
                    "fired_at ({}) is before job registration ({})",
                    dl.fired_at, registered_at
                );
                // fired_at must not be in the future relative to when we received the dl.
                assert!(
                    dl.fired_at <= received_at,
                    "fired_at ({}) is after dead-letter was received ({})",
                    dl.fired_at, received_at
                );
                // fired_at must be the original tick time, so it must be close to
                // registration — not inflated to dead-letter publish time.
                let age_secs = (received_at - dl.fired_at).num_seconds();
                assert!(
                    age_secs >= 0 && age_secs < 30,
                    "fired_at is suspiciously stale ({age_secs} s); \
                     it must reflect the original tick, not retry publish time"
                );
                break;
            }
        }
...
// ── max_retries snapshot is per-cycle, hot-reload affects only future cycles ──

/// Each tick cycle captures `max_retries` at dispatch time.  After a hot-reload
/// that raises `max_retries`, the *next* cycle uses the new value while the
/// previous dead-letter already recorded the old one.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_max_retries_snapshot_per_cycle() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-snapshot");
    ...
    // Start with max_retries=1 → dead-letter shows attempts=2.
    client.register_job(&JobConfig {
        id: id.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn {
    ...
    // Wait for first dead-letter — must have attempts=2.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    let first_attempts;
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        ...
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id { first_attempts = dl.attempts; break; }
        }
    }
    assert_eq!(first_attempts, 2, "first cycle: expected attempts=2 with max_retries=1");

    // Hot-reload to max_retries=3 → next cycle's dead-letter must have attempts=4.
    client.register_job(&JobConfig {
        id: id.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn {
    ...
    let deadline2 = tokio::time::Instant::now() + Duration::from_secs(15);
    loop {
        let remaining = deadline2.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for second dead-letter");
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                assert_eq!(dl.attempts, 4,
                    "second cycle: expected attempts=4 with max_retries=3, got {}", dl.attempts);
                break;
            }
        }
    }
...
// ── concurrent=true retry cycles have independent execution_ids ───────────────

/// With `concurrent: true`, two overlapping tick cycles that both fail must
/// produce dead-letters with *distinct* execution_ids — proving each cycle
/// retries independently.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_concurrent_true_retry_independent_execution_ids() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-conc-retry-ids");
    ...
    client.register_job(&JobConfig {
        ...
        action: Action::Spawn {
            bin: "/usr/bin/false".to_string(),
            args: vec![],
            concurrent: true, // allow overlapping cycles
            timeout_sec: None,
        },
        enabled: true,
        payload: None,
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 1,
            retry_backoff_sec: 2, // ~2 s sleep → ticks overlap easily
            max_backoff_sec: None,
            max_retry_duration_sec: None,
        }),
    }).await.unwrap();
    ...
    // Collect at least 2 dead-letters within the time window.
    let mut execution_ids = Vec::new();
    let deadline = tokio::time::Instant::now() + Duration::from_secs(15);
    while execution_ids.len() < 2 {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO,
            "Timed out collecting 2 dead-letters; got {}", execution_ids.len());
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
    ...
    assert_ne!(
        ...
        "concurrent=true retry cycles must have distinct execution_ids; both were {:?}",
        execution_ids[0]
    );
...
// ── payload=None: CRON_PAYLOAD env var not injected ──────────────────────────

/// When a job has no `payload`, the `CRON_PAYLOAD` environment variable must
/// NOT be set in the spawned process on any attempt of the retry cycle.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_payload_none_cron_payload_not_set() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-nopayload");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    // Log the env var; use "UNSET" sentinel when variable is absent.
    let script = format!(r#"echo "${{CRON_PAYLOAD:-UNSET}}" >> {log}; exit 1"#);
    ...
    client.register_job(&JobConfig {
        ...
        enabled: true,
        payload: None, // no payload → CRON_PAYLOAD must not be set
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 1,
            retry_backoff_sec: 1,
            max_backoff_sec: None,
    ...
    for line in &lines {
        assert_eq!(*line, "UNSET",
            "CRON_PAYLOAD must not be set when job payload is None; got: {line:?}");
    }
...
// ── dead-letter execution_id matches process CRON_EXECUTION_ID env var ────────

/// The `execution_id` field in the dead-letter must equal the `CRON_EXECUTION_ID`
/// environment variable that was injected into the spawned process — proving the
/// ID is consistent across the entire retry lifecycle.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_dead_letter_execution_id_matches_process_env() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-execid-match");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    let script = format!("echo \"$CRON_EXECUTION_ID\" >> {log}; exit 1");
    ...
    loop {
        ...
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id { dl_execution_id = dl.execution_id; break; }
        }
    }
    ...
    for line in &lines {
        assert_eq!(
            *line, dl_execution_id,
            "process CRON_EXECUTION_ID ({line:?}) must equal dead-letter execution_id ({dl_execution_id:?})"
        );
    }
...
// ── Timeout error message in dead-letter ─────────────────────────────────────

/// When a spawned process is killed by the timeout mechanism, the dead-letter
/// `error` field must mention "timed out" and the configured timeout duration.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_timeout_error_message_in_dead_letter() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-timeout-msg");
    ...
    client.register_job(&JobConfig {
        ...
        action: Action::Spawn {
            ...
            concurrent: false,
            timeout_sec: Some(1), // killed after 1 s
        },
        enabled: true,
        payload: None,
        retry: Some(trogon_cron::RetryConfig {
    ...
    #[derive(serde::Deserialize)]
    struct DeadLetter { job_id: String, error: String, attempts: u32 }
    ...
    loop {
        ...
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                assert_eq!(dl.attempts, 2, "both attempts must time out");
                assert!(
                    dl.error.contains("timed out"),
                    "error must mention 'timed out'; got: {:?}", dl.error
                );
                assert!(
                    dl.error.contains("1"),
                    "error must mention the timeout duration (1 s); got: {:?}", dl.error
                );
                break;
            }
        }
...
// ── set_enabled(false) stops dead-letters ────────────────────────────────────

/// Calling `CronClient::set_enabled(id, false)` must stop the job from firing.
/// After disabling, no further dead-letters must arrive even though the retry
/// config is still present.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_set_enabled_false_stops_dead_letters() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-setenabled");
    ...
    // Wait for the first dead-letter to confirm the job is firing.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO, "Timed out waiting for first dead-letter");
    ...
    // Disable via the convenience API.
    client.set_enabled(&id, false).await.unwrap();

    // No further dead-letters must arrive for this job in the next 5 s.
    let mut got_dl_after_disable = false;
    let deadline2 = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let remaining = deadline2.saturating_duration_since(tokio::time::Instant::now());
        if remaining.is_zero() { break; }
        match tokio::time::timeout(remaining, errors.next()).await {
            Ok(Some(Ok(msg))) => {
                if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
                    if dl.job_id == id { got_dl_after_disable = true; break; }
                }
            }
            _ => break,
        }
    ...
    assert!(!got_dl_after_disable,
        "set_enabled(false) must stop dead-letters from the disabled job");

    handle.abort();
    client.remove_job(&id).await.unwrap();
}

// ── Spawn+retry and Publish+retry coexist without interference ────────────────

/// A failing Spawn job (produces dead-letters) and a succeeding Publish job
/// (delivers ticks) must operate independently when both have retry configs.
/// The dead-letter stream must not contain Publish-job entries and the tick
/// subject must receive messages from the Publish job.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_mixed_spawn_publish_with_retry_coexist() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let spawn_id   = unique_id("test-mixed-spawn");
    let publish_id = unique_id("test-mixed-pub");
    let subject    = format!("cron.{publish_id}");

    // Subscribe to ticks from the Publish job.
    let mut ticks = cron_messages(&nats, &js, subject.clone()).await;

    // Subscribe to errors for the Spawn job.
    let errors_inbox = nats.new_inbox();
    let mut errors = js.get_stream(trogon_cron::kv::TICKS_STREAM)
        .await.expect("CRON_TICKS stream not found")
        .create_consumer(consumer::push::Config {
    ...
    let retry = Some(trogon_cron::RetryConfig {
        max_retries: 1,
        retry_backoff_sec: 1,
        max_backoff_sec: None,
        max_retry_duration_sec: None,
    ...
    // Publish job must deliver a tick.
    let tick_msg = tokio::time::timeout(Duration::from_secs(5), ticks.next())
        .await.expect("Timed out waiting for tick from Publish job")
        .unwrap().unwrap();
    let tick: trogon_cron::TickPayload = serde_json::from_slice(&tick_msg.payload).unwrap();
    assert_eq!(tick.job_id, publish_id, "tick must come from the Publish job");

    // Spawn job must produce a dead-letter.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    let mut got_spawn_dl = false;
    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        if remaining.is_zero() { break; }
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == spawn_id { got_spawn_dl = true; break; }
            assert_ne!(dl.job_id, publish_id,
                "Publish job must not produce a dead-letter");
        }
    }
    assert!(got_spawn_dl, "Spawn job must produce a dead-letter");

    handle.abort();
    client.remove_job(&spawn_id).await.unwrap();
    client.remove_job(&publish_id).await.unwrap();
}

// ── Consecutive dead-letters have strictly increasing fired_at ────────────────

/// Each tick fires later than the previous, so consecutive dead-letters from
/// the same job must have `fired_at` timestamps in strictly increasing order.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_consecutive_dead_letters_have_increasing_fired_at() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-firedat-incr");
    ...
    // Collect 3 consecutive dead-letters.
    let mut fired_ats: Vec<chrono::DateTime<chrono::Utc>> = Vec::new();
    let deadline = tokio::time::Instant::now() + Duration::from_secs(20);
    while fired_ats.len() < 3 {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(remaining > Duration::ZERO,
            "Timed out collecting 3 dead-letters; got {}", fired_ats.len());
        let msg = tokio::time::timeout(remaining, errors.next())
            .await.expect("Timed out").unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                fired_ats.push(dl.fired_at);
            }
        }
    }

    assert!(
        fired_ats[0] < fired_ats[1],
        "first fired_at ({}) must be before second ({})",
        fired_ats[0], fired_ats[1]
    );
    assert!(
        fired_ats[1] < fired_ats[2],
        "second fired_at ({}) must be before third ({})",
        fired_ats[1], fired_ats[2]
    );
...
// ══════════════════════════════════════════════════════════════════════════════
// Six targeted retry-policy gap tests
// ══════════════════════════════════════════════════════════════════════════════

// ── 1. dead-letter attempts counter matches actual process executions ─────────

/// The `attempts` field in the dead-letter must equal the number of times the
/// process actually ran.  We cross-validate by counting log-file lines written
/// by the script against the counter reported in the dead-letter.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_attempts_matches_actual_executions() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id  = unique_id("test-retry-xcheck");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);
    ...
    let h = tokio::spawn({
        let n = nats.clone();
        async move { Scheduler::new(n).run().await.ok(); }
    });
    ...
    loop {
        ...
        assert!(rem > Duration::ZERO, "Timed out");
        let msg = tokio::time::timeout(rem, errors.next()).await.unwrap().unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id { dl_attempts = dl.attempts; break; }
        }
    }

    h.abort();

    let log_lines = std::fs::read_to_string(&log).unwrap_or_default()
        .lines().count() as u32;

    assert_eq!(dl_attempts, 4, "expected 4 attempts (max_retries=3)");
    assert_eq!(log_lines, dl_attempts,
        "log lines ({log_lines}) must equal dead-letter attempts ({dl_attempts})");

    client.remove_job(&id).await.unwrap();
    let _ = std::fs::remove_file(&log);
}

// ── 2. two concurrent=false jobs have independent is_running flags ─────────────

/// Job A (backoff=5 s) and Job B (backoff=1 s) both have `concurrent: false`.
/// Job B's retry cycle must complete independently of Job A's longer cycle.
/// B's dead-letter must arrive *before* A's, proving the flags are per-job.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_concurrent_false_two_jobs_independent_is_running() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id_a = unique_id("test-indep-a");
    let id_b = unique_id("test-indep-b");
    ...
    // Job A: long backoff (cycle ≈ 5 s)
    client.register_job(&JobConfig {
        id: id_a.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn { bin: "/usr/bin/false".to_string(), args: vec![],
                                concurrent: false, timeout_sec: None },
        enabled: true, payload: None,
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 1, retry_backoff_sec: 5,
            max_backoff_sec: None, max_retry_duration_sec: None }),
    }).await.unwrap();

    // Job B: short backoff (cycle ≈ 1 s)
    client.register_job(&JobConfig {
        id: id_b.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn { bin: "/usr/bin/false".to_string(), args: vec![],
    ...
    // Collect dead-letters; track order of arrival.
    let mut order: Vec<String> = Vec::new();
    let deadline = tokio::time::Instant::now() + Duration::from_secs(15);
    while order.len() < 2 {
        let rem = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(rem > Duration::ZERO, "Timed out collecting dead-letters; got {:?}", order);
        let msg = tokio::time::timeout(rem, errors.next()).await.unwrap().unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id_a || dl.job_id == id_b {
                if !order.contains(&dl.job_id) { order.push(dl.job_id); }
            }
        }
    }

    // B has shorter backoff → its dead-letter must arrive first.
    assert_eq!(order[0], id_b,
        "Job B (backoff=1 s) must dead-letter before Job A (backoff=5 s); got order={order:?}");
...
// ── 3. CRON_JOB_ID env var value equals the configured job id ─────────────────

/// `CRON_JOB_ID` must contain exactly the job's configured id string on every
/// retry attempt, not an empty string or a different value.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_cron_job_id_env_var_value_correct() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id  = unique_id("test-jobid-env");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    let script = format!("echo \"$CRON_JOB_ID\" >> {log}; exit 1");
    ...
    assert_eq!(lines.len(), 2, "expected 2 log lines (one per attempt)");
    for line in &lines {
        assert_eq!(*line, id.as_str(),
            "CRON_JOB_ID must equal the configured job id; got {line:?}, want {id:?}");
    }
...
// ── 4. Publish retry exhausted → dead-letter (mock publisher) ─────────────────

/// When a Publish action fails on every attempt, the retry cycle must exhaust
/// and publish a dead-letter with action="publish".
/// Uses a mock publisher (no NATS required) that fails for non-error subjects.
#[tokio::test]
async fn test_publish_retry_exhausted_dead_letters_mock() {
    use std::sync::{Arc, Mutex};
    use bytes::Bytes;
    use trogon_cron::executor::{build_job_state_at, execute};

    // Publisher that always fails on non-error subjects, records dead-letters.
    ...
    impl std::fmt::Display for PubFail {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            write!(f, "injected publish failure")
        }
    }
    ...
    let config = JobConfig {
        id: "mock-pub-retry".to_string(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Publish { subject: "cron.mock-pub-retry".to_string() },
        enabled: true,
        payload: None,
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 2,
    ...
    // execute() spawns Publish retries in a background task; wait for it to finish.
    // With retry_backoff=1 and max_retries=2: ~1+2 = 3 s of sleep.
    tokio::time::sleep(Duration::from_secs(5)).await;

    let dls = publisher.dls.lock().unwrap().clone();
    assert_eq!(dls.len(), 1, "exactly one dead-letter must be published");

    let dl = &dls[0];
    assert_eq!(dl["action"], "publish", "action field must be 'publish'");
    assert_eq!(dl["attempts"], 3, "3 total attempts (initial + 2 retries)");
    assert_eq!(dl["job_id"], "mock-pub-retry");
    assert!(dl["error"].as_str().unwrap_or("").contains("injected"),
        "error must contain the publisher's error message");
}

// ── 5. SIGTERM ignored → SIGKILL escalation ────────────────────────────────────

/// A process that ignores SIGTERM must be killed by SIGKILL after the 5-second
/// grace period, and the dead-letter must still arrive — confirming the
/// `kill_gracefully` escalation path works end-to-end.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_sigterm_ignored_escalates_to_sigkill() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id = unique_id("test-retry-sigkill");
    ...
    // SIG_IGN is inherited across exec → sleep 60 also ignores SIGTERM.
    client.register_job(&JobConfig {
        id: id.clone(),
        schedule: Schedule::Interval { interval_sec: 1 },
        action: Action::Spawn {
            bin: "/bin/sh".to_string(),
            args: vec!["-c".to_string(), "trap '' TERM; exec sleep 60".to_string()],
            concurrent: false,
            timeout_sec: Some(1), // trigger timeout → SIGTERM → (ignored) → SIGKILL
        },
        enabled: true, payload: None,
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 1, retry_backoff_sec: 1,
    ...
    #[derive(serde::Deserialize)]
    struct DeadLetter { job_id: String, attempts: u32, error: String }

    // Each attempt: 1 s timeout + up to 5 s SIGTERM grace + retry backoff.
    // Two attempts → budget up to ~14 s; allow 30 s to be safe.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(30);
    loop {
        let rem = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(rem > Duration::ZERO, "Timed out waiting for dead-letter (SIGKILL test)");
        let msg = tokio::time::timeout(rem, errors.next()).await.unwrap().unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id {
                assert_eq!(dl.attempts, 2, "both attempts must be killed");
                assert!(dl.error.contains("timed out"),
                    "error must mention 'timed out'; got {:?}", dl.error);
                break;
            }
        }
...
// ── 6. Two schedulers: leader election ensures only one fires ─────────────────

/// With two `Scheduler` instances running against the same NATS, leader election
/// must ensure only one fires ticks.  Tick count over 5 s must be consistent
/// with a *single* scheduler (≈ 1/s), not double.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_two_schedulers_only_leader_fires_retry_job() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id      = unique_id("test-two-sched");
    let subject = format!("cron.{id}");
    ...
    // Start two scheduler instances simultaneously.
    let h1 = tokio::spawn({ let n = nats.clone(); async move { Scheduler::new(n).run().await.ok(); } });
    let h2 = tokio::spawn({ let n = nats.clone(); async move { Scheduler::new(n).run().await.ok(); } });

    // Collect ticks for 5 s.
    let mut tick_count = 0usize;
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let rem = deadline.saturating_duration_since(tokio::time::Instant::now());
        if rem.is_zero() { break; }
        match tokio::time::timeout(rem, ticks.next()).await {
            Ok(Some(Ok(_))) => tick_count += 1,
            _ => break,
        }
    }
    ...
    // A single scheduler fires ≈ 5 ticks in 5 s.
    // Two schedulers firing would produce ≈ 10 ticks.
    assert!(tick_count >= 3,  "expected at least 3 ticks from one scheduler, got {tick_count}");
    assert!(tick_count <= 8,  "expected ≤ 8 ticks (single-scheduler rate); \
                               got {tick_count} — possible double-fire from both schedulers");
...
// ── CRON_FIRED_AT env var value matches dead-letter fired_at ─────────────────

/// The string value of `$CRON_FIRED_AT` injected into the process must be the
/// same RFC-3339 string stored in the dead-letter's `fired_at` field — both
/// come from the same `TickPayload` so they must be identical.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_cron_fired_at_env_matches_dead_letter_field() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id  = unique_id("test-firedat-cross");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    // Log the raw CRON_FIRED_AT string; always fail so we get a dead-letter.
    let script = format!("echo \"$CRON_FIRED_AT\" >> {log}; exit 1");
    ...
    #[derive(serde::Deserialize)]
    struct DeadLetter { job_id: String, fired_at: chrono::DateTime<chrono::Utc> }
    ...
    loop {
        ...
        let msg = tokio::time::timeout(rem, errors.next()).await.unwrap().unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<DeadLetter>(&msg.payload) {
            if dl.job_id == id { dl_fired_at = dl.fired_at; break; }
        }
    }
    h.abort();
    ...
    let content = std::fs::read_to_string(&log).unwrap_or_default();
    let lines: Vec<&str> = content.lines().collect();
    assert_eq!(lines.len(), 2, "expected 2 log lines (1 initial + 1 retry)");
    for line in &lines {
        let env_fired_at: chrono::DateTime<chrono::Utc> = line.parse()
            .unwrap_or_else(|e| panic!("CRON_FIRED_AT {line:?} not parseable as DateTime: {e}"));
        assert_eq!(
            env_fired_at, dl_fired_at,
            "CRON_FIRED_AT ({line:?}) must represent the same instant as dead-letter fired_at"
        );
    }
...
// ── Args with spaces forwarded correctly on retry ─────────────────────────────

/// Arguments that contain spaces must be forwarded as single tokens on every
/// retry attempt, not split at the space boundary.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_args_with_spaces_forwarded() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id  = unique_id("test-retry-spaces");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    // Script receives $1 (the spaced arg) and logs it, then fails.
    let script = format!("echo \"$1\" >> {log}; exit 1");
    let spaced_arg = "hello world with spaces".to_string();
    ...
    client.register_job(&JobConfig {
        ...
        action: Action::Spawn {
            bin: "/bin/sh".to_string(),
            // argv: ["-c", script, "sh", "hello world with spaces"]
            // $0=sh, $1="hello world with spaces"
            args: vec!["-c".to_string(), script, "sh".to_string(), spaced_arg.clone()],
            concurrent: false, timeout_sec: None,
        },
        enabled: true, payload: None,
        retry: Some(trogon_cron::RetryConfig {
    ...
    assert_eq!(lines.len(), 2, "expected 2 log lines (one per attempt)");
    for line in &lines {
        assert_eq!(*line, spaced_arg.as_str(),
            "spaced arg must arrive as a single token; got {line:?}");
    }
...
// ── Retry task survives scheduler abort ──────────────────────────────────────

/// Because retries are spawned as detached tokio tasks, they must complete and
/// publish their dead-letter even after the scheduler handle is aborted.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_task_survives_scheduler_abort() {
    let (nats, js) = connect_js().await;
    let client = CronClient::new(nats.clone()).await.unwrap();
    let id  = unique_id("test-retry-survive");
    let log = format!("/tmp/{id}.log");
    let _ = std::fs::remove_file(&log);

    // Script appends a line per invocation and fails.
    let script = format!("echo x >> {log}; exit 1");
    ...
    client.register_job(&JobConfig {
        ...
        retry: Some(trogon_cron::RetryConfig {
            max_retries: 1,
            retry_backoff_sec: 4, // long sleep so we can abort mid-cycle
            max_backoff_sec: None, max_retry_duration_sec: None,
        }),
    }).await.unwrap();
    ...
    // Wait long enough for the first attempt to fire and the retry sleep to start.
    tokio::time::sleep(Duration::from_secs(2)).await;

    // Abort the scheduler while the retry task is sleeping.
    h.abort();

    // The detached retry task must still wake up, run attempt 2, and dead-letter.
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    loop {
        let rem = deadline.saturating_duration_since(tokio::time::Instant::now());
        assert!(rem > Duration::ZERO,
            "Dead-letter did not arrive after scheduler abort; \
             retry task must be detached from the scheduler");
        let msg = tokio::time::timeout(rem, errors.next()).await.unwrap().unwrap().unwrap();
        if let Ok(dl) = serde_json::from_slice::<serde_json::Value>(&msg.payload) {
            if dl["job_id"] == id.as_str() {
                assert_eq!(dl["attempts"], 2, "both attempts must complete");
                break;
            }
        }
    }
...
// ── retry_backoff_sec=0 rejected by scheduler → job silently skipped ──────────

/// `retry_backoff_sec=0` passes client-side storage but is rejected by the
/// scheduler's `build_job_state_at` validator.  No ticks and no dead-letters
/// must be produced for such a job.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_backoff_zero_rejected_by_scheduler() {
    let (nats, js) = connect_js().await;
    let _client = CronClient::new(nats.clone()).await.unwrap(); // ensures CONFIG_BUCKET exists
    let id = unique_id("test-backoff-zero-sched");
    ...
    // Bypass client validation by serialising the config directly into KV.
    let raw_config = serde_json::json!({
        "id": id,
        "schedule": {"type": "interval", "interval_sec": 1},
        "action": {"type": "spawn", "bin": "/usr/bin/false", "args": [], "concurrent": false},
        "enabled": true,
        "retry": {"max_retries": 3, "retry_backoff_sec": 0}
    });
    let kv = js.get_key_value(trogon_cron::kv::CONFIG_BUCKET).await.unwrap();
    kv.put(format!("{}{}",  trogon_cron::kv::JOBS_KEY_PREFIX, id),
    ...
    // No dead-letter must appear for this job in 5 s.
    let mut got = false;
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let rem = deadline.saturating_duration_since(tokio::time::Instant::now());
    ...
    h.abort();
    assert!(!got, "retry_backoff_sec=0 must be rejected; job must not fire or dead-letter");
    kv.delete(format!("{}{}", trogon_cron::kv::JOBS_KEY_PREFIX, id)).await.unwrap();
}

// ── max_retries > 10 rejected by scheduler → job silently skipped ─────────────

/// `max_retries=11` (over the cap of 10) must be rejected by
/// `build_job_state_at`.  The job must be silently skipped by the scheduler.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_max_retries_over_cap_rejected_by_scheduler() {
    let (nats, js) = connect_js().await;
    let _client = CronClient::new(nats.clone()).await.unwrap(); // ensures CONFIG_BUCKET exists
    let id = unique_id("test-maxret-cap-sched");
    ...
    let raw_config = serde_json::json!({
        ...
        "action": {"type": "spawn", "bin": "/usr/bin/false", "args": [], "concurrent": false},
        "enabled": true,
        "retry": {"max_retries": 11, "retry_backoff_sec": 1}
    });
    let kv = js.get_key_value(trogon_cron::kv::CONFIG_BUCKET).await.unwrap();
    kv.put(format!("{}{}", trogon_cron::kv::JOBS_KEY_PREFIX, id),
           serde_json::to_vec(&raw_config).unwrap().into()).await.unwrap();
    ...
    h.abort();
    assert!(!got, "max_retries=11 must be rejected; job must not fire or dead-letter");
    kv.delete(format!("{}{}", trogon_cron::kv::JOBS_KEY_PREFIX, id)).await.unwrap();
}

// ── max_retry_duration_sec=0 rejected by scheduler → job silently skipped ─────

/// `max_retry_duration_sec=0` must be rejected by `build_job_state_at`.
/// The job must be silently skipped — no ticks, no dead-letters.
#[tokio::test]
#[ignore = "requires NATS at NATS_TEST_URL"]
async fn test_spawn_retry_duration_zero_rejected_by_scheduler() {
    let (nats, js) = connect_js().await;
    let _client = CronClient::new(nats.clone()).await.unwrap(); // ensures CONFIG_BUCKET exists
    let id = unique_id("test-dur-zero-sched");
    ...
    let raw_config = serde_json::json!({
        ...
        "action": {"type": "spawn", "bin": "/usr/bin/false", "args": [], "concurrent": false},
        "enabled": true,
        "retry": {"max_retries": 3, "retry_backoff_sec": 1, "max_retry_duration_sec": 0}
    });
    let kv = js.get_key_value(trogon_cron::kv::CONFIG_BUCKET).await.unwrap();
    kv.put(format!("{}{}", trogon_cron::kv::JOBS_KEY_PREFIX, id),
           serde_json::to_vec(&raw_config).unwrap().into()).await.unwrap();
