...
pub(crate) const SNAPSHOT_STORE_CONFIG: SnapshotStoreConfig<'static> =
    SnapshotStoreConfig::new("cron.command.resume_job.v1.", None);
...
impl Decide<ResumeJobState, JobEvent> for ResumeJobCommand {
    ...
    fn decide(state: &ResumeJobState, command: &Self) -> Result<Decision<JobEvent>, Self::Error> {
        match state {
            ...
            ResumeJobState::Present { .. } => {
                Ok(Decision::Event(NonEmpty::one(JobEvent::JobResumed {
...
impl CommandState for ResumeJobCommand {
    ...
    fn evolve(state: Self::State, event: JobEvent) -> Result<Self::State, Self::DomainError> {
        match event {
            ...
            JobEvent::JobRemoved { .. } => Ok(ResumeJobState::Deleted),
        }
    }
}
...
) -> CommandResult<ResumeJobState, JobEvent, ResumeJobError, CronError>
where
    E: StreamRead<JobId, Error = CronError> + StreamAppend<JobId, Error = CronError>,
    S: SnapshotStore<ResumeJobState, JobId, Error = CronError>,
{
...
mod tests {
    ...
    use super::*;
    use crate::{
        DeliverySpec, GetJobCommand, JobEventState, JobSpec, PauseJobCommand, RegisterJobCommand,
    ...
    fn decides_resume_from_present_disabled_state() {
        ...
        assert_eq!(
            decision,
            Decision::Event(NonEmpty::one(JobEvent::JobResumed {
                id: "backup".to_string(),
            }))
        );
    }
    ...
    fn timeline_matches_cases_by_command_stream() {
        let register = TestCase::new(decider::<RegisterJobCommand>())
            .given([])
            .when(RegisterJobCommand::new(active_job("backup")).unwrap())
    ...
    async fn run_resumes_job() {
        ...
        assert_eq!(
            outcome.events,
            NonEmpty::one(JobEvent::JobResumed {
                id: "backup".to_string(),
            })
        );
