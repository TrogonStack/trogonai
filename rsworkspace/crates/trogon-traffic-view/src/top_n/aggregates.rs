...
use super::errors::{TopNError, validate_n};
...
fn grouping_key(event: &TrafficEvent) -> Option<&str> {
    if let Some(chain) = event.act_chain.as_ref()
        && let Some(root) = chain.first()
        && let Some(agent_id) = root.agent_id.as_deref()
    {
...
pub fn most_active_agents(events: &[TrafficEvent], n: usize) -> Result<Vec<MostActiveAgent>, TopNError> {
    validate_n(n)?;
...
pub fn most_denied_agents(events: &[TrafficEvent], n: usize) -> Result<Vec<MostDeniedAgent>, TopNError> {
    validate_n(n)?;
    ...
    for event in events {
        ...
        *deny_counts.entry(key).or_default() += 1;
        if let Some(reason) = event.reason.as_ref() {
            *reason_counts.entry(key).or_default().entry(reason.clone()).or_default() += 1;
        }
    }
        ...
        .map(|(agent_id, deny_count)| {
            let top_reason = reason_counts.get(agent_id).and_then(|reasons| {
                ...
                ranked.sort_by(|(left_reason, left_count), (right_reason, right_count)| {
                    right_count.cmp(left_count).then_with(|| left_reason.cmp(right_reason))
                });
...
pub fn deepest_chains(events: &[TrafficEvent], n: usize) -> Result<Vec<DeepestChain>, TopNError> {
    ...
    ranked.sort_by(|left, right| right.depth.cmp(&left.depth).then_with(|| right.ts.cmp(&left.ts)));
    ranked.truncate(n);
    Ok(ranked)
}

pub fn longest_chain_by_tenant(events: &[TrafficEvent]) -> Result<Vec<LongestChainByTenant>, TopNError> {
    ...
    for event in events {
        ...
        best_by_tenant
            ...
            .and_modify(|current| {
                let replace =
                    candidate.depth > current.depth || (candidate.depth == current.depth && candidate.ts > current.ts);
                if replace {
...
mod tests {
    ...
    fn event(
        ...
        TrafficEvent {
            ...
            ts: Utc.with_ymd_and_hms(2026, 5, 27, 14, 0, 0).unwrap()
                + chrono::Duration::seconds(ts.parse::<i64>().expect("seconds offset from base")),
            tenant: tenant.into(),
    ...
    fn most_active_agents_prefers_root_agent_id_over_caller_sub() {
        ...
        let events = [
            ...
            event("e2", "acme", Some("acme/worker"), "allow", None, Some(chain), "2"),
        ];
    ...
    fn most_denied_agents_filters_allows_and_carries_top_reason() {
        let events = [
            event("e1", "acme", Some("acme/agent-a"), "deny", Some("policy"), None, "1"),
            event("e2", "acme", Some("acme/agent-a"), "deny", Some("policy"), None, "2"),
            event("e3", "acme", Some("acme/agent-a"), "deny", Some("quota"), None, "3"),
            event("e4", "acme", Some("acme/agent-a"), "allow", None, None, "4"),
            event("e5", "acme", Some("acme/agent-b"), "deny", Some("scope"), None, "5"),
        ];
    ...
    fn deepest_chains_orders_by_depth_then_ts_desc() {
        ...
        let events = [
            event("e1", "acme", Some("acme/a"), "allow", None, Some(shallow.clone()), "1"),
            event("e2", "acme", Some("acme/c"), "allow", None, Some(deep.clone()), "2"),
            event("e3", "acme", Some("acme/c"), "allow", None, Some(deep), "3"),
            event("e4", "acme", Some("acme/a"), "allow", None, Some(shallow), "4"),
        ];
    ...
    fn longest_chain_by_tenant_returns_one_entry_per_tenant() {
        let acme_chain = vec![hop("agent:acme/a", Some("acme/a")), hop("agent:acme/b", Some("acme/b"))];
        let beta_chain = vec![hop("agent:beta/x", Some("beta/x"))];
        let events = [
            event("e1", "acme", Some("acme/b"), "allow", None, Some(acme_chain), "1"),
            event("e2", "beta", Some("beta/x"), "allow", None, Some(beta_chain), "2"),
    ...
    fn invalid_n_returns_error() {
        ...
        assert_eq!(most_active_agents(&events, 0), Err(TopNError::InvalidN));
