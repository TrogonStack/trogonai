//! Integration tests for the Tier-1 declarative reference bundles
//! shipped with `a2a-pack`. The gateway runtime loads these bundles
//! verbatim, so a regression in either the TOML schema or the
//! evaluator semantics shows up here at unit-test speed rather than
//! at gateway boot.
...
use a2a_nats::{ingress_error_response_wire, ingress_gateway_declarative_denied_response_bytes};
...
fn rule_id(s: &str) -> Tier1DeclarativeRuleId {
    Tier1DeclarativeRuleId::new(s).expect("valid rule id")
}
...
fn per_method_allowlist_denies_other_methods() {
    ...
    assert_eq!(
        ...
        Tier1DeclarativeDecision::Deny {
            rule: rule_id("deny-other-methods"),
        }
    );
}
...
fn per_agent_allowlist_denies_other_callers() {
    ...
    assert_eq!(
        decision,
        Tier1DeclarativeDecision::Deny {
            rule: rule_id("deny-non-allowlisted-callers"),
        }
    );
}
...
fn utc_instant(year: i32, month: u8, day: u8, hour: u8, minute: u8) -> std::time::SystemTime {
    let datetime = time::Date::from_calendar_date(year, time::Month::try_from(month).expect("month in range"), day)
        .expect("valid date")
...
fn time_of_day_after_hours_profile_denies_outside_window() {
    ...
    assert_eq!(
        decision,
        Tier1DeclarativeDecision::Deny {
            rule: rule_id("deny-outside-business-hours"),
        }
    );
}
