...
impl Tier3SkillManifest {
    ...
    pub fn parse(skill_id: SkillId, raw: &serde_json::Value) -> Option<Self> {
        ...
        // `SkillId::new` is fallible on main — if the manifest carries
        // an invalid override we ignore it and fall back to the file
        // stem (which the loader already validated).
        let manifest_skill = raw
            .get("skill_id")
            .and_then(|v| v.as_str())
            .and_then(|raw| SkillId::new(raw).ok())
