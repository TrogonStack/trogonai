...
pub enum SkillScope {
    Skill { tenant: String, agent: String, skill: SkillId },
...
pub enum PerSkillDecision {
    Allow { contributor: Option<ResolvedRule> },
    Deny { reason: String, contributor: Option<ResolvedRule> },
    Shadow { would_be: Box<PerSkillDecision> },
}
    ...
    fn resolve(&self, req: &PerSkillRequest) -> PerSkillDecision {
        ...
        match best {
            Some(r) => match r.rule.effect {
                SkillEffect::Allow => PerSkillDecision::Allow {
                    contributor: Some(r),
                },
