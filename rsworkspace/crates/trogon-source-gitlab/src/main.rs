...
use {
    ...
    trogon_nats::jetstream::NatsJetStreamClient, trogon_source_gitlab::GitlabConfig,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let config = GitlabConfig::from_env(&SystemEnv);

    acp_telemetry::init_logger(
        ServiceName::TrogonSourceGitlab,
        &config.subject_prefix,
        &SystemEnv,
        &SystemFs,
    );

    info!("GitLab webhook server starting");
    ...
    let result = trogon_source_gitlab::serve(js, config).await;

    if let Err(ref e) = result {
        error!(error = %e, "GitLab webhook server stopped with error");
    } else {
        info!("GitLab webhook server stopped");
    }
