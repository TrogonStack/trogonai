...
use config::LlmGeminiConfig;
use futures::StreamExt;
use llm_types::{llm_subject_for_account, LlmPromptRequest, LLM_REQUEST_GEMINI};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let config = LlmGeminiConfig::from_env(&SystemEnv);
    let account_id = config.account_id.as_deref();

    tracing::info!(
        model = %config.default_model,
        max_tokens = config.default_max_tokens,
        "Starting llm-gemini worker"
    );
    ...
    let subject = llm_subject_for_account(LLM_REQUEST_GEMINI, account_id);

    let mut sub = nats_client
        .queue_subscribe(subject.clone(), "llm-gemini-workers".to_string())
        .await?;
