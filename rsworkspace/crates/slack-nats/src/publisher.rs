...
async fn js_publish<T: Serialize>(
    ...
    js.publish_with_headers(subject.to_string(), async_nats::HeaderMap::new(), payload.into())
        .await?
        .await?;
    Ok(())
}
...
pub async fn publish_link_shared(
    js: &jetstream::Context,
    account_id: Option<&str>,
...
pub async fn request_get_emoji(
    client: &async_nats::Client,
    account_id: Option<&str>,
...
pub async fn publish_response_url(
    js: &jetstream::Context,
    account_id: Option<&str>,
