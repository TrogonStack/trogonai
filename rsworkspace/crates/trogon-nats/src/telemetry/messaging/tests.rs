...
fn client_operation_attributes_follow_messaging_semantic_conventions() {
    ...
    assert_eq!(
        value_for(&attributes, attribute::MESSAGING_SYSTEM)
            .unwrap()
            .as_str()
            .as_ref(),
        "nats"
    ...
    assert_eq!(
        value_for(&attributes, attribute::MESSAGING_DESTINATION_NAME)
            .unwrap()
            .as_str()
            .as_ref(),
        "acp.session.new"
    ...
    assert_eq!(
        value_for(&attributes, attribute::MESSAGING_OPERATION_NAME)
            .unwrap()
            .as_str()
            .as_ref(),
        "request"
    ...
    assert_eq!(
        value_for(&attributes, attribute::MESSAGING_OPERATION_TYPE)
            .unwrap()
            .as_str()
            .as_ref(),
        "send"
...
fn error_attribute_uses_semantic_error_type_key() {
    ...
    assert_eq!(attribute.key.as_str(), attribute::ERROR_TYPE);
