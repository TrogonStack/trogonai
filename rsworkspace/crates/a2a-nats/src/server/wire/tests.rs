...
use crate::wire::{decode_request_params, encode_error, encode_success, is_notification, response_id_from_request_headers};
...
fn parse_request_params_decodes_wire_request() {
    ...
    let params: serde_json::Value =
        parse_request_params("tasks/get", &encoded.headers, &encoded.body).unwrap();
...
fn encode_success_reply_sets_result_body_only() {
    ...
    let encoded = encode_success(response_id_from_request_headers(&headers), &serde_json::json!({"ok": true})).unwrap();
