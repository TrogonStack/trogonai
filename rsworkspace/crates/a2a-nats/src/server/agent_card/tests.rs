...
async fn success_publishes_agent_card() {
    ...
    handle(&handler, &headers, &payload, Some("r".into()),
        &nats,
    )
    .await;
...
async fn no_reply_drops_request() {
    ...
    handle(&handler, &headers, &payload, None,
        &nats,
    )
    .await;
...
async fn request_without_params_still_calls_handler() {
    ...
    let payload =
        serde_json::to_vec(&serde_json::json!({"jsonrpc":"2.0","id":1,"method":"agent/getAuthenticatedExtendedCard"}))
            .unwrap();
    handle(&handler, &payload.0, &payload.1, Some("r".into()), &nats).await;
    let body = parse_published_response(&nats, 0);
...
async fn malformed_json_still_publishes_parse_error_with_null_id() {
    let nats = AdvancedMockNatsClient::new();
    let handler = stub();
    handle(&handler, &async_nats::HeaderMap::new(), b"not json at all", Some("r".into()), &nats).await;
    let body = parse_published_response(&nats, 0);
