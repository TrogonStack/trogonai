...
fn get_payload(req_id: i64, cfg_id: &str) -> (async_nats::HeaderMap, Vec<u8>) {
    wire_request(
        "tasks/pushNotificationConfig/get",
        RequestId::Number(req_id),
        serde_json::to_value(get_request(cfg_id)).unwrap(),
    )
}
...
async fn success_publishes_config() {
    ...
    let (headers, payload) = get_payload(1, "c-1");
...
async fn task_not_found_error_uses_typed_code() {
    ...
    let (headers, payload) = get_payload(2, "c");
    handle(&handler, &headers, &payload, Some("r".into()), &nats).await;
    let body = parse_published_response(&nats, 0);
    assert_eq!(
...
async fn no_reply_drops_request() {
    let nats = AdvancedMockNatsClient::new();
    let handler = stub();
    let (headers, payload) = get_payload(3, "c");
