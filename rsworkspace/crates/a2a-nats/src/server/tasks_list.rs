...
#[instrument(name = "a2a.server.tasks_list", skip(handler, headers, payload, reply_subject, nats))]
    ...
    N: trogon_nats::PublishClient,
{
    ...
    let result = match parse_request_params::<serde_json::Value>(METHOD, headers, payload) {
        ...
        Ok(raw) => match serde_json::from_value::<a2a::types::ListTasksRequest>(raw) {
    ...
    publish_reply(nats, &reply, encoded, "tasks/list reply").await;
}
