...
#[instrument(name = "a2a.server.message_send", skip(handler, payload, reply_subject, nats))]
    ...
    N: trogon_nats::PublishClient,
{
    ...
    let result = match parse_request::<a2a::types::SendMessageRequest>(payload) {
        Ok(req) => match req.params {
            Some(params) => handler.message_send(params).await,
    ...
    match bytes {
        Ok(b) => {
            ...
            if let Err(e) = nats
                ...
                .await
            {
                warn!(error = %e, "failed to publish message/send reply");
            }
        }
