...
//!
//! Two flavors:
//! - `stream_events_consumer`: filters on the exact `{task_id, req_id}` subject and
...
pub fn stream_events_consumer(prefix: &A2aPrefix, task_id: &A2aTaskId, req_id: &ReqId) -> Config {
    let pfx = prefix.as_str();
    Config {
        filter_subject: format!("{pfx}.task.{task_id}.events.{req_id}"),
...
mod tests {
    ...
    #[test]
    fn stream_events_consumer_filter_includes_req_id() {
        let config = stream_events_consumer(&p("a2a"), &tid("t1"), &rid("r1"));
        assert_eq!(config.filter_subject, "a2a.task.t1.events.r1");
    }
    ...
    fn consumers_use_explicit_ack_and_instant_replay() {
        let stream = stream_events_consumer(&p("a2a"), &tid("t1"), &rid("r1"));
    ...
    fn custom_prefix_in_consumers() {
        let stream = stream_events_consumer(&p("myapp"), &tid("t1"), &rid("r1"));
        assert_eq!(stream.filter_subject, "myapp.task.t1.events.r1");
