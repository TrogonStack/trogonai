...
#[cfg(test)]
mod tests {
    use super::{A2aStream, StreamAssignment, agents, tasks, wildcards};
    ...
    fn custom_prefix() {
        assert_eq!(
            agents::MessageSendSubject::new(&p("myapp"), &aid("planner")).to_string(),
            "myapp.agents.planner.message.send"
        );
        assert_eq!(
            tasks::TaskEventsSubject::new(&p("myapp"), &tid("t1"), &rid("r1")).to_string(),
            "myapp.tasks.t1.events.r1"
        );
    }

    #[test]
    fn stream_assignments() {
        assert_eq!(agents::MessageSendSubject::STREAM, None);
        assert_eq!(agents::MessageStreamSubject::STREAM, None);
        assert_eq!(agents::TasksGetSubject::STREAM, None);
        assert_eq!(agents::TasksListSubject::STREAM, None);
        assert_eq!(agents::TasksCancelSubject::STREAM, None);
        assert_eq!(agents::TasksResubscribeSubject::STREAM, None);
        assert_eq!(agents::PushSetSubject::STREAM, None);
        assert_eq!(agents::PushGetSubject::STREAM, None);
        assert_eq!(agents::PushListSubject::STREAM, None);
        assert_eq!(agents::PushDeleteSubject::STREAM, None);
        assert_eq!(agents::AgentCardSubject::STREAM, None);
        assert_eq!(tasks::TaskEventsSubject::STREAM, Some(A2aStream::Events));
