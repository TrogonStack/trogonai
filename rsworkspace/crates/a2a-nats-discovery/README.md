...
For KV **`watch`** patterns and client ergonomics see [`docs/a2a/how-to/catalog/kv-watch.md`](../../../docs/a2a/how-to/catalog/kv-watch.md).

Multi-tenant isolation is enforced by **separate NATS Accounts** (one per tenant), not a `{tenant}` subject segment. Each Account gets its own JetStream KV bucket **`A2A_AGENT_CARDS`** with the same name and shape; discovery subjects are `{prefix}.discover.{agent_id}` inside that Account namespace. See [`docs/a2a/how-to/operators/nsc-account-bootstrap.md`](../../../docs/a2a/how-to/operators/nsc-account-bootstrap.md) for operator provisioning.
...
Aggregated env reference: [`docs/a2a/reference/runtime-env.md`](../../../docs/a2a/reference/runtime-env.md) (**`a2a-nats-discovery`**). Docs hub: [`docs/a2a/README.md`](../../../docs/a2a/README.md).
