...
const CREDENTIAL_NAMESPACE: Uuid = Uuid::from_u128(0x73c6_35ee_2d9e_4a07_a77a_9b0b_6a87_3171);

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub(crate) struct CredentialKey(Uuid);

#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub(crate) struct CredentialStreamId(String);

impl CredentialStreamId {
    fn as_bytes(&self) -> &[u8] {
        self.0.as_bytes()
    }
}

impl From<&str> for CredentialStreamId {
...
impl CredentialKey {
    pub(crate) fn derive(credential_id: &CredentialId) -> Self {
        Self(Uuid::new_v5(&CREDENTIAL_NAMESPACE, credential_id.as_str().as_bytes()))
    }

    pub(crate) fn for_stream(stream_id: &CredentialStreamId) -> Self {
        Self(Uuid::new_v5(&CREDENTIAL_NAMESPACE, stream_id.as_bytes()))
    }
...
pub(crate) struct CredentialSubject {
...
pub(crate) fn stream_config() -> stream::Config {
    stream::Config {
        name: CREDENTIAL_STREAM.to_string(),
        subjects: vec![format!("{CREDENTIAL_EVENT_SUBJECT_PREFIX}.>")],
...
mod tests {
    ...
    fn key_derivation_is_deterministic_for_the_same_credential_id() {
        let first = CredentialKey::derive(&credential_id("openbao:tenant-1:github/primary:webhook_secret"));
        let second = CredentialKey::derive(&credential_id("openbao:tenant-1:github/primary:webhook_secret"));
    ...
    fn key_derivation_keeps_nats_unsafe_ids_out_of_subject_tokens() {
        ...
        assert_eq!(
            subject.as_str(),
            format!("{CREDENTIAL_EVENT_SUBJECT_PREFIX}.{}", key.simple())
        );
    ...
    fn stream_id_routing_preserves_key_derivation() {
        ...
        let stream_id = CredentialStreamId::from(raw);

        assert_eq!(
            CredentialKey::for_stream(&stream_id),
            CredentialKey::derive(&credential_id(raw))
        );
    }

    #[test]
    fn stream_config_matches_credential_contract() {
        let config = stream_config();

        assert_eq!(config.name, CREDENTIAL_STREAM);
        assert_eq!(config.subjects, vec![format!("{CREDENTIAL_EVENT_SUBJECT_PREFIX}.>")]);
