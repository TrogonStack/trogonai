...
/// Shared secret token the operator configures as a custom header on the
/// Datadog webhook. Datadog does not sign webhook requests, so this header is
/// the only authentication signal.
pub const HEADER_WEBHOOK_TOKEN: &str = "x-datadog-webhook-token";
