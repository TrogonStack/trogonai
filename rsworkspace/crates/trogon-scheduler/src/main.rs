...
#[cfg(not(coverage))]
use trogon_scheduler::SchedulerController;
#[cfg(not(coverage))]
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    trogon_telemetry::init_logger(
        trogon_telemetry::ServiceName::TrogonScheduler,
        [],
        &SystemEnv,
        &SystemFs,
    );

    info!("trogon-scheduler starting");

    let result = run(cli).await;

    match &result {
        Ok(()) => info!("trogon-scheduler stopped"),
        Err(e) => error!(error = %e, "trogon-scheduler stopped with error"),
    }
...
async fn run_controller(nats: async_nats::Client) -> Result<(), Box<dyn std::error::Error>> {
    let controller = SchedulerController::from_nats(nats).await?;
