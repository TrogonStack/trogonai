...
use trogon_orchestrator::{
    AnthropicOrchestratorProvider, OrchestratorConfig, OrchestratorEngine, caller::NatsAgentCaller, serve,
};
...
async fn main() {
    ...
    let caller = NatsAgentCaller::new(nats).with_timeout(Duration::from_secs(config.agent_timeout_secs));
