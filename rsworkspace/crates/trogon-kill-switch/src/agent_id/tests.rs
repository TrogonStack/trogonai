...
fn rejects_surrounding_whitespace() {
    for raw in [" agent", "agent ", "\tagent", "agent\n"] {
...
fn accepts_max_length() {
    ...
    assert_eq!(AgentId::parse(&raw).unwrap().as_str(), raw);
}
...
fn accepts_non_ascii() {
    for raw in ["agent-café", "エージェント", "Agente-1"] {
...
fn supports_standard_string_conversions_and_display() {
    ...
    assert_eq!(
        AgentIdViolation::TooLong { max: 256, actual: 257 }.to_string(),
...
fn orders_lexicographically() {
    let a = AgentId::parse("agent-a").unwrap();
    let b = AgentId::parse("agent-b").unwrap();
    assert!(a < b);
}
