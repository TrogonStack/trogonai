...
use crate::projection::{ProjectionError, build_status, encode_gateway_config, project_mcp_gateway_config};
...
pub async fn run_mcp_gateway_config_controller(
    ...
    run_controller(api, watcher_config(), ctx, reconcile_mcp_gateway_config).await
}
...
async fn reconcile_mcp_gateway_config(
    ...
    let revision = publish_bytes(&ctx.kv, &projection.key, &config_bytes, &projection.content_hash).await?;

    if let Some(pointer) = &projection.active_pointer {
        let pointer_bytes =
            serde_json::to_vec(pointer).map_err(|error| ConfigKvError::Put(format!("active pointer json: {error}")))?;
