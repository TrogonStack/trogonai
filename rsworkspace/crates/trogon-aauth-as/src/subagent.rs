...
//! that type from this crate. Since [`trogon_aauth_verify::TokenVerifier`]
//! has already verified the token's signature and freshness by the time this
//! runs, re-parsing the (trusted) payload for one extra optional field does
//! not weaken verification -- it only recovers a claim the shared type drops.
