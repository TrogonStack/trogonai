...
use agent_client_protocol::schema::v1::{
    AuthenticateResponse, InitializeResponse, LogoutResponse, NewSessionResponse, PromptResponse, StopReason,
};
use agent_client_protocol::{Error as AcpError, ErrorCode};
use std::sync::Mutex;
...
struct MockAgent {
    ...
    fail_cancel: bool,
    received_new_session: Mutex<Option<NewSessionRequest>>,
}
...
impl MockAgent {
    fn new() -> Self {
        Self {
            ...
            fail_cancel: false,
            received_new_session: Mutex::new(None),
        }
    }
...
impl AgentHandler for MockAgent {
    ...
    async fn new_session(&self, args: NewSessionRequest) -> agent_client_protocol::Result<NewSessionResponse> {
        *self.received_new_session.lock().unwrap() = Some(args);
    ...
    async fn cancel(&self, args: CancelNotification) -> agent_client_protocol::Result<()> {
        ...
        self.cancelled.lock().unwrap().push(args.session_id.to_string());
        Ok(())
    }
}
...
async fn dispatch_cancel_with_empty_payload_is_silently_ignored() {
    ...
    let (nats, agent) = dispatch_raw("acp.v1.session.s1.agent.cancel", b"", None).await;
...
async fn dispatch_ext_error_reply_is_discriminated() {
    ...
    impl AgentHandler for ErrAgent {
        ...
        async fn ext_method(
            &self,
            _: agent_client_protocol::schema::v1::ExtRequest,
    ...
    let nats = MockNatsClient::new();
    let agent = ErrAgent;
    let args = agent_client_protocol::schema::v1::ExtRequest::new("my_tool", raw_value("{}"));
    let encoded = wire_encode_request("_my_tool", &args);
    let mut msg = make_nats_message("acp.v1.global.agent.ext.my_tool", &encoded.body, Some("_INBOX.ext_err"));
    msg.headers = Some(encoded.headers);
    dispatch_message(msg, &agent, &nats).await;
