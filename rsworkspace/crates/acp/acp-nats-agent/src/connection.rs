...
async fn dispatch_global<N: PublishClient + FlushClient, A: AgentHandler + Sync>(
    ...
    match method {
        ...
        GlobalAgentMethod::Ext(ref method_name) => {
            let ext_method: std::sync::Arc<str> = method_name.as_str().into();
            if msg.reply.is_some() {
                handle_jsonrpc_request(msg, method.protocol_method().as_str(), nats, |mut req: ExtRequest| {
                    req.method = ext_method;
