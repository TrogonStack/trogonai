...
use agent_client_protocol::schema::v1::{
    ...
    ContentBlock, ContentChunk, CurrentModeUpdate, Diff, ExtNotification, ExtRequest, ExtResponse, ForkSessionRequest,
    ...
    NewSessionResponse, Plan, PlanEntry, PlanEntryPriority, PlanEntryStatus, PromptCapabilities, PromptRequest,
    PromptResponse, ResumeSessionRequest, ResumeSessionResponse, SessionCapabilities, SessionConfigOption,
    SessionConfigOptionCategory, SessionConfigOptionValue, SessionForkCapabilities, SessionId, SessionInfo,
    ...
    SetSessionModeRequest, SetSessionModeResponse, TextContent, ToolCall, ToolCallContent, ToolCallLocation,
...
use acp_nats::{AcpPrefix, AcpSessionId, AgentHandler, Bridge};
    ...
    Notif: SessionNotifier,
{
    ...
    pub(crate) terminal_output_cap: std::sync::atomic::AtomicBool,
    ...
    JsMessageOf<J>: JsRequestMessage,
{
    pub fn new(
        ...
        Self {
            ...
            gateway_config,
            terminal_output_cap: std::sync::atomic::AtomicBool::new(false),
    ...
    async fn replay_history(&self, session_id: &SessionId, state: &SessionState) {
        ...
        let supports_terminal = self.terminal_output_cap.load(std::sync::atomic::Ordering::Relaxed);
    ...
    async fn spawn_stdio_bridges(
        ...
        for s in servers {
            if let McpServer::Stdio(stdio) = s {
    ...
    /// Set the model for a session and notify the client of the updated config options.
    ///
    /// This is the direct-call counterpart to `set_session_config_option`'s `"model"`
    /// branch: `agent-client-protocol` 1.2.0 removed the dedicated `set_session_model`
    /// agent method (and its `SetSessionModelRequest`/`SetSessionModelResponse` wire
    /// types) entirely, so callers that used to invoke it as a trait method now call
    /// this inherent method with plain `(session_id, model)` arguments instead.
    pub(crate) async fn set_session_model(
        ...
        let model = Self::resolve_model(&raw_model).map(|s| s.to_string()).unwrap_or(raw_model);
        info!(session_id = %session_id_key, model = %model, "Set session model");

        let mut state = self.store.load(&session_id_key).await.map_err(
            #[cfg_attr(coverage, coverage(off))]
            |e| Error::new(ErrorCode::InternalError.into(), format!("Failed to load session: {e}")),
        )?;
        state.model = Some(model.clone());
        if let Err(e) = self.store.save(&session_id_key, &state).await {
            Self::warn_save_session_model_failed(&session_id_key, &e);
        }
...
impl<N, C, J, S, Notif> AgentHandler for TrogonAcpAgent<N, C, J, S, Notif>
where
    ...
    Notif: SessionNotifier + Send + Sync + 'static,
{
    async fn initialize(&self, args: InitializeRequest) -> Result<InitializeResponse> {
        ...
        let terminal_output = args
            ...
            .unwrap_or(false);
        self.terminal_output_cap
            .store(terminal_output, std::sync::atomic::Ordering::Relaxed);
    ...
    async fn new_session(&self, args: NewSessionRequest) -> Result<NewSessionResponse> {
        ...
        Ok(NewSessionResponse::new(sid).modes(modes).config_options(config_options))
    }
    ...
    async fn load_session(&self, args: LoadSessionRequest) -> Result<LoadSessionResponse> {
        ...
        Ok(LoadSessionResponse::new().modes(modes).config_options(config_options))
    }
    ...
    async fn prompt(&self, args: PromptRequest) -> Result<PromptResponse> {
        AgentHandler::prompt(&self.bridge, args).await
    }

    #[cfg_attr(coverage, coverage(off))]
    async fn cancel(&self, args: CancelNotification) -> Result<()> {
        AgentHandler::cancel(&self.bridge, args).await
    }
...
mod tests {
    ...
    fn build_config_options_first_option_is_mode() {
        ...
        if let agent_client_protocol::schema::v1::SessionConfigKind::Select(s) = &opts[0].kind {
    ...
    fn build_config_options_second_option_is_model() {
        ...
        if let agent_client_protocol::schema::v1::SessionConfigKind::Select(s) = &opts[1].kind {
    ...
    fn build_config_options_with_bypass_mode_has_five_select_options() {
        ...
        if let agent_client_protocol::schema::v1::SessionConfigKind::Select(s) = &opts[0].kind {
            let flat: Vec<agent_client_protocol::schema::v1::SessionConfigSelectOption> = match &s.options {
                agent_client_protocol::schema::v1::SessionConfigSelectOptions::Ungrouped(v) => v.clone(),
    ...
    mod unit {
        ...
        fn make_mock_agent() -> (MockAgent, mpsc::Receiver<agent_client_protocol::schema::v1::SessionNotification>) {
        ...
        async fn list_sessions_returns_saved_sessions() {
            run_in_local(|| async {
                ...
                let resp = agent
                    .list_sessions(agent_client_protocol::schema::v1::ListSessionsRequest::new())
                    .await
                    .unwrap();
    ...
    mod integration {
        ...
        async fn initialize_returns_protocol_version() {
            ...
            let req = InitializeRequest::new(agent_client_protocol::schema::ProtocolVersion::LATEST);
            let resp = agent.initialize(req).await.unwrap();
            assert_eq!(resp.protocol_version, agent_client_protocol::schema::ProtocolVersion::LATEST);
        }
        ...
        async fn initialize_with_client_info_logs_client_name() {
            ...
            let req = InitializeRequest::new(agent_client_protocol::schema::ProtocolVersion::LATEST)
                .client_info(agent_client_protocol::schema::v1::Implementation::new("test-client", "1.0.0"));
        ...
        async fn initialize_with_terminal_output_meta_sets_bridge_cap() {
            ...
            let req = InitializeRequest::new(agent_client_protocol::schema::ProtocolVersion::LATEST).client_capabilities(caps);
            agent.initialize(req).await.unwrap();

            assert!(
                agent.terminal_output_cap.load(std::sync::atomic::Ordering::Relaxed),
        ...
        async fn new_session_returns_mode_and_model_state() {
            ...
            assert!(
                resp.config_options
                    .as_ref()
                    .is_some_and(|opts| opts.iter().any(|o| o.id.0.as_ref() == "model")),
                "model config option must be present"
            );
        }
        ...
        async fn set_session_model_fuzzy_persists() {
            ...
            agent.set_session_model(sid.clone(), "sonnet").await.unwrap();
        ...
        async fn resume_session_returns_modes_and_models() {
            ...
            assert!(
                resp.config_options
                    .as_ref()
                    .is_some_and(|opts| opts.iter().any(|o| o.id.0.as_ref() == "model")),
                "model config option must be present on resume"
            );
        }
        ...
        async fn resume_session_returns_correct_stored_mode_and_model() {
            ...
            let config_options = resp.config_options.expect("config_options must be present on resume");
            let model_option = config_options
                .iter()
                .find(|o| o.id.0.as_ref() == "model")
                .expect("model config option must be present on resume");
            if let agent_client_protocol::schema::v1::SessionConfigKind::Select(s) = &model_option.kind {
                assert_eq!(
                ...
                panic!("model config option must be a Select kind");
            }
        }
        ...
        async fn fork_session_branch_at_index_zero_produces_empty_history() {
            ...
            assert_eq!(forked_state.branched_at_index, Some(0));
        ...
        async fn ext_list_children_only_returns_direct_children_not_grandchildren() {
            ...
            let parse = |resp: agent_client_protocol::schema::v1::ExtResponse| -> Vec<String> {
    ...
    async fn load_session_returns_session_modes() {
        ...
        assert!(
            resp.config_options
                .as_ref()
                .is_some_and(|opts| opts.iter().any(|o| o.id.0.as_ref() == "model")),
            "must return model config option"
        );
    }
    ...
    async fn set_session_model_resolves_token_to_full_id() {
        ...
        agent.set_session_model(session_id.clone(), "opus").await.unwrap();
