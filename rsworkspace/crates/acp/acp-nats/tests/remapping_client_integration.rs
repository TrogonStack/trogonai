...
use acp_nats::{AcpPrefix, Bridge, ClientHandler, Config, IdRemap, NatsAuth, NatsConfig, RemappingClient, client};
use agent_client_protocol::schema::v1::{
    RequestPermissionOutcome, RequestPermissionRequest, RequestPermissionResponse, SessionId, SessionNotification,
...
async fn permission_request_is_relayed_with_remapped_session_id() {
    ...
    local
        .run_until(async move {
            ...
            let id_remap: IdRemap = Arc::new(Mutex::new(HashMap::new()));
            id_remap
                .lock()
                .unwrap()
