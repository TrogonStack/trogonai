// Some helpers below (verify_jwt_issuer + decode_server_auth_request_jwt +
// normalize_auth_request_payload) are not exercised until the server-auth
// claims + codec modules land in a later slice — see
