...
use trogon_semconv::span::A2A_SERVER_TASKS_GET;

use crate::constants::TASKS_GET_METHOD as METHOD;
...
#[instrument(name = A2A_SERVER_TASKS_GET, skip(handler, headers, payload, reply_subject, nats))]
    ...
    N: trogon_nats::PublishClient,
{
    ...
    let result = match parse_request_params::<serde_json::Value>(METHOD, headers, payload) {
        ...
        Ok(raw) => match serde_json::from_value::<a2a::types::GetTaskRequest>(raw) {
            Err(e) => Err(A2aError::new(-32602, format!("Invalid params: {e}"))),
            Ok(params) => handler.tasks_get(params).await,
        },
    };
    ...
    publish_reply(nats, &reply, encoded, "tasks/get reply").await;
}
