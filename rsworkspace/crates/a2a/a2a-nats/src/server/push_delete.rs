...
use trogon_semconv::span::A2A_SERVER_PUSH_DELETE;

use crate::constants::PUSH_DELETE_METHOD as METHOD;
    ...
    N: trogon_nats::PublishClient,
{
    ...
    let Some(reply) = reply_subject else {
        warn!("push/delete received without reply subject; dropping");
        return;
    };
    ...
    let result = match parse_request_params::<serde_json::Value>(METHOD, headers, payload) {
        ...
        Ok(raw) => match serde_json::from_value::<a2a::types::DeleteTaskPushNotificationConfigRequest>(raw) {
            Err(e) => Err(A2aError::new(-32602, format!("Invalid params: {e}"))),
            Ok(params) => handler.push_notification_delete(params).await,
        },
    };
    let encoded = match result {
        Ok(()) => encode_success_reply(headers, &serde_json::Value::Null),
        Err(e) => encode_error_reply(headers, e.code, e.message, None),
    };
    publish_reply(nats, &reply, encoded, "push/delete reply").await;
}
