...
use trogon_semconv::span::A2A_SERVER_PUSH_LIST;

use crate::constants::PUSH_LIST_METHOD as METHOD;
...
#[instrument(name = A2A_SERVER_PUSH_LIST, skip(handler, headers, payload, reply_subject, nats))]
    ...
    N: trogon_nats::PublishClient,
{
    ...
    let Some(reply) = reply_subject else {
        warn!("push/list received without reply subject; dropping");
        return;
    };
    ...
    let result = match parse_request_params::<serde_json::Value>(METHOD, headers, payload) {
        ...
        Ok(raw) => match serde_json::from_value::<a2a::types::ListTaskPushNotificationConfigsRequest>(raw) {
    ...
    publish_reply(nats, &reply, encoded, "push/list reply").await;
}
