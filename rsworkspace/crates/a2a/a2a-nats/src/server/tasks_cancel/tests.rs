...
fn cancel_payload(id: i64, task_id: &str) -> (async_nats::HeaderMap, Vec<u8>) {
    wire_request(
        "tasks/cancel",
        RequestId::Number(id),
...
async fn success_publishes_canceled_task() {
    ...
    let (headers, payload) = cancel_payload(1, "t-1");
...
async fn task_not_cancelable_error_uses_typed_code() {
    ...
    let (headers, payload) = cancel_payload(2, "t");
    handle(&handler, &headers, &payload, Some("r".into()), &nats).await;
    let body = parse_published_response(&nats, 0);
    assert_eq!(
...
async fn no_reply_drops_request() {
    let nats = AdvancedMockNatsClient::new();
    let handler = stub();
    let (headers, payload) = cancel_payload(3, "t");
...
async fn missing_params_returns_invalid_params_error() {
    let nats = AdvancedMockNatsClient::new();
    let handler = stub();
    let (headers, payload) = wire_request("tasks/cancel", RequestId::Number(5), serde_json::Value::Null);
    handle(&handler, &headers, &payload, Some("r".into()), &nats).await;
    let body = parse_published_response(&nats, 0);
