...
use crate::constants::TASKS_RESUBSCRIBE_METHOD as METHOD;
    ...
    N: trogon_nats::PublishClient,
{
    ...
    let result = match parse_request_params::<serde_json::Value>(METHOD, headers, payload) {
        ...
        Ok(raw) => match serde_json::from_value::<a2a::types::SubscribeToTaskRequest>(raw) {
    ...
    publish_reply(nats, &reply, encoded, "tasks/resubscribe reply").await;
}
