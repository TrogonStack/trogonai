    ...
    R: trogon_registry::RegistryStore,
{
    ...
    async fn export_history(&self, prefix: &str, session_id: &str) -> Option<Vec<PortableMessage>> {
        ...
        {
            Ok(resp) => match serde_json::from_str::<Vec<PortableMessage>>(resp.0.get()) {
                Ok(msgs) => Some(msgs),
                Err(e) => {
    ...
    async fn import_history(&self, prefix: &str, session_id: &str, history: &[PortableMessage]) {
        let Some(bridge) = self.get_or_create_bridge(prefix) else {
            return;
        };
        let messages_json = match serde_json::to_string(history) {
            Ok(j) => j,
            Err(e) => {
                ...
                && let Some(prefix) = self.resolve_external_prefix(model).await
            {
                ...
                if let Some(runner_sid) = self.open_runner_session(&prefix, &cwd, model).await {
                    if !state.messages.is_empty() {
                        ...
                        self.import_history(&prefix, &runner_sid, &portable).await;
                    }
                    self.active_sessions
                        .borrow_mut()
                        .insert(acp_sid.clone(), (prefix.clone(), runner_sid.clone()));
...
mod tests {
    ...
    async fn set_session_model_cwd_from_store_when_session_cwd_absent() {
        run_local(|| async {
            ...
            state.cwd = "/from/store".to_string();
    ...
    async fn prompt_lazy_reinit_populates_session_cwd_from_store() {
        run_local(|| async {
            ...
            let mut state = SessionState::default();
            state.cwd = "/restored/cwd".to_string();
            store.save(acp_sid, &state).await.unwrap();
            let nats = AdvancedMockNatsClient::new();
            let reg = Registry::new(MockRegistryStore::new());
            let (notif_tx, _) = mpsc::channel(64);
    ...
    mod integration {
        ...
        async fn history_migration_exports_from_source_and_imports_to_target() {
            ...
            assert!(
                export_rx.is_terminated() || export_rx.try_recv().is_ok() || matches!(export_rx.try_recv(), Err(_)),
                "session/export must have been called on the source runner (xai) during migration"
            );
            assert!(
                import_rx.is_terminated() || import_rx.try_recv().is_ok() || matches!(import_rx.try_recv(), Err(_)),
