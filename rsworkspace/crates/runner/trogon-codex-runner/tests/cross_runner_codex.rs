...
use acp_nats::AgentHandler as _;
use acp_nats::acp_prefix::AcpPrefix;
use agent_client_protocol::schema::v1::{ContentBlock, ExtRequest, NewSessionRequest, PromptRequest, TextContent};
...
async fn cross_runner_codex_export_into_openrouter_import() {
    ...
    local
        .run_until(async {
            ...
            let src = codex_agent.new_session(NewSessionRequest::new("/tmp")).await.unwrap();
            ...
            let import_body = format!(r#"{{"sessionId":"or-codex-s1","messages":{exported_json}}}"#);
            ...
            let or_export_params: Arc<serde_json::value::RawValue> =
                serde_json::value::RawValue::from_string(serde_json::json!({ "sessionId": "or-codex-s1" }).to_string())
                    .unwrap()
                    .into();
...
async fn cross_runner_xai_export_into_codex_import() {
    ...
    local
        .run_until(async {
            ...
            let export_params: Arc<serde_json::value::RawValue> = serde_json::value::RawValue::from_string(
            ...
            let dst = codex_agent.new_session(NewSessionRequest::new("/tmp")).await.unwrap();
            let dst_id = dst.session_id.to_string();

            let import_body = format!(r#"{{"sessionId":"{}","messages":{exported_json}}}"#, dst_id);
            let import_params: Arc<serde_json::value::RawValue> =
                serde_json::value::RawValue::from_string(import_body).unwrap().into();
            codex_agent
                .ext_method(ExtRequest::new("session/import", import_params))
            ...
            let codex_portable: Vec<PortableMessage> =
                serde_json::from_str(codex_export_resp.0.get()).expect("codex export should be valid JSON");
            ...
            for (xai_msg, codex_msg) in xai_portable.iter().zip(codex_portable.iter()) {
                assert_eq!(xai_msg.role, codex_msg.role, "role mismatch in xai→codex round-trip");
                assert_eq!(xai_msg.text, codex_msg.text, "text mismatch in xai→codex round-trip");
            }
        })
        .await;
}
...
/// Import ACP-style export (containing PortableBlock::ToolUse and
...
async fn cross_runner_acp_style_export_into_codex_import() {
    ...
    local
        .run_until(async {
            ...
            let codex_portable: Vec<PortableMessage> =
                serde_json::from_str(codex_export.0.get()).expect("codex export must be valid JSON");
            ...
            assert!(
                has_tool_call,
                "ToolCall block must survive codex import/export round-trip"
            );
            ...
            assert!(
                has_tool_result,
                "ToolResult block must survive codex import/export round-trip"
            );
        })
        .await;
}
