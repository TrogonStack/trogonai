...
use agent_client_protocol::schema::v1::{
    CancelNotification, CloseSessionRequest, ContentBlock, ExtRequest, ForkSessionRequest,
    ListSessionsRequest, LoadSessionRequest, NewSessionRequest, PromptRequest,
    ResumeSessionRequest, SetSessionConfigOptionRequest, SetSessionModeRequest, StopReason,
    TextContent,
};
...
/// create → prompt → fork → prompt fork → close source → verify source is
/// gone, fork still works.
#[tokio::test(flavor = "current_thread")]
...
/// subprocess, then load it and verify the state.
#[tokio::test(flavor = "current_thread")]
...
async fn ext_list_children_only_returns_direct_children_not_grandchildren_integration() {
    ...
    local
        .run_until(async {
            ...
            let parse_children = |resp: agent_client_protocol::schema::v1::ExtResponse| -> Vec<String> {
...
async fn codex_tool_events_written_as_portable_blocks_in_export() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                has_tool_call,
                "export must contain PortableBlock::ToolUse for 'bash'; got: {msgs:?}"
            );
