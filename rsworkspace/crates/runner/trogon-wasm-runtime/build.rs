...
fn main() {
    ...
    // Cargo.lock lives at the workspace root, three levels above this crate
    // (crates/<group>/<crate>).
    let workspace_root = Path::new(&manifest_dir).parent().unwrap().parent().unwrap().parent().unwrap();
