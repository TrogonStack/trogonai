...
fn to_proto(s: &SessionState) -> sp::SessionRecord {
    sp::SessionRecord {
        ...
        // Platform-branch SessionState parity (ADR 0009): native fields for simple
        // types, JSON strings for complex nested types (empty string = default).
        system_prompt_override: s.system_prompt_override.clone(),
        additional_read_dirs: s.additional_read_dirs.clone(),
        tool_allowlist: s.tool_allowlist.clone(),
        env_json: encode_json_skip_empty(s.env.is_empty(), &s.env),
        scope_json: s.scope.as_ref().map(|v| serde_json::to_string(v).unwrap_or_default()).unwrap_or_default(),
        tool_hooks_json: encode_json_skip_empty(s.tool_hooks.is_empty(), &s.tool_hooks),
        background_jobs_json: encode_json_skip_empty(s.background_jobs.is_empty(), &s.background_jobs),
        openapi_servers_json: encode_json_skip_empty(s.openapi_servers.is_empty(), &s.openapi_servers),
        __buffa_unknown_fields: Default::default(),
    }
}

/// Serialize `value` to JSON, or return an empty string when `is_empty` (so the
/// proto field stays empty and decodes back to the type's default).
fn encode_json_skip_empty<T: serde::Serialize>(is_empty: bool, value: &T) -> String {
    if is_empty {
        String::new()
    } else {
...
/// Decode a JSON string written by [`encode_json_skip_empty`]; an empty string
/// (legacy payloads or skipped defaults) yields the type's default.
fn decode_json_or_default<T: serde::de::DeserializeOwned + Default>(json: &str) -> T {
    if json.is_empty() {
        T::default()
    } else {
...
fn from_proto(r: sp::SessionRecord) -> SessionState {
    ...
    SessionState {
        ...
        // Platform-branch SessionState parity (ADR 0009): native fields read back
        // directly; complex nested types are JSON-decoded (empty string = default).
        system_prompt_override: r.system_prompt_override,
        additional_read_dirs: r.additional_read_dirs,
        tool_allowlist: r.tool_allowlist,
        env: decode_json_or_default(&r.env_json),
        scope: if r.scope_json.is_empty() {
            None
        } else {
            serde_json::from_str(&r.scope_json).ok()
        },
        tool_hooks: decode_json_or_default(&r.tool_hooks_json),
        background_jobs: decode_json_or_default(&r.background_jobs_json),
        openapi_servers: decode_json_or_default(&r.openapi_servers_json),
    }
}
...
fn mcp_from_proto(m: &sp::McpServer) -> StoredMcpServer {
    StoredMcpServer {
        ...
        // stdio fields (platform branch) are not carried by the proto schema.
        command: String::new(),
        args: Vec::new(),
        env: Vec::new(),
        timeout_secs: None,
...
mod tests {
    ...
    fn fully_populated() -> SessionState {
        SessionState {
            ...
            // Platform-branch fields now persisted via proto (ADR 0009). Simple fields
            // populated with non-defaults to exercise the round trip; complex nested
            // types (scope/tool_hooks/background_jobs/openapi) left at default here.
            openapi_servers: Vec::new(),
            system_prompt_override: Some("custom identity".into()),
            additional_read_dirs: vec!["/read-only-a".into(), "/read-only-b".into()],
            tool_hooks: crate::hooks::HooksConfig::default(),
            tool_allowlist: vec!["Read".into(), "Bash".into()],
            scope: None,
