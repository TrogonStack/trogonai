...
use acp_nats::{
    AgentHandler, Bridge, FlushClient, JetStreamGetStream, JetStreamPublisher, PublishClient, RequestClient,
    SubscribeClient,
};
    ...
    JsMessageOf<J>: JsRequestMessage,
{
    ...
    let resp = AgentHandler::new_session(bridge, req)
        .await
    ...
    if let Some(text) = permission_rules_text {
        AgentHandler::set_session_config_option(
            bridge,
    ...
    JsMessageOf<J>: JsRequestMessage,
{
    ...
    let prompt_result = tokio::time::timeout(timeout, AgentHandler::prompt(bridge, prompt_req)).await;

    // Best-effort close — ignore errors so partial results are still returned.
    let _ = AgentHandler::close_session(bridge, CloseSessionRequest::new(SessionId::new(session_id))).await;
