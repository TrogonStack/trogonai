...
pub fn timeout_from_meta(meta: Option<&agent_client_protocol::schema::v1::Meta>) -> Option<u64> {
...
pub async fn build_session_mcp(
    ...
    for server in servers {
        ...
        let (client, tools): (Arc<dyn trogon_mcp::McpCallTool>, Vec<trogon_mcp::McpTool>) = if !server
            .command
            .is_empty()
        {
            let stdio = match trogon_mcp::StdioMcpClient::spawn(&server.command, &server.args, &server.env, None).await
            {
...
mod tests {
    ...
    fn convert_reads_timeout_from_meta() {
        use agent_client_protocol::schema::v1::{McpServerHttp, McpServerSse};
