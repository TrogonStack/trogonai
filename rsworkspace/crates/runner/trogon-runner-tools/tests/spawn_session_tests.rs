...
use agent_client_protocol::schema::v1::{CloseSessionResponse, NewSessionResponse, PromptResponse, SessionId, StopReason};
...
/// In `bypassPermissions` mode, `create_sub_session` applies the mode via
/// `set_session_mode` after creation (best-effort); with only a `new_session`
/// response mocked, the mode call is swallowed and it still returns `Ok`.
#[tokio::test]
async fn create_sub_session_bypass_mode_applies_set_session_mode() {
    let (mock, _js, bridge) = make_mock_bridge("acp");
...
async fn run_sub_session_returns_ok_on_success() {
    ...
    enqueue_js_response(&js, &CloseSessionResponse::new());
