...
use agent_client_protocol::schema::v1::{
    ...
    CloseSessionRequest, CloseSessionResponse, ContentBlock, EmbeddedResourceResource, ExtRequest, ExtResponse,
...
impl<S: SessionStore, A: AgentRunner + 'static, N: SessionNotifier, M: TrogonMdLoading + Clone + 'static> TrogonAgent<S, A, N, M> {
    ...
    #[cfg_attr(coverage, coverage(off))]
    /// The models offered via the `model` session config option (`set_session_config_option`).
    ///
    /// Model switching moved to the config-option mechanism in ACP 1.2 (the
    /// dedicated `set_session_model` agent method was removed upstream).
    fn available_models(&self) -> Vec<(&'static str, &'static str)> {
        vec![
    ...
    fn make_acp_session_id(&self, session_id: &SessionId) -> agent_client_protocol::Result<AcpSessionId> {
    ...
    fn acquire_session_lock(&self, session_id: &str) -> Arc<tokio::sync::Semaphore> {
        let mut locks = self.session_locks.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
        locks
    ...
    fn session_mcp_cache(&self, session_id: &str) -> Arc<trogon_runner_tools::SessionMcpCache> {
        let mut caches = self.session_mcp_caches.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
        caches
            .entry(session_id.to_string())
    ...
    async fn run_prompt(
        ...
        let agent: Arc<A> = {
            ...
            if needs_clone {
                ...
                if needs_perm && let Some(ref perm_tx) = self.permission_tx {
                    // `TrogonMdLoading::load` is `?Send` (from `trogon-runner-tools`), so its
                    // future can't be held across an `.await` inside this `Send`-bound
                    // `AgentHandler::prompt` call chain. Run it to completion on the current
                    // `LocalSet` (which the whole runner already executes inside) and await
                    // the `Send` `JoinHandle` instead.
                    let md_loader = self.md_loader.clone();
                    let cwd = state.cwd.clone();
                    let trogon_md = tokio::task::spawn_local(async move { md_loader.load(&cwd).await })
                        .await
                        .unwrap_or_default();
                    let mut rules = if let Some(trogon_md) = trogon_md {
        ...
        //
        // `TrogonMdLoading::load` is `?Send` (from `trogon-runner-tools`), so its future
        // can't be held across an `.await` inside this `Send`-bound `AgentHandler::prompt`
        // call chain. Run it to completion on the current `LocalSet` (which the whole
        // runner already executes inside) and await the `Send` `JoinHandle` instead.
        let md_loader = self.md_loader.clone();
        let cwd = state.cwd.clone();
        let trogon_md = tokio::task::spawn_local(async move { md_loader.load(&cwd).await })
            .await
                            ...
                            let prompt_event = match event {
                                ...
                                AgentEvent::ToolCallFinished { id, output, exit_code, signal } => {
                                    ...
                                    let exit_mode = if is_exit_plan {
                                        exit_plan_mode_cell.lock().unwrap_or_else(std::sync::PoisonError::into_inner).take()
                                    } else {
                                        None
                                    };
...
impl<S: SessionStore, A: AgentRunner + 'static, N: SessionNotifier, M: TrogonMdLoading + Clone + Sync + 'static>
    AgentHandler for TrogonAgent<S, A, N, M>
{
    #[cfg_attr(coverage, coverage(off))]
    ...
    async fn new_session(&self, req: NewSessionRequest) -> agent_client_protocol::Result<NewSessionResponse> {
        ...
        let response = NewSessionResponse::new(session_id.clone()).modes(self.session_mode_state(&state.mode));
    ...
    async fn load_session(&self, req: LoadSessionRequest) -> agent_client_protocol::Result<LoadSessionResponse> {
        ...
        let response = LoadSessionResponse::new().modes(self.session_mode_state(&state.mode));
        self.publish_session_ready(&session_id).await?;
        Ok(response)
    }
    ...
    async fn set_session_config_option(
        ...
        let value = match &req.value {
            agent_client_protocol::schema::v1::SessionConfigOptionValue::ValueId { value } => value.to_string(),
            agent_client_protocol::schema::v1::SessionConfigOptionValue::Boolean { value } => value.to_string(),
            _ => String::new(),
        };
        ...
        let model_options: Vec<SessionConfigSelectOption> = self
            .available_models()
            .into_iter()
            .map(|(model_id, name)| SessionConfigSelectOption::new(model_id, name))
            .collect();
    ...
    async fn close_session(&self, req: CloseSessionRequest) -> agent_client_protocol::Result<CloseSessionResponse> {
        ...
        self.session_locks.lock().unwrap_or_else(std::sync::PoisonError::into_inner).remove(&session_id);
        self.session_mcp_caches.lock().unwrap_or_else(std::sync::PoisonError::into_inner).remove(&session_id);
    ...
    async fn ext_method(&self, args: ExtRequest) -> agent_client_protocol::Result<ExtResponse> {
        ...
        if args.method.as_ref() == "session/import" {
            ...
            self.store
                ...
                .await
                .map_err(|e| internal_error(e.to_string()))?;
            // `"{}"` is a hardcoded, always-valid JSON literal, so this can never fail.
...
mod tests {
    ...
    async fn list_sessions_exposes_token_totals_after_prompt() {
        use agent_client_protocol::schema::v1::{ListSessionsRequest, PromptRequest};
        use trogon_agent_core::agent_loop::AgentEvent;
        use trogon_runner_tools::session_store::{SessionState, mock::MemorySessionStore};
