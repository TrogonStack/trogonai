...
use agent_client_protocol::schema::v1::{
    CancelNotification, CloseSessionRequest, ExtRequest, ForkSessionRequest, InitializeRequest, ListSessionsRequest,
    LoadSessionRequest, NewSessionRequest, PromptRequest, ResumeSessionRequest, SessionConfigOptionValue,
    ...
    local
        .run_until(async {
            ...
            let prompt_req = PromptRequest::new(
                session_id.clone(),
                vec![agent_client_protocol::schema::v1::ContentBlock::Text(TextContent::new("hello"))],
            );
...
async fn prompt_sets_title_from_first_text_block() {
    ...
    local
        .run_until(async {
            ...
            let prompt_req = PromptRequest::new(
                session_id.clone(),
                vec![agent_client_protocol::schema::v1::ContentBlock::Text(TextContent::new(
                    "What is Rust?",
                ))],
            );
...
async fn prompt_max_iterations_returns_max_turn_requests() {
    ...
    local
        .run_until(async {
            ...
            let prompt_req = PromptRequest::new(
                session_id,
                vec![agent_client_protocol::schema::v1::ContentBlock::Text(TextContent::new("loop"))],
            );
