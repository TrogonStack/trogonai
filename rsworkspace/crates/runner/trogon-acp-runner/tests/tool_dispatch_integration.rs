...
use agent_client_protocol::schema::v1::{ContentBlock, NewSessionRequest, PromptRequest, TextContent};
...
async fn acp_tool_use_cycle_exported_as_portable_blocks_via_wire() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                has_tool_call,
                "exported messages must contain PortableBlock::ToolUse for read_file; got: {messages:?}"
            );

            let has_tool_result = messages.iter().any(|m| {
                m.blocks.iter().any(|b| {
                    matches!(b, PortableBlock::ToolResult { output_summary, .. }
                        if output_summary.contains("sentinel-export-content-xyz"))
                })
            });
            assert!(
                has_tool_result,
