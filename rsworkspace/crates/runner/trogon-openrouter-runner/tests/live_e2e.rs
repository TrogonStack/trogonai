...
use agent_client_protocol::schema::v1::{
    AuthenticateRequest, CloseSessionRequest, ContentBlock, McpServer, McpServerHttp, NewSessionRequest, PromptRequest,
    SessionNotification,
};
    ...
    let responder = tokio::spawn(async move {
        if let Some(req) = elic_rx.recv().await {
            *cq.lock().unwrap() = Some(req.request.message.clone());
            ...
            let resp = agent_client_protocol::schema::v1::CreateElicitationResponse::new(
