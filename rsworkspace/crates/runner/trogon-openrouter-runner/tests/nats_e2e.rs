...
use agent_client_protocol::schema::v1::{
    CloseSessionRequest, ContentBlock, CreateTerminalResponse, ExtRequest, ForkSessionRequest, LoadSessionRequest,
    NewSessionRequest, PromptRequest, PromptResponse, SessionConfigKind, SessionId, SetSessionConfigOptionRequest,
...
/// Records the model and messages array passed to each chat_stream call.
#[derive(Clone)]
...
async fn history_and_model_survive_kv_round_trip() {
    ...
    // new_session → prompt (adds history) → close_session (saves to KV).
    let session_id = {
        let http = QueuedHttpClient::new();
        http.push(vec![OpenRouterEvent::TextDelta {
    ...
    tokio::task::LocalSet::new()
        .run_until(async move {
            ...
            let (model, msgs) = recorded
                .last()
                .expect("at least one chat_stream call must have been made");
            // The model must come from the KV snapshot ("saved-model"), not from agent2's default ("test-model").
            assert_eq!(model, "saved-model", "model must survive KV round-trip; got {model:?}");
    ...
    tokio::task::LocalSet::new()
        .run_until(async move {
            ...
            let r_a = agent
                ...
                .unwrap();
            assert!(
                matches!(r_a.stop_reason, agent_client_protocol::schema::v1::StopReason::EndTurn),
            ...
            let r_b = agent
                ...
                .unwrap();
            assert!(
                matches!(r_b.stop_reason, agent_client_protocol::schema::v1::StopReason::EndTurn),
