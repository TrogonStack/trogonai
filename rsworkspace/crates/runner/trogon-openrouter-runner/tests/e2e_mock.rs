...
use agent_client_protocol::schema::v1::{
    ...
    ResourceLink, ResumeSessionRequest, ResumeSessionResponse, SessionConfigKind, SessionNotification, SessionUpdate,
    SetSessionConfigOptionRequest, SetSessionConfigOptionResponse, SetSessionModeRequest, SetSessionModeResponse,
    TextResourceContents, ToolCallStatus, ToolKind,
};
...
impl OpenRouterHttpClient for TestHttpClient {
    async fn chat_stream(
        ...
        match response {
            TestResponse::Events(events) => stream::iter(events).boxed(),
...
async fn create_session_with_mcp(h: &Harness, servers: Vec<agent_client_protocol::schema::v1::McpServer>) -> String {
    let before = h.nats.published_payloads().len();
    h.global(
...
// ── load_session has modes; resume_session does not ───────────────────────────

#[tokio::test]
async fn load_session_has_modes_resume_does_not() {
    tokio::task::LocalSet::new()
        .run_until(async {
            let h = Harness::new();
            let sid = create_session(&h).await;
...
async fn two_sessions_have_independent_histories() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            assert!(
                matches!(r1.stop_reason, agent_client_protocol::schema::v1::StopReason::EndTurn),
                "s1 must complete with EndTurn: {:?}",
                r1.stop_reason
            );
            assert!(
                matches!(r2.stop_reason, agent_client_protocol::schema::v1::StopReason::EndTurn),
...
// ── Nonexistent session errors for set_session_mode/config_option ─────────────
...
async fn cancel_during_second_http_call_of_tool_round_via_nats() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let resp: PromptResponse = serde_json::from_slice(&payloads[1]).unwrap();
            assert!(
                matches!(resp.stop_reason, agent_client_protocol::schema::v1::StopReason::Cancelled),
...
async fn mcp_tool_round_trip_via_nats() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async move {
            let h = Harness::new();
            let server =
                agent_client_protocol::schema::v1::McpServer::Http(agent_client_protocol::schema::v1::McpServerHttp::new("web", mcp_url));
