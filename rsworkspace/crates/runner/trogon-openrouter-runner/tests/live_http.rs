...
use agent_client_protocol::schema::v1::{
    AuthenticateRequest, CancelNotification, ContentBlock, ForkSessionRequest, NewSessionRequest, PromptRequest,
...
/// on the agent.
#[tokio::test(flavor = "current_thread")]
async fn wire_model_field_reflects_default_model() {
    let srv = TestServer::new().await;
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            assert_eq!(
                body1["model"].as_str(),
                Some("model-a"),
                "default model must appear in wire request: {body1}"
            );
        })
        .await;
}
