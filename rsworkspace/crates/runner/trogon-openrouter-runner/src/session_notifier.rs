...
#[async_trait]
impl<T: SessionNotifier + Send + Sync> SessionNotifier for Arc<T> {
    async fn notify(&self, notification: SessionNotification) {
...
mod tests {
    ...
    async fn mock_notify_captures_notifications() {
        ...
        notifier.notify(test_notification("or-sess-1")).await;
        notifier.notify(test_notification("or-sess-2")).await;
    ...
    async fn arc_blanket_impl_delegates_to_inner() {
        ...
        notifier.notify(test_notification("arc-or-sess")).await;
