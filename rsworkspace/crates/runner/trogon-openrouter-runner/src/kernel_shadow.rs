//! Fase 4 (§1875 "runners como bindings/adapters") shadow emission for the OpenRouter runner.
//!
...
/// Convert the OpenRouter runner's own snapshot messages into canonical messages (§11
/// no-lossy: full role and text content preserved; OpenRouter snapshot messages are
/// text-only). The `message_id` is positional + stable so event-log replay is deterministic.
...
fn shadow_actor() -> Actor {
    Actor {
        ...
        id: "trogon-openrouter-runner".to_string(),
        ..Actor::default()
    }
}
    ...
    L: SessionLeaseFactory + Clone + Send + Sync + 'static,
{
    async fn record_turn(&self, session_id: &str, snapshot: &SessionSnapshot) {
        ...
        if let Err(err) = self
            ...
            .await
        {
            tracing::warn!(session_id, error = %err, "openrouter kernel shadow: record_conversation failed (non-fatal)");
        }
    }
}
...
pub async fn provision(js: &async_nats::jetstream::Context) -> Option<Arc<dyn ShadowRecorder>> {
    ...
    let snapshot_kv = match provision_snapshot_store(js, &config).await {
        Ok(kv) => kv,
        Err(err) => {
            tracing::warn!(error = %err, "openrouter kernel shadow: snapshot store provisioning failed; disabled");
            return None;
        }
    };
    let lease_kv = match provision_lease_store(js, &config).await {
        Ok(kv) => kv,
        Err(err) => {
            tracing::warn!(error = %err, "openrouter kernel shadow: lease store provisioning failed; disabled");
            return None;
        }
    };
    ...
    if let Err(err) = event_log
        ...
        .await
    {
        tracing::warn!(error = %err, "openrouter kernel shadow: event-log stream provisioning failed; disabled");
        return None;
    }
    let snapshots = SnapshotStore::new(snapshot_kv, config.clone());
    let leases = SessionLeaseManager::new(SessionKvLeaseFactory::new(lease_kv, &config), "trogon-openrouter-runner");
    let kernel = SessionKernel::new(config, event_log, snapshots, leases);

    tracing::info!("openrouter kernel shadow: canonical conversation shadow recording enabled");
...
mod tests {
    ...
    fn canonical_messages_preserve_role_text_and_model() {
        ...
        assert_eq!(canonical[0].model.as_deref(), Some("anthropic/claude-sonnet-4"));
    ...
    fn canonical_messages_empty_when_no_messages() {
        assert!(canonical_messages(&snapshot_with(vec![], Some("anthropic/claude-sonnet-4"))).is_empty());
    }
}
