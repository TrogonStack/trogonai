...
impl XaiClient {
    ...
    async fn do_chat_stream(
        ...
        let result = self
            ...
            .await;
        match result {
            Ok(response) => parse_sse(response.bytes_stream()).boxed(),
            Err(e) => {
                let msg = e.to_string();
                warn!(error = %msg, "xai: request failed");
                stream::once(async move { XaiEvent::Error { message: msg } }).boxed()
            }
        }
    }
...
fn parse_sse(bytes: impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static) -> impl Stream<Item = XaiEvent> {
    stream::unfold(
        SseState {
            stream: bytes.boxed(),
            buf: String::new(),
