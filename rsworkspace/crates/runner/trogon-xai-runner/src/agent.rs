...
use agent_client_protocol::schema::v1::{
    ...
    SessionConfigOptionValue, SessionConfigSelectOption, SessionForkCapabilities, SessionId, SessionInfo,
    SessionListCapabilities, SessionMode, SessionModeState, SessionNotification, SessionResumeCapabilities,
    SessionUpdate, SetSessionConfigOptionRequest, SetSessionConfigOptionResponse, SetSessionModeRequest,
    SetSessionModeResponse, StopReason, ToolCall, ToolCallStatus, ToolCallUpdate, ToolCallUpdateFields, ToolKind,
    UsageUpdate,
};
...
impl<H: XaiHttpClient, N: SessionNotifier, M: TrogonMdLoading> XaiAgent<H, N, M> {
    ...
    fn session_mcp_cache(&self, session_id: &str) -> Arc<trogon_runner_tools::SessionMcpCache> {
        let mut caches = self.session_mcp_caches.lock().unwrap_or_else(|e| e.into_inner());
        caches
            .entry(session_id.to_string())
    ...
    fn maybe_evict_oldest(
            ...
            .map(|(id, _)| id.clone())
        {
            ...
            session_locks.lock().unwrap_or_else(|e| e.into_inner()).remove(&oldest_id);
...
impl<
    H: XaiHttpClient + Send + Sync + 'static,
    N: SessionNotifier + Sync + 'static,
    M: TrogonMdLoading + Sync + Clone + 'static,
> acp_nats::AgentHandler for XaiAgent<H, N, M>
{
    ...
    async fn new_session(&self, req: NewSessionRequest) -> agent_client_protocol::Result<NewSessionResponse> {
        ...
        let permission_rules = load_trogon_md(&self.md_loader, &cwd)
            .await
    ...
    async fn close_session(&self, req: CloseSessionRequest) -> agent_client_protocol::Result<CloseSessionResponse> {
        ...
        self.session_locks.lock().unwrap_or_else(|e| e.into_inner()).remove(&session_id);
    ...
    async fn prompt(&self, req: PromptRequest) -> agent_client_protocol::Result<PromptResponse> {
        ...
        let trogon_md = load_trogon_md(&self.md_loader, &cwd).await;
            ...
            if !pending_tool_calls.is_empty() {
                ...
                for (call_id, name, arguments) in pending_tool_calls.drain(..) {
                    ...
                    let allowed = if name == "ask_user" {
                        ...
                        let mut rules = if let Some(tmd) = load_trogon_md(&self.md_loader, &cwd).await {
        ...
        let audit_entries = std::mem::take(&mut *audit_buf.lock().unwrap_or_else(|e| e.into_inner()));
...
/// Drives `TrogonMdLoading::load` (a `?Send` dependency future) on a
/// dedicated local task so it can be awaited from this crate's `Send`-bound
/// `AgentHandler` methods.
async fn load_trogon_md<M>(loader: &M, cwd: &str) -> Option<String>
where
    M: TrogonMdLoading + Clone + 'static,
{
    let loader = loader.clone();
    let cwd = cwd.to_string();
    match tokio::task::spawn_local(async move { loader.load(&cwd).await }).await {
        Ok(md) => md,
        Err(e) => {
            warn!(error = %e, "xai: trogon_md load task failed");
            None
        }
    }
}
...
async fn execute_bash_stateful(
    ...
    use agent_client_protocol::schema::v1::{
        CreateTerminalRequest, CreateTerminalResponse, EnvVariable, TerminalOutputRequest,
    };
...
mod tests {
    ...
    use agent_client_protocol::schema::ProtocolVersion;
    use agent_client_protocol::schema::v1::{
        AuthenticateRequest, CancelNotification, CloseSessionRequest, ContentBlock, ForkSessionRequest,
    ...
    async fn load_session_returns_state() {
        ...
        let resp = agent
            ...
            .await
            .unwrap();
        assert_eq!(resp.modes.unwrap().current_mode_id.to_string(), "default");
        assert_eq!(agent.test_session_model("s2").await.as_deref(), Some("grok-3-mini"));
    }
    ...
    async fn set_session_config_option_unknown_option_returns_full_config_options() {
        ...
        assert!(
            resp.config_options.iter().any(|o| o.id.to_string() == "compactor_model"),
            "compactor_model option must be advertised"
        );
    }
