...
use agent_client_protocol::schema::v1::{
    AuthMethod, AuthenticateRequest, CancelNotification, CloseSessionRequest, ContentBlock, EmbeddedResource,
    EmbeddedResourceResource, ForkSessionRequest, InitializeRequest, ListSessionsRequest, LoadSessionRequest,
    NewSessionRequest, PromptRequest, ResourceLink, ResumeSessionRequest, SetSessionConfigOptionRequest, StopReason,
...
#[tokio::test]
async fn new_session_creates_session_and_appears_in_list() {
...
#[tokio::test]
async fn fork_session_creates_independent_session() {
    let _guard = env_lock().lock().unwrap();
    let agent = make_agent(Arc::new(MockXaiHttpClient::new())).await;
...
async fn set_session_mode_unknown_mode_returns_error() {
    ...
    let err = agent
        .set_session_mode(agent_client_protocol::schema::v1::SetSessionModeRequest::new(
            session_id,
            "nonexistent-mode",
        ))
        .await
...
async fn set_session_mode_default_succeeds() {
    ...
    agent
        .set_session_mode(agent_client_protocol::schema::v1::SetSessionModeRequest::new(session_id, "default"))
        .await
...
async fn bash_tool_call_round_trips_through_wasm_runtime() {
    use agent_client_protocol::schema::v1::{CreateTerminalResponse, TerminalId, TerminalOutputResponse};
...
async fn binary_blob_resource_in_prompt_is_included_as_text() {
    use agent_client_protocol::schema::v1::BlobResourceContents;
    let _guard = env_lock().lock().unwrap();
    let mock = Arc::new(MockXaiHttpClient::new());
    mock.push_response(text_response(&["ok"]));
    let agent = make_agent(Arc::clone(&mock)).await;
...
/// A function-call response that signals a tool round so the agent dispatches the
/// call and makes a follow-up request with the tool output.
fn tool_call_round(call_id: &str, name: &str, args: &str) -> Vec<XaiEvent> {
    vec![
        XaiEvent::ResponseId {
            id: "resp_mcp".to_string(),
        },
        XaiEvent::FunctionCall {
            call_id: call_id.to_string(),
            name: name.to_string(),
...
#[tokio::test]
async fn mcp_tool_call_dispatched_and_result_fed_back() {
    let _guard = env_lock().lock().unwrap();

    // Stand up a mock HTTP MCP server: initialize + tools/list + tools/call.
    ...
    let mock = Arc::new(MockXaiHttpClient::new());
    // Round 1: model calls the prefixed MCP tool.
    mock.push_response(tool_call_round("call_mcp", "web__search", r#"{"query":"rust"}"#));
    // Round 2: model replies after receiving the tool output.
    mock.push_response(text_response(&["all done"]));
    ...
    agent
        .prompt(PromptRequest::new(
            sid.clone(),
            vec![ContentBlock::Text(TextContent::new("search rust"))],
        ))
        .await
        .unwrap();
    ...
    assert_eq!(
        calls.len(),
        2,
        "agent must make a follow-up call after MCP tool execution"
    );
    let follow_up = &calls[1].input;
    ...
    assert_eq!(
        mcp_output.as_deref(),
        Some("MCP RESULT: rust"),
        "follow-up turn must carry the MCP tool result as FunctionCallOutput"
    );
}
...
/// the elicitation channel and returns the answer as the tool output.
#[tokio::test]
async fn ask_user_round_trips_through_elicitation_channel() {
    let _guard = env_lock().lock().unwrap();

    let mock = Arc::new(MockXaiHttpClient::new());
    // Round 1: model asks the user a question.
    mock.push_response(tool_call_round(
        "call_ask",
        "ask_user",
        r#"{"question":"What is your favorite color?"}"#,
    ));
    // Round 2: model replies after receiving the answer.
    ...
    let responder = tokio::spawn(async move {
        if let Some(req) = elic_rx.recv().await {
            ...
            let q = req.request.message.clone();
            let mut content = std::collections::BTreeMap::new();
            content.insert(
                "answer".to_string(),
                agent_client_protocol::schema::v1::ElicitationContentValue::String("blue".to_string()),
            );
            let resp =
                agent_client_protocol::schema::v1::CreateElicitationResponse::new(agent_client_protocol::schema::v1::ElicitationAction::Accept(
    ...
    assert_eq!(
        ...
        "elicitation request must carry the model's question"
    );
    ...
    assert_eq!(
        answer.as_deref(),
        Some("blue"),
        "follow-up turn must carry the user's answer as the ask_user tool output"
    );
}

/// Without an elicitation channel, `ask_user` is not advertised; if the model
/// somehow calls it anyway, the runner returns a graceful unavailable message.
#[tokio::test]
async fn ask_user_unavailable_without_elicitation_channel() {
    let _guard = env_lock().lock().unwrap();
...
async fn xai_write_file_tool_dispatched_via_wire_format() {
    ...
    push_function_call(
        ...
        "cid-wf",
        "write_file",
        r#"{"path":"out.txt","content":"xai-write-sentinel-789"}"#,
    );
...
async fn xai_str_replace_replace_all_tool_def_and_dispatch() {
    ...
    let resp = agent
        .prompt(PromptRequest::new(
            sess.session_id.to_string(),
            vec![ContentBlock::Text(TextContent::new("replace all"))],
        ))
        .await
        .unwrap();
    assert_eq!(resp.stop_reason, StopReason::EndTurn);
    ...
    assert_fco_content(
        ...
        "cid-sr-all",
        |o| !o.contains("Unknown tool"),
        "replace_all must be dispatched",
    );
}
...
async fn xai_multi_edit_tool_def_and_dispatch() {
    ...
    let resp = agent
        .prompt(PromptRequest::new(
            sess.session_id.to_string(),
            vec![ContentBlock::Text(TextContent::new("multi edit"))],
        ))
        .await
        .unwrap();
    assert_eq!(resp.stop_reason, StopReason::EndTurn);
    ...
    let responder = tokio::spawn(async move {
        if let Some(req) = elic_rx.recv().await {
            *cq.lock().unwrap() = Some(req.request.message.clone());
            ...
            content.insert(
                "answer".to_string(),
                agent_client_protocol::schema::v1::ElicitationContentValue::String("teal".to_string()),
            );
            let resp =
                agent_client_protocol::schema::v1::CreateElicitationResponse::new(agent_client_protocol::schema::v1::ElicitationAction::Accept(
                    agent_client_protocol::schema::v1::ElicitationAcceptAction::new().content(content),
