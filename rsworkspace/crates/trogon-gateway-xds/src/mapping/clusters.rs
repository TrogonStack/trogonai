...
use crate::proto::envoy::config::endpoint::v3::{ClusterLoadAssignment, Endpoint, LbEndpoint, LocalityLbEndpoints};
...
pub fn map_clusters(snapshot: &GatewayConfigSnapshot) -> Vec<Cluster> {
    snapshot
        ...
        .map(|backend| Cluster {
            ...
            connect_timeout: Some(prost_types::Duration { seconds: 5, nanos: 0 }),
                        ...
                        host_identifier: Some(lb_endpoint::HostIdentifier::Endpoint(Endpoint {
                            address: Some(Address {
                                address: Some(address::Address::SocketAddress(SocketAddress {
                                    ...
                                    port_specifier: Some(socket_address::PortSpecifier::PortValue(endpoint.port)),
                                    ..Default::default()
                                })),
                            }),
                            ..Default::default()
...
mod tests {
    ...
    fn assignment_round_trip_preserves_endpoint_socket() {
        ...
        let host = endpoint.host_identifier.as_ref().expect("host identifier");
        match host {
